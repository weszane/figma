...
import { Gq, yW } from '../figma_app/38430';
import { hasMonetizedResourceMetadata, HubTypeEnum, ResourceTypeNoComment, ShelfViewType } from '../figma_app/45218';
...
import { canAssociatedUserPurchaseResource, filterPublishedResources, filterResourcesByOrgId, getPluginVersion, isValidForSelectedViewAndWhitelist } from '../figma_app/300692';
...
function ec({
  ...
  let r = initiateResourcePurchaseFlow(e);
  return jsxs('div', {
    'className': I4,
    'data-not-draggable': !0,
    'children': [jsx('div', {
...
function eG(e) {
  ...
  let g = getResourcePaymentFromState(resource);
  let v = isResourcePaymentFailed(resource);
  let j = isSubscriptionActive(g);
  let w = useSelector(e => e.authedActiveCommunityProfile);
  let b = _$$b();
  let E = useCurrentUserOrg();
  let P = useAllowlistedPlugins();
  let M = useAllowlistedWidgets();
  let S = Object.values(getLocalPlugins()).find(t => String(t.plugin_id) === e.resourceId);
  let {
    dragState,
...
function e7() {
  ...
  let s = useLocalPluginsExcludingWidgets({
...
function tM(e) {
  ...
  let y = filterDevModeEntriesAndMemo(Object.values(j)).map(e => e.plugin_id);
  let w = useAllowlistedWidgets();
  let b = useMemo(() => Object.keys(w), [w]);
  let L = useInstalledPluginsAndWidgets();
  let I = useRunnableInstalledPluginsAndWidgets(L);
  let E = I.orgWidgets;
  let P = I.orgPlugins;
  ...
  let B = filterArrayByEditorTypeAndMemo(M);
  B = filterDevModeEntriesAndMemo(B);
  ...
  let A = filterArrayByEditorTypeAndMemo(H);
...
function tJ({
  ...
  } = useExtensionAllowlist(l);
  let f = filterDevModeEntriesAndMemo(Object.values(allowList)).map(e => e.plugin_id);
  ...
  return useMemo(() => {
    ...
    return publicExtensionsDisallowed || n.length === 0 ? null : jsxs(TrackingProvider, {
...
function t1(e) {
  ...
  let s = t.widgets;
  let r = Object.values(t.plugins).sort(comparePluginsByName).filter(e => e.installed_by === 'user');
  let a = memoizeByArgs(() => r);
  let i = filterArrayByEditorTypeAndMemo(a());
  let l = (i = filterDevModeEntriesAndMemo(i)).map(e => e.plugin_id);
  let o = Object.values(s).sort(comparePluginsByName).filter(e => e.installed_by === 'user');
  let d = memoizeByArgs(() => o);
  let c = filterArrayByEditorTypeAndMemo(d()).map(e => e.plugin_id);
...
function t3(e) {
  ...
  return t.length === 0 ? null : jsxs(TrackingProvider, {
    name: 'saved',
    children: [jsx('div', {
      className: to,
...
function t2(e) {
  ...
  c = filterDevModeEntriesAndMemo(c);
  ...
  let _ = t1(l);
  if (!0 === i.loaded && c.length + _.length === 0) {
    return jsxs(TrackingProvider, {
...
function st(e) {
  ...
  let J = useDedupedRecentlyUsedPlugins();
  let ee = useDedupedRecentlyUsedWidgets();
  let es = getLocalPlugins();
  let er = useLocalPluginsExcludingWidgets();
  let ea = useAllowlistedPlugins();
  let en = useAllowlistedWidgets();
  let ei = usePublishedPlugins();
  let el = usePluginedWidgets();
  ...
  let ef = (!ev || isWhitelistEnforcedAndRequestsAllowed({
    org: ed,
    isWidget: r
...
function sc(e) {
  ...
  let p = jsxs(Fragment, {
    ...
    }), e.currentView === 'recents_and_saved' && jsx(t2, {
      viewExpandedList: e.viewExpandedList
    }), e.currentView === 'development' && jsx(e7, {}), e.currentView === 'org' && jsx(tR, {
      viewExpandedList: e.viewExpandedList
    })]
  });
  return jsxs('div', {
...
function s_(e) {
  ...
  return (l && d.push('development'), d.length !== 1 || r) ? jsxs(l6, {
