...
import { aT, b1, tR, XC } from '../905/512783'
...
import { Ao, Ko, LX, xN } from '../figma_app/70421'
...
class I {
  ...
  assertIndexes(e) {
    let t = this.leftToRight.length
    this.windowAssert(this.leftIdx === 0 || this.leftToRight[this.leftIdx - 1].x < e.x)
    this.windowAssert(this.leftIdx === t || this.leftToRight[this.leftIdx].x >= e.x)
    this.windowAssert(this.rightIdx === 0 || this.leftToRight[this.rightIdx - 1].x <= e.x + e.width)
    this.windowAssert(this.rightIdx === t || this.leftToRight[this.rightIdx].x > e.x + e.width)
    this.windowAssert(this.topIdx === 0 || this.topToBottom[this.topIdx - 1].y < e.y)
    this.windowAssert(this.topIdx === t || this.topToBottom[this.topIdx].y >= e.y)
    this.windowAssert(this.bottomIdx === 0 || this.topToBottom[this.bottomIdx - 1].y <= e.y + e.height)
...
let D = class e extends HTMLElement {
  ...
  clusterComesFromFrameMove(e, t) {
    ...
    for (let n of t) {
      if (e.anchoredNode === n.anchoredNode && o()(i, n.pins.map(e => e.id).sort()))
        return !0
    }
    return !1
  }
  ...
  processDatasetUpdate(e) {
    ...
    let o = [...n.values()].filter((e) => {
      let t = r.get(e.id)
      return !!t && (e.isPinnedToFile || this.ds.clusters.minimizeUnpinned || this.ds.selectedPinId !== null || this.hasHighlightedPinnedThreads ? xN(e, t) : (s = !0, !0))
    })
  ...
  focusOneOf(e) {
    for (let t of (this.focusOneOfIDs = e, this.slidingWindow.allValuesInWindow())) {
      if (t.pins.map(e => e.id).some(e => this.focusOneOfIDs.has(e))) {
  ...
  updateElementTransforms(e) {
    for (let t of this.slidingWindow.allValuesInWindow()) {
      ...
      if (i && t.pins.length === 1) {
        ...
        i && i.positionClusterElement(t, e)
      }
    }
  }
  ...
  addClusterToDom(t, i, r) {
    let s
    if (this.clusterElementsById.get(t.id))
      return
    let o = null
    if (t.isSinglePin) {
      let r = t.pins[0]
      ...
      nt.commentsA11y.getValue() && (a.onkeydown = (e) => {
        e.key === 'Enter' && this.onPinElementClicked(t.id)
      })
    ...
    if (this.clusterElementsById.set(t.id, s), this.wrapper.append(s), !this.ds.clusters.applyInstant && r === 'data' && s.animateDatasetChange && t.isSinglePin) {
    ...
    else if (!this.ds.clusters.applyInstant && o && r === 'aggregation') {
  ...
  removeClusterFromDom(e, t, i, n) {
    ...
    if (!(n || this.ds.clusters.applyInstant)) {
      if (i === 'data' && a.animateDatasetChange) {
        r = a.animateDatasetChange({
          type: 'destroy',
        })
      }
      else if (s && i === 'aggregation') {
