...
export class $$o0 {
  ...
  onFrame() {
    ...
    let i = e || t;
    let r = glU?.displayedRenderChangeCounterIndex() ?? 0;
    let a = glU?.latestRenderChangeCounterIndex() ?? 0;
    ...
    if (s && null !== this._renderTreeSwapStartTime) {
      ...
      let t = !glU?.lastChangeToDisplayedRenderTreeIncludesLocalEdit();
    ...
    let o = CeL?.getRenderTreeStaleTimeMs() ?? 0;
    if (this._lastFrameStaleness = o, 0 !== this._lastFrameTime) {
      ...
      this._collectBenchmarkOnlyMetrics && (this._benchmarkOnlyMetrics.allFramesViewportCoverage.push({
        value: glU?.estimatedViewportRenderCoverage(pWM.DEFINITELY_COVERED_ONLY) ?? 0,
        duration: e
      }), this._benchmarkOnlyMetrics.allFramesMinSharpness.push({
        value: glU.minViewportSharpness(),
        duration: e
      }), this._benchmarkOnlyMetrics.allFramesAvgSharpness.push({
        value: glU.avgViewportSharpness(),
        duration: e
      }));
    }
