...
export const LeafItem = memo<LeafItemProps>(({
  ...
  const shouldShowBorder = useMemo(() => !!backgroundColor && !!getThemeBorderStyle(themeContext, backgroundColor, visibleTheme), [backgroundColor, themeContext, visibleTheme]);
  const {
    setKeyboardNavigationElement
  } = useKeyboardNavigationItemForLeaf(i, numCols, !visible);
  const [containerClass, thumbnailClass] = useMemo(() => {
    if (gridLayoutMode === rp.SMALL || gridLayoutMode === rp.NORMAL) {
      return [GRID_CONTAINER_CLASS, 'drilldown_item--leafGridThumbnailContainer--sFOtz drilldown_item--_gridThumbnailContainer--2HzOl drilldown_item--_thumbnailContainer--kDsBt'];
    } else if (gridLayoutMode === rp.WIDE) {
      return [GRID_CONTAINER_CLASS, 'drilldown_item--leafWideGridThumbnailContainer--bcL9L drilldown_item--_gridThumbnailContainer--2HzOl drilldown_item--_thumbnailContainer--kDsBt'];
    } else {
      return ['drilldown_item--leafListContainer--BPWKm drilldown_item--_drilldownItemBase--YOfRd', 'drilldown_item--leafListThumbnailContainer--rTXS7 drilldown_item--_thumbnailContainer--kDsBt'];
    }
  }, [gridLayoutMode]);
  const tooltipAttributes = useMemo(() => {
    const tooltip = getLeafItemTooltip ? getLeafItemTooltip(drilldownItem.item, {
  ...
  if (fauxFocused && selected) {
    focusClass = isList ? 'drilldown_item--selectedListFauxFocused--jZ8Gn drilldown_item--selectedList--Y6KmC drilldown_item--listFauxFocused--0tIb6' : 'drilldown_item--selectedGridFauxFocused--Id7cL drilldown_item--selectedGrid--qI35b drilldown_item--gridFauxFocused--g8Aez';
  } else if (fauxFocused) {
    focusClass = isList ? 'drilldown_item--listFauxFocused--0tIb6' : 'drilldown_item--gridFauxFocused--g8Aez';
  } else if (selected) {
    focusClass = isList ? 'drilldown_item--selectedList--Y6KmC' : 'drilldown_item--selectedGrid--qI35b';
  }
  const thumbnail = getLeafItemThumbnail?.(drilldownItem.item, gridLayoutMode);
  const nameElement = shouldShowName && jsx(ph, {
    className: isList ? 'drilldown_item--displayText--pdJKd text--fontPos11--2LvXf text--_fontBase--QdLsd' : 'drilldown_item--displayTextWithMargin--A4LD0 drilldown_item--displayText--pdJKd text--fontPos11--2LvXf text--_fontBase--QdLsd',
  ...
  const mouseEventHandler = useHandleMouseEvent(`${recordingKey}${multiselect ? '' : '__UNUSED'}`, 'click', handleClick);
  const handleCheckboxChange = useCallback((_, {
    event
  }: {
    event: any;
  }) => {
        ...
        children: [content, jsx('div', {
          ...
          children: jsx(ManuallyLabeledCheckbox, {
            ...
            htmlAttributes: {
              ...
              onKeyDownCapture: (e: any) => handleKeyboardEventByState(e.nativeEvent)
            }
          })
        })]
      })
