...
import { gw, ms, OR } from '../figma_app/236327'
...
export function getDropdownTypeProps(props: { type?: number }): { type?: DropdownType } {
  // Return strongly typed object based on input type
  switch (props.type) {
    case DropdownType.LIGHT:
      return { type: DropdownType.LIGHT }
    case DropdownType.MATCH_BACKGROUND:
      return { type: DropdownType.MATCH_BACKGROUND }
    case DropdownType.DEFAULT:
      return { type: DropdownType.DEFAULT }
    default:
...
class PointingDropdownInner extends PureComponent<any, any> {
  ...
  private getContentLocation = (): Point => {
    ...
    switch (leanDirection) {
      case 'center':
        return new Point(
          targetRect.left + targetRect.width / 2 - contentRect.width / 2,
          verticalPosition,
        )
      case 'left':
        return new Point(
          targetRect.right + leanPadding - contentRect.width,
          verticalPosition,
        )
      case 'right':
        return new Point(
          targetRect.left - leanPadding,
          verticalPosition,
        )
      default:
    ...
    switch (this.props.type) {
      case DropdownType.LIGHT:
        return {
          ...
          ...(elementType === 'arrow' ? { border: '0.5px solid rgba(0, 0, 0, 0.2)' } : {}),
        }
  ...
  componentDidUpdate(prevProps: any, _prevState: any) {
    ...
    const targetRectChanged
      = prevProps.targetRect.bottom !== this.props.targetRect.bottom
      || prevProps.targetRect.left !== this.props.targetRect.left
      || prevProps.targetRect.right !== this.props.targetRect.right
      || prevProps.targetRect.top !== this.props.targetRect.top
      || prevProps.targetRect.width !== this.props.targetRect.width
      || prevProps.targetRect.height !== this.props.targetRect.height
  ...
  private renderDropdownOptions() {
    const dropdownOptions = jsx('div', {
      ...
      "className": this.props.orientation === 'horizontal'
        ? 'pointing_dropdown--horizontalOrientation--z9DRY'
        : '',
  ...
  render() {
    ...
    const targetRect = this.props.targetRect
    const arrowLeftPosition = this.props.arrowOffsetFromRectLeft
      ? targetRect.left + this.props.arrowOffsetFromRectLeft - 5.5
      : targetRect.left + targetRect.width / 2 - 5.5

    const arrowVerticalPosition = this.props.displayAboveTarget
      ? targetRect.top - 7
      : targetRect.bottom
    ...
    const dropdownContent = this.props.propagateCloseClick
      ? jsxs(ms, {
...
export function PointingDropdown(props: any) {
  ...
  return jsx(ConnectedPointingDropdown, {
    ...
    'children': Children.map(childrenComponents, (child: any, index) =>
      cloneElement((child), {
...
export class DropdownScrollContainer extends Component<any, any> {
  ...
  private updateScrollIndicatorVisibility = () => {
    ...
    if (
      showTopIndicator !== this.state.showTopScrollIndicator
      || showBottomIndicator !== this.state.showBottomScrollIndicator
    ) {
      this.setState({
