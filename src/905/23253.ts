...
export function findImageFillByHash({
  ...
  const fill = node.fills.find((f: any) => f.image && f.image.hash && uint8ArrayToBase64(f.image.hash) === hash);
  if (!fill) {
...
export async function getImagePaintSignedUrl(paint: any) {
  ...
  if (!url && (await new Promise(resolve => setTimeout(resolve, 1000)), !(url = (signedUrls = getImageManager().getSignedUrls([hashHex])).get(hashHex)))) {
...
async function checkAiMeterUsage({
  ...
  const wbInstance = getCurrentLiveGraphClient();
  const state = debugState.getState();
  const fileKey = state.openFile?.key;
  if (wbInstance && fileKey) {
    await new Promise<void>((resolve, reject) => {
      const unsubscribe = wbInstance.subscribe(AiMeterUsageView, {
        fileKey
      }, (result: any) => {
...
export async function modifyImages({
  ...
  try {
    if (nodeMap.size > 1) {
      await checkAiMeterUsage({
        name,
        needed: nodeMap.size
      });
    }
    await Promise.allSettled(Array.from(nodeMap.entries()).map(async ([hash, nodes]) => {
      try {
        nodes.forEach(node => {
        ...
        if (error instanceof ImageTooLargeError) errorCounts.too_large_error_count++;else if (error instanceof ImageTooSmallError) errorCounts.too_small_error_count++;else if (error instanceof AlreadyMaxUpscaledError) errorCounts.already_max_upscaled_error_count++;
    ...
    if (modifiedNodes.every(n => n.error)) {
      ...
      if (modifiedNodes.every(n => n.error?.message === firstError?.message)) throw firstError;
      throw new ImageModificationError('All images failed to modify', {
        reportToSentry: true
      });
    }
    ...
    let reason = sZ(error);
    if (error instanceof MeterExceededError) reason = ErrorType.METER_EXCEEDED;
...
function updateNodePaint({
  ...
  permissionScopeHandler.ai('image-modification', () => {
    const fillIndex = node.fills.findIndex((f: any) => f.type === 'IMAGE' && f.image?.hash && uint8ArrayToBase64(f.image.hash) === previousHash);
    getMatchingNodesToUpdateForQuery(node, 'fill-paint-data').forEach((n: any) => {
      if (fillIndex !== -1) {
        ...
        const fills = [...n.fills];
        if (modifyPaint) modifyPaint(newPaint);
...
function handlePickerShown(paintId: string) {
  const state = debugState.getState();
  if (state.mirror.appModel.currentSelectedProperty.type !== NodePropertyCategory.FILL) return;
  ...
  if (initialX !== undefined && initialY !== undefined) {
    debugState.dispatch(showPickerThunk({
      id: paintId,
      initialX,
      initialY,
      data: {
        isInStyleModal: false
      }
    }));
  }
}
