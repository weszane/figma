...
function SubpathPreview({ item, isList }: SubpathPreviewProps) {
  const { subpathGridHeight } = useContext({} as any) // DrilldownContext would be passed here
...
export const SubpathItem = memo<SubpathItemProps>(({
  ...
  const { setKeyboardNavigationElement } = useKeyboardNavigationItemForSubpath(
    i, 
    drilldownItem.id, 
    !visible
  )
  ...
  const { subpathGridHeight, subpathListHeight } = useContext({} as any) // DrilldownContext would be passed here
  ...
  return jsxs(ButtonPrimitive, {
    ...
    className: g()(
      isList
        ? 'drilldown_item--subpathContainerList--n-4-2 drilldown_item--_drilldownItemBase--YOfRd drilldown_item--_subpathContainerBase--krA4j'
        : 'drilldown_item--subpathContainerGrid--8bV5l drilldown_item--_drilldownItemBase--YOfRd drilldown_item--_subpathContainerBase--krA4j',
      Qx
    ),
