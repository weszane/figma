...
export function setupDefaultEmbedWidget({
  ...
  const title = H(widget, 'title', {
    name: 'Inter Medium',
    size: 18,
    lineHeight: 22
  }, scaledDimensions.width - 48, 2, false);
  const description = H(widget, 'description', {
    name: 'Inter',
    size: 13,
    lineHeight: 20
  }, scaledDimensions.width - 48, 2, false);
  ...
  widget.useEffect(() => {
    if (thumbnailImageHash && !resolvedThumbnailImageHash) {
      ...
      if (imageData) setResolvedThumbnailImageHash(figmaApi.createImage(imageData).hash);
    }
  });
  widget.useEffect(() => {
    if (faviconImageHash && !resolvedFaviconImageHash) {
      const imageData = _$$L(faviconImageHash);
      if (imageData) setResolvedFaviconImageHash(figmaApi.createImage(imageData).hash);
    }
  });
  widget.useEffect(() => {
  ...
  const renderProviderIcon = () => {
    ...
    if (resolvedFaviconImageHash) {
      return widget.h(Frame, {
        ...
        fill: {
          type: 'image',
          imageHash: resolvedFaviconImageHash,
          scaleMode: 'fill'
        },
        cornerRadius: 3
      });
  ...
  return widget.h(AutoLayout, {
    ...
    fill: resolvedThumbnailImageHash ? {
      type: 'image',
      imageHash: resolvedThumbnailImageHash,
      scaleMode: 'fill'
    } : gray400Color,
    cornerRadius: K4
  }, widget.h(AutoLayout, {
    ...
    lineHeight: 22,
    ...textStyle
  }, title), description && widget.h(Text, {
    width: 'fill-parent',
    fontSize: 13,
    fontWeight: 400,
    letterSpacing: '-0.25%',
    ...
    width: Number(scaledDimensions.width),
    height: Number(scaledDimensions.height)
  }, embedType === 'autoplayable_video' || embedType === 'autoplayable_audio' ? widget.h(SVG, {
    name: 'play-button',
    src: Rr,
    onClick: () => {
...
export function registerEmbedWidget({
  ...
  function renderEmbedWidgetVersion(embedVersionId: string) {
    switch (embedVersionId) {
      case jU.EMBED_VERSION_ID_V0:
        return setupDefaultEmbedWidget({
          figma
        });
      case jU.EMBED_VERSION_ID_V1:
        return renderEmbedWidgetV1({
          figma
        });
      default:
        return setupDefaultEmbedWidget({
  ...
  function renderEmbedWidgetV1({
    ...
    // Title and description
    const title = H(_widget, 'title', {
      name: 'Inter Medium',
      size: 16,
      lineHeight: 24
    }, scaled.width - 96, 1, false);
    const description = H(_widget, 'description', {
      name: 'Inter',
      size: 13,
      lineHeight: 24
    }, scaled.width - 96, 1, false);
    ...
    const srcUrlText = H(_widget, 'srcUrl', {
      name: 'Inter Medium',
      size: 16,
      lineHeight: 24
    }, scaled.width - 96, 1, true, true);

    // Effects for resolving images
    _widget.useEffect(() => {
      if (thumbnailImageHash && !resolvedThumbnailImageHash) {
        const imageData = _$$L(thumbnailImageHash);
        if (imageData) setResolvedThumbnailImageHash(figma.createImage(imageData).hash);
      }
    });
    _widget.useEffect(() => {
      if (faviconImageHash && !resolvedFaviconImageHash) {
        const imageData = _$$L(faviconImageHash);
        if (imageData) setResolvedFaviconImageHash(figma.createImage(imageData).hash);
      }
    });
    _widget.useEffect(() => {
    ...
    return figma.widget.h(_AutoLayout, {
      ...
      width: Number(scaled.width),
      height: Number(scaled.height)
    }, embedType === 'autoplayable_video' || embedType === 'autoplayable_audio' ? figma.widget.h(SVG, {
      name: 'play-button',
      src: Rr,
      onClick: () => {
        trackEmbedFileEvent(LinkMetadataEvent.EMBED_CLICK);
    ...
    }, hasTitle || hasDescription ? renderTextContainer(renderTitleText(title), description && figma.widget.h(Text, {
      width: 'fill-parent',
      fontSize: 13,
      fontWeight: 400,
      letterSpacing: '-0.25%',
    ...
    }, description)) : renderTextContainer(renderTitleText(srcUrlText)), figma.widget.h(_AutoLayout, {
      name: 'provider-container',
      direction: 'horizontal',
      horizontalAlignItems: 'start',
      verticalAlignItems: 'center',
  ...
  widget.register(() => {
    ...
    return resolved ? renderEmbedWidgetVersion(embedVersionId) : renderLoadingEmbedWidget();
  });
}
