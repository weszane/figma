...
export const gpuFullscreenEventNames = new Set([
  'initialize-gpu',
  'render-tree-generation',
  'mp-receive-schema',
  'mp-receive-decompress-buffer',
  'mp-receive-decode-buffer-node-change',
  'mp-receive-decode-buffer-other',
  'handle-changes-from-server',
  'handle-joining-changes-from-server',
  'playback-joining-messages',
  'synthesize-missing-geometry-cache-changes',
  ...
  'mp-ws-onmessage',
  'mp-ws-onopen',
  'receiveSceneGraphReply',
])

/** Set of time event names. Original variable: A */
const timeEventNames = new Set([
  'initialize-gpu.start',
  'initialize-gpu.end',
  'AFTER_FIRST_RENDER',
  'createStoreStart',
  'initAndHydrateActionNoUserStateStart',
  'initAndHydrateActionNoUserStateEnd',
  'hydrateActionWithUserStateStart',
  'hydrateActionWithUserStateEnd',
  'initialReactRenderStart',
  'initialReactRenderEnd',
  'loadingBinaryStart',
  'loadingCodeStart',
  'instantiateStreamingStart',
  'instantiateStreamingEnd',
  'awaitInstantiateStreamingStart',
  'awaitInstantiateStreamingEnd',
  'instantiateStart',
  'instantiateEnd',
  'awaitInstantiateStart',
  'awaitInstantiateEnd',
  'callMainStart',
  'callMainEnd',
  'editorPreloaded',
  'loadAndStartFullscreen',
  'fullscreenIsReady',
  'appEntryStart',
  'appEntryEnd',
  'fileMetadataRequestStart',
  'fileMetadataRequestEnd',
])
...
class FullscreenPerfManager {
  ...
  wsMaxMsgInfo: { size?: number, timestamp?: number } = { size: undefined, timestamp: undefined }
  wsTotalMsgSize = 0
  ...
  reportAbandon = (): void => {
    if (this.reportedLoad || this.reportedAbandon)
      return
  ...
  reportFocusGained = (): void => {
    if (this.reportedLoad || this.reportedFocusGained)
      return
  ...
  reportBackgrounded = (): void => {
    if (this.reportedLoad || document.visibilityState !== 'hidden' || this.reportedBackgrounded)
      return
  ...
  reportOffline = (): void => {
    if (this.reportedLoad || this.reportedOffline)
      return
  ...
  startFs(event: string): void {
    ...
    if (key !== null) {
      startPerformanceSpan(key, _$$e.SCENEGRAPH_AND_SYNC)
      this.mark(`${key}.start`)
    }
  }
  ...
  logWsMsg(event: string, size?: number, timestamp?: number): void {
    ...
    if (event === 'mp-ws-onopen') {
      ...
      this.wsMaxMsgInfo = { size: undefined, timestamp: undefined }
    ...
    if (eventCount === 0)
      startPerformanceSpan(rootEvent, _$$e.SCENEGRAPH_AND_SYNC, time)
    else if (eventCount === 1)
      endPerformanceSpan(rootEvent, time)
    const wsKey = this.wsHitsManager.getKeyWithIndex(event)
    if (wsKey !== null)
      this.wsEventTimes[wsKey] = time
    ...
    if (wsSizeKey !== null && size)
      this.wsMsgSizes[wsSizeKey] = size
  ...
  logOpenFileAction(fileKey: string, isVisibleLoad: boolean, folderId: string, teamId: string, orgId: string) {
    ...
    for (const key in this.timeEvents) {
      if (!timeEventNames.has(key))
        delete this.timeEvents[key]
    }
  ...
  time<T>(event: string, fn: () => T): T {
    startPerformanceSpan(event, _$$e.SCENEGRAPH_AND_SYNC)
  ...
  get isColdBoot(): boolean {
    return !this.timeEvents.editorPreloaded
      && !this.timeEvents.androidProtoEditorPreloaded
      && !this.timeEvents.mobileFileViewerPreloaded
      && this.numTimesLoaded === 1
      && !!getInitialOptions().editing_file
  }
  ...
  getEventsToReport(includeGpuInfo: boolean) {
    ...
    function getAppJsTiming() {
      ...
      const preloads = allLink
        .filter(e => e.href.includes('webpack') && (e.href.endsWith('.js') || e.href.endsWith('.js.gz') || e.href.endsWith('.js.br')))
        .map(e => e.href)
    ...
    function getResourceTiming() {
      ...
      for (const entry of performance.getEntriesByType('resource')) {
        if (cssResourceRegex.test(entry.name))
          result.appCss = entry as PerformanceResourceTiming
        else if (entry.name === Fig.fullscreenURLs['compiled_wasm.wasm'])
          result.wasm = entry as PerformanceResourceTiming
        else if (entry.name === Fig.fullscreenURLs['compiled_wasm.js'])
          result.wasmJs = entry as PerformanceResourceTiming
      }
      const navEntries = performance.getEntriesByType('navigation')
      if (navEntries.length === 1)
        result.pageNavigation = navEntries[0] as PerformanceNavigationTiming
      return result
    }
    ...
    function aggregateEvents(
      ...
      resourceTiming: any,
      meta: any,
      includeAppJsDetails: boolean,
    ) {
      const result: any = { times, wsMsgSizes, meta: meta || {} }
      if (appJsTiming) {
    ...
    const events = aggregateEvents(
      { ...this.timeEvents, ...this.wsEventTimes },
      this.wsMsgSizes,
      getAppJsTiming(),
      getResourceTiming(),
      this.getFilePerfMetadata(),
      includeGpuInfo,
    )
  ...
  getClientRenderedMetadata() {
    if (this._loadID === null) {
      reportError(_$$e.FIGFILE_PLATFORM, new Error('reporting client-rendered metadata without load ID'))
    }
    return {
      loadID: this._loadID ?? '',
