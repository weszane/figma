...
import { PropTypes, validateWithVm } from '../905/816730';
...
class WidgetRenderScheduler {
   ...
   */
  schedule(): void {
    widgetInteractionTracker.renderWasScheduled();
    if (this.isRendering) {
      this.needsReschedule = true;
    }
    if (!this.currentTask) {
...
export class WidgetManager {
  ...
  getCurrentWidgetNodeId(): string {
    if (!this.currentWidgetId) throw new Error('Not actively rendering widget');
    return this.currentWidgetId;
  }
  ...
  getRenderingState(widgetId: string): WidgetRenderingState {
    ...
    if (!state) throw new Error('Widget rendering state not found');
    return state;
  }
  ...
  scheduleRender(widgetId: string, _force: boolean = false): void {
    if (!this.widgetFunction) throw new Error('Widget has not been registered');
  ...
  maybeRunEffects(widgetId: string): void {
    ...
    if (!effectManager.hasRunEffects) effectManager.runEffects();
  }
      ...
      const state = {
        ...
        renderScheduler: new WidgetRenderScheduler(async () => {
          try {
            ...
            const allowedDomains = pluginManifest?.manifest.networkAccess?.allowedDomains ?? DEFAULT_ALLOWED_ORIGINS;
            const {
              imgInfoMap,
              vRoot
            ...
            permissionScopeHandler.plugin('widget-rerender', () => {
              ...
              widgetInteractionTracker.didReconciliation(duration);
            });
            ...
            if (err instanceof InternalError || isDevEnvironment()) {
              ...
              if (typeof err === 'string') {
                ...
                reportError(ServiceCategories.EXTENSIBILITY, errorObj);
              } else {
                reportError(ServiceCategories.EXTENSIBILITY, err);
              }
            }
   ...
   */
  renderWidgetTree(widgetId: string, renderMode: 'current' | 'previous'): {
    rootNode: any;
    syncedState: any;
  } | RequestError {
    if (!this.widgetFunction) throw new Error('Widget has not been registered');
    try {
      ...
      widgetInteractionTracker.didRender(afterRender - start, afterRender - afterCall);
  ...
  addShutdownAction(action: () => void): void {
    if (this.isCleared) throw new Error('Cannot add shutdown action after widget manager has been cleared');
  ...
  releaseRenderingStateHandles(state: WidgetRenderingState): void {
    ...
    if (this.isValidFunctionHandle(t)) this.vm.releaseHandle(t);
    const {
      attachedStickablesChangedHandle,
      stuckStatusChangedHandle
    } = state.stickableState;
    if (this.isValidFunctionHandle(attachedStickablesChangedHandle)) this.vm.releaseHandle(attachedStickablesChangedHandle);
    if (this.isValidFunctionHandle(stuckStatusChangedHandle)) this.vm.releaseHandle(stuckStatusChangedHandle);
  }
}
