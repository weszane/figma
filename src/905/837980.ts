...
function getHeaderText(modalType: ModalType): string {
  return isTrialEnded(modalType) ? renderI18nText('community.buyer.your_trial_has_ended') : renderI18nText('community.buyer.unlock_this_feature');
}
 ...
 */
function getSecondaryText(modalType: ModalType, pluginName: string, isLoggedOut: boolean, isTrialEnded: boolean): string {
  if (isLoggedOut) {
...
export let FreemiumApiPreCheckoutModal = registerModal((props: FreemiumApiPreCheckoutModalProps) => {
  ...
  const secondaryText = getSecondaryText(props.type, props.plugin.name, isUserLoggedOut, isTrialEnded);
  ...
  return jsx(Xj, {
    ...
    children: jsxs('div', {
      ...
      children: [jsx(ThemeIcon, {}), jsx('div', {
