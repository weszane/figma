...
export function $$ea0(e) {
  ...
  let D = useCallback((e, t) => {
    ...
    e.folderPerms?.canSkipDeletionConfirmation
      ? w({
          folderId: e.id,
        }).catch(() => {
  ...
  return jsx(_$$A, {
    ...
    handleContextMenu: e.onContextMenuOverride
      ? e.onContextMenuOverride
      : (e, t) => {
          t.stopPropagation()
          t.preventDefault()
          let i = e[0]
    ...
    listViewProps: {
      columns,
      gutterColumn,
      currentSort: e.currentSortOverride
        ? e.currentSortOverride
        : {
            field: x?.sortKey,
