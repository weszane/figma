...
export type FunctionWrapper<TArgs extends any[] = any[], TReturn = any> = (
  this: INoOpVm<any>,
  ...args: INoOpVm<TArgs[number]>[]
) => INoOpVm<TReturn> | TReturn;
...
export class NoOpVm extends PluginWrapper {
  ...
  newFunction<TArgs extends any[] = any[], TReturn = any>(
    name: string,
    fn: FunctionWrapper<TArgs, TReturn>,
  ): INoOpVm<(...args: any[]) => any> {
    const vm = this;
  ...
  setProp<T = any>(
    objHandle: INoOpVm<object>,
    prop: string,
    valueHandle: INoOpVm<T>,
  ): void {
    const obj = unwrapHandle(this, objHandle);
  ...
  defineProp<T = any>(
    ...
    if ("get" in desc && desc.get) {
      descriptor.get = unwrapHandle(
        this,
        this.newFunction(`get_${prop}`, desc.get),
      );
    }
    if ("set" in desc && desc.set) {
      descriptor.set = unwrapHandle(
        this,
        this.newFunction(`set_${prop}`, desc.set),
      );
    }
  ...
  callFunction<T = any>(
    fnHandle: INoOpVm<Fn>,
    thisHandle: INoOpVm<any>,
    ...argHandles: INoOpVm<any>[]
  ): CallResult<T> {
  ...
  callConstructor<T = any>(
    constructorHandle: INoOpVm<(...args: any[]) => T>,
    ...argHandles: INoOpVm<any>[]
  ): CallResult<T> {
    const constructFn = wrapHandle(
      this,
      (Ctor: any, ...args: any[]) => new Ctor(...args),
    );
