...
function R({
  ...
  let G = useCallback(() => {
    if (!e.current || 0 === B.current.size) return;
  ...
  let H = e => {
    if (e.stopPropagation(), (e.metaKey || e.ctrlKey) && "f" === e.key) {
      e.preventDefault();
      return;
    }
  };
  ...
  let Q = jsx("div", {
    ...
    children: jsx(_$$L2, {
      ...
      onKeyDown: e => {
        H(e);
        "Enter" === e.key && (e.metaKey || e.ctrlKey ? K() : W(), e.preventDefault());
      }
    })
  });
  let J = jsx("div", {
...
export function $$M0({
  ...
  useEffect(() => {
    let e = null;
    let t = t => {
      "DEPENDENCY_TYPES_STATUS" === t.type ? (C(t), "finished" === t.status && V.current ? (V.current.dispatch({
        effects: [Yt.of(t)]
      }), e = setTimeout(() => {
        e = null;
