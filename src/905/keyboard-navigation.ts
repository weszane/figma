...
const NavigationContext = createContext<[NavigationState, (state: NavigationState) => void]>([
...
export function KeyboardNavigationProvider({
  ...
  const overrideDown = useCallback((navState: any) => 
    navState.path[0] === NavigationPath.Search && isSearching
      ? {
  ...
  return jsxs(dP, {
    ...
    children: [
      beforeItemsRefs.map((ref, i) => jsx(BeforeItemRef, {
        index: i,
        ref,
        shouldRender,
      }, i)),
...
const BeforeItemRef = forwardRef<HTMLElement, { index: number; shouldRender: boolean }>((props, ref) => {
  const { setKeyboardNavigationElement, keyboardNavigationItem } = M3({
...
function useNavigationEffect(navigationItem: any) {
  const [{ path, column, action }, setNavigationState] = useContext(NavigationContext)

  useEffect(() => {
    if (navigationItem && 
        navigationItem.path.join() === path.join() && 
        navigationItem.column === (column ?? null) && 
        action !== void 0) {
      switch (action) {
        case NavigationAction.Focus:
          navigationItem.focus({ preventScroll: true })
          break
        case NavigationAction.FauxFocus:
          navigationItem.fauxFocus({ preventScroll: true })
          break
...
export function useKeyboardNavigationItemForSearch() {
  ...
  return { setKeyboardNavigationElement }
}
...
export function useKeyboardNavigationItemForParentSubpath(disabled: boolean) {
  ...
  return { setKeyboardNavigationElement, keyboardNavigationItem }
}
