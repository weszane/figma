...
export function getVariableThumbnail(variableSetKey: string, thumbnails: Record<string, any>, variableId: string, variableType: string, modeContext: Record<string, string>): any | null {
  ...
  if (!thumbnail || !thumbnail.variableThumbnails)
    return null
  ...
  while (current?.type === VariableDataType.ALIAS) {
    ...
    if (!defaultMode || !current.values || !current.values[defaultMode])
      return null
...
export const loadVariableSetThumbnails = createOptimistThunk((dispatch, payload) => {
  ...
  fetchVariableSetThumbnails(variableSet).then((result) => {
    const thumbnail = getVariableThumbnail(
      convertKiwiToString(kiwiId),
      result.thumbnails,
      variableId,
      variableType,
      modeContext,
 ...
 * (Original: $$w2)
 */
export async function fetchVariableSetThumbnails(variableSet: { key: string, checkpoint_id: string }) {
  return {
    thumbnails: await teamLibraryCache.getVariableSetThumbnails(
      generateVariableSetThumbnailUrl(variableSet.key, variableSet.checkpoint_id),
    ),
  }
}
...
function getStyleThumbnail(
  ...
  if (!existingThumbnail || existingThumbnail.content_hash !== style.content_hash || !existingThumbnail.url
    || existingThumbnail.url === LOADING_THUMBNAIL || existingThumbnail.url === FAILED_THUMBNAIL) {
    const signal = thumbnailTaskController.acquire(style.node_id, style.content_hash)
    if (!signal)
      return null

    const loadImagesPromise = style.style_type !== 'FILL'
      ? Promise.resolve()
      : runWithTimeout(
          getImageManager().loadAllImagesUnder([style.node_id], ImageExportType.NON_ANIMATED_ONLY, 'style-thumbnail'),
          () => {
    ...
    return loadImagesPromise
      .then(() =>
        scheduler.postTask(() => generateThumbnailFromStyleMaster(style.node_id, style.style_type), signal)
          .catch((err) => {
...
export const replaceThumbnailsOptimist = createOptimistThunk((context, payload) => {
  ...
  for (const { nodeId, url, contentHash, styleThumbnail, type } of payload.thumbnails) {
    let collection
    switch (type) {
      ...
      default:
        if (
          ...
          || type === PrimaryWorkflowEnum.CONSTRAINED_TEMPLATE
          || type === PrimaryWorkflowEnum.CODE_LIBRARY
          || type === PrimaryWorkflowEnum.CODE_FILE
          || type === PrimaryWorkflowEnum.MANAGED_STRING
    ...
    if (payload.styleKind === SubscriptionStatusEnum.LOCAL && !(nodeId in collection))
      continue
    ...
    if (
      thumbnailEntry
      && thumbnailEntry.url === url
      && thumbnailEntry.content_hash === resolvedContentHash
    ) {
      // No update needed
    }
    else {
      if (!updatedThumbnails)
        updatedThumbnails = { ...localThumbnails }
    ...
    if (type === PrimaryWorkflowEnum.STYLE && payload.styleKind === SubscriptionStatusEnum.LOCAL) {
      ...
      if (
        styleEntry
        && (!styleEntry.meta || styleEntry.meta.style_thumbnail !== styleThumbnail)
      ) {
        if (!updatedStyles)
  ...
  if (updatedThumbnails) {
    context.dispatch(replaceLocalThumbnails({ thumbnails: updatedThumbnails }))
  }
...
const thumbnailTaskController = (() => {
  const controllerMap = new Map<string, { controller: any, contentHash: string }>()
  return {
    acquire(nodeId: string, contentHash: string = 'INVALID') {
      ...
      controllerMap.set(nodeId, { controller, contentHash })
      return controller.signal
    },
    release(nodeId: string, contentHash: string = 'INVALID') {
      const entry = controllerMap.get(nodeId)
      if (entry?.contentHash === contentHash)
        controllerMap.delete(nodeId)
    },
  }
})()
