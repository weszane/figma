...
async function x({
  ...
  if (e.type !== "SYMBOL" && !e.isStateGroup) throw new Error(E);
  ...
  if (!backingLibraryKey || !o || !backingNodeId || !s) throw new Error(`Failed to link node ${e.guid} to Code Connect. Missing backing library key, open file key, or backing node ID. Backing Library Key: ${backingLibraryKey}, Open File Key: ${o}, Backing Node ID: ${backingNodeId}`);
  try {
...
async function N(e) {
  ...
  if (!r) throw new Error("Failed to export image to base64 string");
  let [a, s] = r.split(",");
  return {
    content: [{
      type: "image",
  ...
  return {
    content: [{
      ...
      text: e.map(e => function e(t, i, n) {
        ...
        if (t.type === "INSTANCE" || t.type === "SYMBOL" || t.isIconLikeContainer || t.isDevModeAsset) return `${r}<${s} ${o} />`;
        let l = t.childrenGuids.map(t => {
          let r = i.get(t);
          return r ? e(r, i, n + 1) : null;
        }).filter(e => e !== null).join("\n");
        return l.length === 0 ? `${r}<${s} ${o} />` : `${r}<${s} ${o}>
${l}
${r}</${s}>`;
      }(e, t, 0)).join("\n")
    }, {
...
export async function $$z1(e, t, i, d) {
  ...
  if (function (e) {
    if (e.getState().selectedView.view !== "fullscreen") throw new Error("The MCP server is only available if your active tab is a Figma design file");
    if (!getFeatureFlags().dt_my_cool_plugin || !EditorPreferencesApi().enableCodegenMcpServer) throw new Error("The MCP server is not enabled for this user");
  ...
  if (uT(i.getState())) throw new Error("The MCP server has been disabled by admin");
  if (!function (e) {
    let t = e.getState();
  ...
  if (v.toolName === MCP_INTERNAL_GET_RESOURCE) {
    ...
    return function (e) {
      if (!getFeatureFlags().dt_mcp_auto_resources) throw new Error("Resources not supported");
  ...
  try {
    ...
    switch (toolName) {
      ...
      case GET_CODE_CONNECT_MAP:
        {
          if (!(await Kk(d.getState().openFile?.key ?? ""))) throw new Error(V);
      ...
      case GET_VARIABLE_DEFS:
        return [{
          content: [{
            type: "text",
            text: JSON.stringify(collectNodeVariablesAndStyles(l, i, !1))
          }]
        }, {}];
    ...
    if (t.message.includes("Code Connect mapping already exists for this node")) throw new Error("Code Connect mapping already exists for this node. If you want to change the mapping, please delete the existing mapping first.");
