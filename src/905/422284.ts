...
let $$H = createOptimistThunk((e, t, {
  ...
  if (f.openFile) {
    ...
    let W = V === "inspect" || V === "dev"
    H.editorType === FEditorType.Design && W ? fullscreenValue.requestEditorType(WorkspaceType.DEV_HANDOFF) : H.editorType !== FEditorType.DevHandoff || W || V === "auto" || fullscreenValue.requestEditorType(WorkspaceType.DESIGN)
    ...
    if (E.vars === "1") {
      ...
      else if (E["component-browser"] === "1") {
        let i = SelectorType.NONE
        E["gh-settings"] === "repo" ? i = SelectorType.REPO_SELECTOR : E["gh-settings"] === "dirs" ? i = SelectorType.DIRECTORY_SELECTOR : E["gh-repo-selector"] === "1" && (i = SelectorType.REPO_SELECTOR)
        e.dispatch(selectViewAction({
          ...t,
    ...
    E.vars !== "1" && atomStoreManager.set(_$$$, null)
    ...
    let q = (t) => {
      let i = t.guid
      let n = t.styleType !== "NONE"
      let a = t.type === "SYMBOL"
      let s = t.parentNode
      if (n) {
        Fullscreen.selectStyleByGuid(i)
      }
      else if (a && s && s.type === "CANVAS" && !s.visible) {
    ...
    if (Z != null
      ...
      let t = F.slice(1)
      f.selectedView.view === "fullscreen" && f.mirror.appModel.topLevelMode !== ViewType.BRANCHING && t !== "commentPreferences" && f.selectedView?.commentThreadId !== t && e.dispatch(selectViewAction({
        ...f.selectedView,
        commentThreadId: t,
      }))
    }
...
let W = createOptimistThunk((e, t) => {
  ...
  if (selectedView.view === "prototype") {
    let n = selectedView.isPresenterView ? "presenter" : selectedView.file.editor_type === "slides" ? "deck" : "proto"
...
let $$Y0 = createOptimistThunk((e, t) => {
  ...
  let r = Array.from(n.entries()).every(([e, t]) => e === "node-id" ? i.get(e)?.replace("-", ":") === t.replace("-", ":") : i.get(e) === t)
  let a = i.get("m") != null && n.get("m") == null
  let s = i.get("version-id") != null && n.get("version-id") == null;
