    ...
    return jsx('div', {
      children: sortedPageIds.map(pageId => {
        ...
        return jsx(StickySection, {
          className: showPageTitle ? 'component_tiles--stickySectionPage--VcqIa component_tiles--stickySection--aI9lh' : STICKY_SECTION_CLASS,
...
class LibraryItemTilesByFrame extends PureComponent<LibraryItemTilesByFrameProps> {
  ...
  render() {
    ...
    return jsxs('div', {
      ...
      children: [ungroupedItems.length > 0 && jsx(LibraryItemTileGrid, {
        ...this.props,
        ...
        children: frameNodeIds.map(frameNodeId => {
          ...
          return firstItem ? jsx(LibrarySection, {
...
export function LibraryItemTileGrid(props: LibraryItemTileGridProps) {
  ...
  const shouldUsePortal = isLibraryModalContextAvailable();
  ...
  const isFigJamFullscreen = props.selectedView.view === 'fullscreen' && props.selectedView.editorType === FEditorType.Whiteboard;
  const itemElements: any[] = [];
  const itemWidth = props.ui3Compact ? 64 : useRefreshUi ? 56 : calculateItemWidth(props.width, 24);
  ...
  const containerClass = props.ui3Compact ? 'component_tiles--componentTilesCompact--7vcle component_tiles--componentTiles_v2--9-itI' : useRefreshUi ? 'component_tiles--componentTiles_v2--9-itI' : 'component_tiles--componentTiles--axRNc';
  const containerStyle = props.ui3Compact ? {
    marginLeft: `-${gutterWidth}px`
  } : undefined;
  const itemListKey = props.items.map(item => item.type === PrimaryWorkflowEnum.COMPONENT ? item.component_key : item.key).join(',');
  return jsxs('div', {
    className: `${COMPONENT_TILES_MARGIN_CLASS} ${props.className ? props.className : ''}`,
    children: [jsx('div', {
      className: useRefreshUi || props.ui3Compact ? 'component_tiles--componentContainer_v2--cgysm' : 'component_tiles--componentContainer--BZZuQ',
      children: jsx(RenderListByChunks, {
        chunkSize: 50,
        listKey: itemListKey,
        className: containerClass,
        style: containerStyle,
        children: isFigJamFullscreen ? jsx('div', {
