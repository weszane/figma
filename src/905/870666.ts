 ...
 */
export function isFullscreenWhiteboard(viewObj: { view: string, editorType: FEditorType }) {
...
export class SelectedViewPathManager {
  ...
  pathToSelectedView(
    ...
    if (
      ...
      // Merge params
      if (
        query['merge-branch-key']
        && query['merge-source-key']
        ...
        if (query['merge-from-checkpoint-key']) {
          viewObj.mergeParams.fromCheckpointKey
            = query['merge-from-checkpoint-key']
        }
      }
      // Review number
      ...
      if (query['try-plugin-editor-type']) {
        viewObj.tryPluginEditorType = mapViewTypeToMainfestEditorType(
          query['try-plugin-editor-type'],
        )
      }
      ...
      if (query.source) {
        viewObj.trackingInfo = { source: query.source }
      }
      ...
      if (
        ...
        && mirror.appModel.topLevelMode === ViewType.LAYOUT
      ) {
        if (
          (CorePerfInfo?.getTotalUsedHeapMemory() ?? 0)
          > 2 * 0x40000000
          || query.recovery === 'true'
        ) {
      ...
      if (query['prev-selected-view']) {
        if (query['prev-selected-view'] === 'recentsAndSharing') {
          ...
          query['prev-selected-view'] === 'folder'
          && query['prev-folder-id']
        ) {
          viewObj.prevSelectedView = {
            view: 'folder',
      ...
      if (
        query.workshop_id
        && query.workshop_username
        && query.workshop_username.trim().length > 0
      ) {
        viewObj.workshopUserNames = {
          [query.workshop_id]: query.workshop_username,
        }
        Vv(query.workshop_id, query.workshop_username)
      }
      if (query.claim) {
        ...
        debugState.dispatch(
          VisualBellActions.enqueue({
            message: getI18nString(
              'whiteboard.google_classroom.addon.submission_reminder',
            ),
    ...
    if (
      pathSegments[1] === 'test'
      && (pathSegments[2] === 'interactions' || pathSegments[2] === 'eval')
    ) {
  ...
  requireHistoryChange(
    prev: SelectedViewParams,
    next: SelectedViewParams,
  ): boolean {
    return (
      prev.view === 'fullscreen' !== (next.view === 'fullscreen')
      || (prev.view === 'fullscreen'
        && next.view === 'fullscreen'
  ...
  DEPRECATED_getDefaultResources(view: SelectedViewParams) {
    const { fileByKey, repos } = debugState.getState()
   ...
   */
  selectedViewToPath(
    view: SelectedViewParams,
    context: { resources?: any, sharingAttributionContextKey?: string },
  ): string | null {
    if (view.view === 'fullscreen') {
      let path: string
      if (view.fileKey === INTERACTION_FAKE_FILE_KEY) {
        return isEvalViewPathCheck()
      ...
      if (
        view.nodeId
        && isValidSessionLocalID(parseSessionLocalID(view.nodeId))
      ) {
        queryParams['node-id'] = replaceColonWithDash(view.nodeId)
      }
      if (
        view.codeNodeId
        && isValidSessionLocalID(parseSessionLocalID(view.codeNodeId))
      ) {
        queryParams['code-node-id'] = replaceColonWithDash(view.codeNodeId)
      }
      if (
        view.editorType === FEditorType.Sites
        && view.sitesView
      ) {
      ...
      const attributionKey = combineWithHyphen(
        context.sharingAttributionContextKey,
        ShareContext.BROWSER_ADDRESS_BAR,
      )
      if (attributionKey) {
      ...
      if (
        window.EARLY_ARGS?.multiplayer_preconnect_options
          ?.forceIncrementalForEditors !== undefined
      ) {
   ...
   */
  selectedViewName(view: SelectedViewParams, context: { resources?: any }): string {
    if (view.view === 'fullscreen') {
  ...
  selectedViewHasMissingResources(
    state: { fileByKey: Record<string, any> },
    view: SelectedViewParams,
  ): boolean {
