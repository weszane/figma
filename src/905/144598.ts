...
export function setupRoleRow(props: {
  ...
  dropdownShown?: { type: string, data: { dropdownId: string } }
  ...
  if (canUpgradeSeat && roleOptions) {
    roleOptions = roleOptions.map(option =>
      option.type === 'option' && option.key === props.level
        ? { ...option, selectedLabelClass: mL }
        : option,
    )
  }
  // Dropdown open state
  const isDropdownOpen
    = !!(
      props.dropdownShown
      && props.dropdownShown.type === 'level-change-dropdown'
      && props.dropdownShown.data.dropdownId === props.id
    )
    ...
    'children': [
      ...
      jsxs('div', {
        ...
        children: [
          ...
          inviteWithSeatExperiment && isAdminUser
            ? jsxs(e6, {
                'trackingProperties': {
          ...
          roleOptions
            ...
            : props.isPrototypeRole && props.level === AccessLevelEnum.VIEW_PROTOTYPES
              ? jsx('span', {
                  className: _$$C,
...
export function UserTeamAvatar({
  ...
  user: { email: string, img_url: string }
  plan?: { parentId?: string, name?: string, imgUrl?: string }
  id: string
}) {
  return jsxs('div', {
    className: e0,
 ...
 * Original: $$C1
 */
export function getUserHandleOrEmail(e: { pending: boolean, user: { handle: string, email: string } }, t: { usersByEmail: { [email: string]: { handle: string } } }) {
  return !e.pending
    ? e.user.handle
    : e.user.email in t
      ? t.usersByEmail[e.user.email].handle
      : e.user.email
}
...
export function generateRoleOptions(isOwner: boolean, isLeaving: boolean, isAdmin: boolean, isEditor: boolean, currentLevel: AccessLevelEnum, isPending: boolean, isPrototypeRole: boolean) {
  ...
  if (isPrototypeRole && currentLevel === AccessLevelEnum.VIEW_PROTOTYPES) {
    ...
    if (isOwner && !isPending)
      options.push(createRoleOption(AccessLevelEnum.OWNER))
    if (isEditor)
      options.push(createRoleOption(AccessLevelEnum.ADMIN))
    options.push(createRoleOption(AccessLevelEnum.EDITOR))
    options.push(createRoleOption(AccessLevelEnum.VIEWER))
    isResend = isPending
  }

  const removeText = isLeaving && !isOwner
    ? getI18nString('confirm_remove_role.leave')
    : getI18nString('confirm_remove_role.remove')
