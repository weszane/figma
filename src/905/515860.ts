 ...
 * (original variable: o)
 */
const teamRelatedViews = ['recentsAndSharing', 'draftsToMove', 'communityHub', 'search', 'user'];
...
export function resolveTeamId(state: {
  ...
  try {
    ...
    if (view === 'abandonedDraftFiles' && adminPlanType === OrganizationType.TEAM || view === 'seatRequests' && adminPlanType === OrganizationType.TEAM) {
      return planId || '';
    }
    ...
    const otherViews = [...teamRelatedViews, 'deletedFiles', 'limitedTeamSharedProjects', 'trashedFolders', 'desktopNewTab', 'resourceHub'];
    if (otherViews.includes(view!) && !state.currentUserOrgId) {
      return state.currentTeamId || '';
    }
  } catch (err: any) {
    // Error handling (original: reportError)
    reportError(ServiceCategories.GROWTH_PLATFORM, new Error(`Error getting team ID from Redux state: ${err.message}`));
  }
  return '';
}
