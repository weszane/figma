...
import { getCurrentUserOrgId, handleSelectedView, isDevModeFocusViewCopyActive } from '../905/622391';
import { logger } from '../905/651849';
import { loadCanvasWithEulaCheck, PluginAction } from '../905/656545';
...
import { loadStyleByKey, loadComponentV2ByKey, loadStateGroupByKey } from '../figma_app/933328';
      ...
      children: [jsx('div', {
        children: jsx(TextWithTruncation, {
          ...
          children: renderI18nText('textreview.as_you_type_plugin_name_will_review_your_text_and_provide_suggestions', {
            pluginName: jsx(TextWithTruncation, {
              fontWeight: 'semi-bold',
              children: i
            })
          })
...
export function extractWidgetEventHandlers(vNode, widgetNode, command, runtime) {
  ...
  while (childNodes.length && currentNode.type === 'FRAME') {
    ...
    if (currentVNode && currentVNode.renderMetaData.children?.length) {
      ...
      currentVNode = processChildren(currentVNode.renderMetaData.children)[childIndex];
...
Move figma.showUI outside the callback and use figma.ui.postMessage within the callback instead to ensure that your plugin handles concurrent "generate" events correctly.`);
  ...
  getRunMode() {
    return atomStoreManager.get<string>(runModeAtom) ?? 'default';
  }
  getWidgetContext(e) {
  ...
  windowToCanvasPosition(e) {
    ...
    let n = new Point(e.x, e.y).subtract(i);
    return applyOffsetToViewport(t, n);
  }
  ...
  async fireCloseEventAsync() {
    ...
    try {
      await runTimeoutEventCloseCallback(5000); // 5 second timeout
    } finally {
  ...
  getCurrentUserInfo() {
    ...
    userId = handleSelectedView() ?? null;
    return {
      name,
      photoUrl,
      userId
  ...
  createUserApi() {
    ...
    const userId = handleSelectedView();
    const multiplayerState = debugState.getState().multiplayer;
  ...
  createVsCodeApi() {
    let e = this.vm;
    return isVsCodeEnvironment() ? e.newObject() : e.undefined;
  }
  ...
  handleStorageError(reject, vm, operation, key, error) {
    ...
    reportError(ServiceCategories.EXTENSIBILITY, error instanceof Error ? error : new Error(`figma.clientStorageAsync.${operation}Async failed: ${error}.`));
  }
  ...
  parseCheckoutOptions(optionsHandle, vm) {
    const defaultOptions = {
      interstitial: ModalType.PAID_FEATURE
    };
    ...
    const userOptions = validateWithZSchema({
      ...
      handle: optionsHandle,
      zSchema: _$$z.object({
        interstitial: _$$z.nativeEnum(ModalType)
      }).partial(),
  ...
  validateCheckoutRequirements(publishedExtension, localPlugin, isLocal) {
    if (publishedExtension) {
      ...
      if (!hasMonetizedResourceMetadata(publishedExtension) && !localPlugin) {
  ...
  generatePaymentToken(apiType, pluginID) {
    // TODO: Phase 18 Refactoring - Replace sendWithRetry with Advanced HTTP Client:
    ...
    return sendWithRetry.put(`/api/${apiType}/${pluginID}/id_token`).then(({
      data
  ...
  async handlePluginDataUpdate(widgetNode) {
    ...
    if (this.options.isLocal && this.options.code && !hasKey(this.options.pluginID)) {
      const existingData = widgetNode.getPluginData(this.options.pluginID, localWidgetCodeMD5Key);
  ...
  async handleMountOrRerenderEvent(widgetEvent, widgetManager, pluginDataHash) {
    ...
    if (pluginDataHash) {
      ...
      permissionScopeHandler.plugin('widget-code-change-setPluginData', () => {
        widgetNode.setPluginData(this.options.pluginID, localWidgetCodeMD5Key, pluginDataHash);
      });
    }
    ...
    this.defineVmFunction({
      ...
      cb: idHandle => {
        ...
        vm.registerPromise((async () => {
          const componentModule = await _require; // Fix Promise chain
          const id = vm.toString(idHandle);
  ...
  createCortexApi() {
    ...
    n('completeChat', 'cortex.internal.openai.completeChat', cortexAPI.openai.completeChat);
    r('streamChat', 'cortex.internal.openai.streamChat', cortexAPI.openai.streamChat);
    n('computeEmbeddings', 'cortex.internal.openai.embeddings', cortexAPI.openai.computeEmbeddings);
    n('getTextImageEmbeds', 'cortex.internal.clip.embeddings', cortexAPI.design.getTextImageEmbeds);
    r('streamText', 'cortex.internal.streamText', cortexAPI.internal.streamText);
    n('generateImages', 'cortex.internal.design.generateImages', cortexAPI.design.generateImages);
    n('jamGPT', 'cortex.internal.figjam.jamGPT', cortexAPI.figjam.jamGPT);
    r('createTemplate', 'cortex.internal.figjam.createTemplate', cortexAPI.figjam.createTemplate);
    n('createVisual', 'cortex.internal.figjam.createVisual', cortexAPI.figjam.createVisual);
    e.shallowFreezeObject(t);
    return t;
  }
  ...
  createAPI() {
          ...
          get: memoizedHandle(variableApiConfig.vm, () => {
            ...
            variableApiConfig.defineVmFunction({
              ...
              cb: t => {
                ...
                if (!VariableIdHandler.fromString(id)) throw new Error('Invalid variable id');
                const {
                  promise,
                  resolve,
                  reject
    ...
    this.defineVmFunction({
      ...
      cb: t => {
        ...
        atomStoreManager.set(isSetCurrentPageAsyncAtom, !0);
        e.registerPromise(ensurePluginPageLoaded(i.guid, this.documentAccessState)).then(() => {
          ...
          atomStoreManager.set(isSetCurrentPageAsyncAtom, !1);
        });
        return promise;
      },
      isAllowedInReadOnly: !0,
    ...
    for (const {
      ...
      const isAllowed = C && combinedNodes1.includes(nodeType) || I && combinedNodes2.includes(nodeType) || R && combinedNodes3.includes(nodeType) || M && combinedNodes4.includes(nodeType) || D && combinedNodes5.includes(nodeType);
    ...
    this.defineVmFunction({
      ...
      cb: t => {
        ...
        let s = fetchImageAsByteArray(i, this.options.allowedDomains);
    ...
    this.defineVmFunction({
      ...
      cb: () => {
        ...
        e.registerPromise(loadAllPagesWithUI(null, null, PluginModalType.DYNAMIC_PLUGIN_LOAD_ALL)).then(t => resolve(e.deepWrap(t)));
    ...
    this.defineVmIncrementalFunction({
      ...
      resolveValue: t => function ({
        ...
        if (!timeRegex.test(e)) return t.$$null;
    ...
    if (this.options.enableNativeJsx) {
      this.defineVmFunction({
        ...
        cb: (t, i, n) => {
          ...
          let d = loadAndReconcileResources(() => ({
            rootNode: a,
            syncedState: {}
          }), runtimeBridge, this.options.allowedDomains);
          let c = loadAndReconcileResources(() => ({
            rootNode: s,
            syncedState: {}
          }), runtimeBridge, this.options.allowedDomains);
          let {
    ...
    this.defineVmFunction({
      ...
      cb: t => {
        ...
        shutdownCallback && this.options.addShutdownAction(shutdownCallback);
        let s = loadAndReconcileResources(() => ({
          rootNode: n,
          syncedState: {}
...
async function ag(nodeMap, urlText) {
  ...
  const urlInfo = parseEmbedInput(urlText);
  if (!urlInfo || urlInfo.isFromIFrame) {
    throw new Error('The provided text was not a URL');
  }
  const linkPreviewResult = await finalizeLinkPreviewHandler(urlText, urlInfo.url, urlInfo.isFromIFrame, PluginModalTypeEnum.PLUGIN, debugState.getState());
...
async function a_(_componentKey, styleKey, permissions) {
  ...
  const publishedStyle = findStyleDataByKey(styleKey, currentState.library.used__LIVEGRAPH.styles, currentState.library.openFilePublished__LIVEGRAPH.styles) || (await loadStyleByKey(debugStateInstance, styleKey));
  if (!publishedStyle) {
    throw new Error(`Could not find a published style with the key "${styleKey}"`);
  }

  // Check if style is already available locally
  if (compareWithGeneratedKey(publishedStyle, permissions)) {
    return {
      key: createInvalidString(publishedStyle.key),
      version: createInvalidString(publishedStyle.content_hash) ?? 'INVALID'
    };
  }

  // Download the style from library
  try {
    downloadedStyle = await loadCanvasWithEulaCheck(debugStateInstance.dispatch, publishedStyle, publishedStyle.library_key, PluginAction.PLUGIN_INSERT_STYLE, currentState.userFlags, currentState.fonts);
  } catch {
  ...
  return {
    key: createInvalidString(upsertResult.key),
    version: createInvalidString(upsertResult.version)
  };
}
async function aA(componentData, permissions) {
  ...
  if (!compareWithGeneratedKey(componentData, permissions)) {
    let downloadedComponent;

    // Download the component from library
    try {
      downloadedComponent = await loadCanvasWithEulaCheck(debugStateInstance.dispatch, componentData, componentData.library_key, PluginAction.PLUGIN_INSERT_COMPONENT, currentState.userFlags, currentState.fonts);
    } catch {
...
async function ay(stateGroupData, permissions) {
  ...
  if (!compareWithGeneratedKey(stateGroupData, permissions)) {
    let downloadedStateGroup;

    // Download the state group from library
    try {
      downloadedStateGroup = await loadCanvasWithEulaCheck(debugStateInstance.dispatch, stateGroupData, stateGroupData.library_key, PluginAction.PLUGIN_INSERT_STATE_GROUP, currentState.userFlags, currentState.fonts);
    } catch {
...
export async function $$ab4(componentKey, sceneGraphInstance = getSceneGraphInstance()) {
  const debugStateInstance = debugState;
  const currentState = debugStateInstance.getState();
  const currentFile = getSelectedFile(currentState);
  if (!currentFile) {
  ...
  if (!finalComponent) {
    const fetchResult = await loadComponentV2ByKey(debugStateInstance, resolvedComponentKey, 'importComponentByKeyAsync');
...
export async function $$av5(componentSetKey) {
  ...
  if (!foundStateGroup) {
    foundStateGroup = await loadStateGroupByKey(debugState, resolvedComponentSetKey);
    if (!foundStateGroup) {
