...
export function openAutosaveDatabase(): Promise<IDBPDatabase<unknown>> {
  ...
  if (DATABASE_VERSION > 100) {
    return databasePromise = Promise.reject(
      new Error('Trying to open autosave with unsupported DB version'),
    )
  }
...
export function getAutosaveDatabaseWithErrorHandling(): Promise<IDBPDatabase<unknown> | null> {
  return getAutosaveDatabase().catch((error) => {
    ...
    const isIgnoredError
      = /A mutation operation was attempted on a database that did not allow mutations/.test(message)
        || /QuotaExceededError/.test(message)
        || /Encountered full disk while opening backing store for indexedDB.open/.test(message)
        || /Failed to openDatabase in database because not enough space for domain/.test(message)
        || /Timeout when opening IDB/.test(message)
        || /Internal error opening backing store for indexedDB.open/.test(message)
        || /DataError/.test(message)
        || /Internal error retrieving bucket data directory/.test(message)
        || (name === 'UnknownError' && message === 'Internal error.')
 ...
 */
export async function executeDatabaseTransaction<T = any>(
  storeNames: string | string[],
  callback: (transaction: IDBPTransaction<unknown, string[], IDBTransactionMode>) => Promise<T>,
