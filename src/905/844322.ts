...
let $$E2 = createOptimistThunk((e, t) => {
  ...
  let a = sendWithRetry.post('/api/files_batch/restore', {
    files: r.map(e => ({
      key: e
    })),
  ...
  a.then(t => {
    if (t.status === 207) {
      ...
      try {
        ...
        let r = getTrashedFolderFilesById(n, e.getState());
        e.dispatch($$x1({
          fileKeys: r,
          userInitiated: !1
        }));
      ...
      for (let e of r) {
        if (o === null) {
          o = i.fileByKey[e].folder_id;
        } else if (o !== i.fileByKey[e].folder_id) {
          p = !1;
          break;
        }
      }
      ...
      m.commit();
      getCurrentLiveGraphClient()?.optimisticallyUpdate(h, a);
    }
  }).catch(({
    response: t
  }) => {
...
let $$x1 = createOptimistThunk((e, t) => {
  ...
  e.dispatch(restoreRecentPrototype({
...
let S = liveStoreInstance.Mutation((e, {
  ...
  let d = sendWithRetry.put('/api/files_batch', {
    files: files.map(e => ({
      key: e.key,
      folder_id: folderId,
