...
function tO({
  ...
  return jsxs(AutoLayout, {
    ...
    children: [jsx(tP, {
      ...
      dispatch: c,
      onChange: (t) => {
        t === PublisherRole.OWNER
          ? l?.monetized_resource_metadata
            ? c(showModalHandler({
                type: _$$K2,
                data: {},
              }))
...
function tU({
  ...
  let p = null
  p = o.id && o.plugin_publishers && getPluginVersion(o).id
    ? jsx(tM, {
        allowedTabs: d,
        closePluginPublishModal: c,
        contacts: e,
...
function tY({
  ...
  let n = e.roles
  let a = e.is_widget
  if (isResourceApprovedPublic(e) || t === PublisherType.ORG)
    return null
...
function tX(e) {
  return jsx('div', {
    className: ey()(v_, E3),
    role: 'tablist',
    children: tZ.map(t => e.allowedTabs.includes(t.tab) && t.isVisible()
      ? jsx(fJ, {
...
function iE({
  ...
  if (qu(t) && !e) {
    ...
    let o = UR(t, e)
    s = a && e
      ? e.status === FRequestStatusType.APPROVED
        ? jsx(ix, {
            children: jsx(TextWithTruncation, {
...
class iW extends Component {
  constructor(e) {
    ...
    this.doPublish = () => {
      ...
      this.authorWillChange() && r && !n.some(e => f7(e, r))
        ? this.props.dispatch(showModalHandler({
            type: to,
            data: {
    ...
    this.getOrgIdFromPublisherOrRole = () => {
      ...
      if ('org_id' in author)
        return author.org_id
      if ('team_id' in author)
        return this.props.teams[author.team_id]?.org_id || void 0
      {
    ...
    this.isAuthorSame = (e) => {
      if (isAcceptedPublisher(this.props.publishedPlugin, this.props.user.id))
        return !0
    ...
    this.renderPricingSection = (e, t) => {
      if (!this.canUserSellPluginOnCmty())
        return null
      let i = this.props.publishedPlugin
      let n = !!e.hasPaymentsApi
      if (i.current_plugin_version_id && !i.monetized_resource_metadata && !n && i.third_party_m10n_status !== ProductStatus.FLAGGED)
        return null
    ...
    this.existingPluginPublishersPermissions = async () => {
      ...
      if (!this.authorWillChange() || !('team_id' in e || 'org_id' in e) || t && !t.is_public)
        return
      let i = this.isWidget()
        ? {
            ...e,
    ...
    this.getPublishFormPrimaryButtonText = () => {
      ...
      if (!this.state.shouldIncrementVersion)
        return getI18nString('community.publishing.save_changes')
      switch (e) {
        case 'none':
  ...
  getTimeSinceComponentDidMount() {
    if (void 0 !== this.componentDidMountTime)
      return Date.now() - this.componentDidMountTime
  }
  ...
  componentDidUpdate(e) {
    ...
    }), (e.publishingState.metadata.isPaid || !this.props.publishingState.metadata.isPaid) && ((e.publishingState.metadata !== this.props.publishingState.metadata || e.validPluginId !== this.props.validPluginId) && (this.state.errors.publishers
      ? this.setState(e => ({
          errors: {
            ...this.getFormErrors(),
    ...
    return jsxs(Fragment, {
      children: [jsxs(_$$A14, {
        ...
        children: [jsx(iM, {
          ...
          children: [this.hasFormErrors(errors)
            ? jsx(iU, {})
            : jsx(e6, {
  ...
  getReviewStatus() {
    switch (this.state.roleToPublishAs) {
      ...
      case PublisherType.PUBLIC:
        if (isResourceApprovedPublic(this.props.publishedPlugin))
          return 'approved'
        if (isResourcePendingPublishing(this.props.publishedPlugin))
          return 'pending'
        return 'required'
    }
  }
...
let $$iq0 = registerModal((e) => {
  ...
  return i !== 'loading' && a !== 'loading' && l.loaded
    ? jsx(iY, {
        ...e,
        Sprig,
...
(n || (n = {})).PublishModalPostFormFlow = function (e) {
  ...
  return step === _$$D2.Step.CHOOSE_PROFILE_CREATION_ROUTE_WITH_ACCOUNTS
    ? jsx(nn, {
        isLoading: loading,
        secondaryButton: {
    ...
    : step === _$$D2.Step.CHOOSE_PROFILE_CREATION_ROUTE_NO_ACCOUNTS
      ? jsx(nn, {
          isLoading: loading,
          primaryButton: {
            text: getI18nString('community.publishing.create_new_profile'),
      ...
      : step === _$$D2.Step.CONNECT_PROFILES
        ? jsx(_$$$, {
            onSubmit: (r) => {
              if (!r || !e.publisher) {
        ...
        : step !== _$$D2.Step.SET_PROFILE_HANDLE
          ? jsx(XT, {
              title: e.title,
              body: e.description,
              footerLeftSide: e.role.org
                ? null
                : renderI18nText('community.publishing.review_our_community_guidelines', {
