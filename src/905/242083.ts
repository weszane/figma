...
class WithFullscreenFunctionality extends WithEXIFAndWindowBindings {
  ...
  public isReady(): boolean {
    return this._fullscreenIsReady && fullscreenCrashHandler.getFullscreenCrashState() === 'ok' && !!Fullscreen;
  }
  ...
  public reparentRootElement(containerElement: HTMLElement | null): void {
    ...
    if (document.activeElement instanceof HTMLElement && fullscreenRoot.contains(document.activeElement)) {
  ...
  public isRootElementVisible(): boolean {
    const fullscreenRoot = document.getElementById('fullscreen-root');
    return this.containerElement != null && fullscreenRoot?.parentElement === this.containerElement;
  }
   ...
   */
  public updateViewportInfo(x: number, y: number, width: number, height: number, offsetX: number, offsetY: number, zoomScale: number, isZooming: boolean, isPanning: boolean): void {
    this._viewportInfo = {
      x,
      y,
      width,
...
class FullscreenUserInfoHandler {
  ...
  public getUserId(): string | null {
    ...
    const currentUser = this._state.multiplayer.allUsers.find((user: any) => this._state.multiplayer.sessionID === user.sessionID);
...
let lX = class e extends WithFullscreenFunctionality {
          ...
          acceptCallback: () => {
            this.dispatch(showModalHandler({
              ...
              data: {
                ...
                entrypoint: PublishingUIContext.FULLSCREEN_MOVE_COMPONENTS_PROMPT
              }
            }));
          },
          movableItemIds: n
  ...
  toggleTeamLibraryModal() {
    ...
    if (e.view === 'fullscreen' && e.editorType === FEditorType.Whiteboard) {
      this._store.getState().modalShown?.type === FIGJAM_LIBRARY_MODAL ? this.dispatch(hideModal()) : this.dispatch(showModalHandler({
  ...
  forcePublishStateGroup(e) {
    let t = this._state.openFile;
    this._state.user != null && t != null && t.canEdit && hasTeamPaidAccess(t.teamId ? this._state.teams[t.teamId] : void 0) && this.dispatch(executePublishProcess({
    ...
    let t = () => {
      this.dispatch(showModalHandler({
        ...
        data: {
          ...
          entrypoint: PublishingUIContext.FULLSCREEN_OTHER
        }
      }));
    };
