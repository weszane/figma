import { props as stylex } from '@stylexjs/stylex'
...
import { reportError, reportValueError, setTagGlobal } from '../905/11'
...
import { aalSettingsConfig, resetAAL } from '../905/55273'
...
import { showCompareChangesNotification } from '../905/95280'
import { ModalSupportsBackground, registerModal } from '../905/102752'
import { setupPlaybackHandler } from '../905/104019'
import { useNavigateToViewport } from '../905/104740'
import { RemoveBackgroundAction, UpscaleImageAction } from '../905/112768'
import { closeUniversalInsertModal, handlePinningState, setUniversalInsertModalOpen } from '../905/116101'
import { executeInFakeStack } from '../905/117966'
import { fullscreenPerfManager } from '../905/125218'
import { brushStyleAtom, connectorColorAtom, highlighterStyleAtom, shapeColorAtom, shapeSidebarModeAtom, shapeStrokeStyleAtom, sharedStateAtom, someStateAtom, stickyColorAtom, strokeColorAtom, toolStylesAtom, vectorPencilStyleAtom, vectorPenStyleAtom } from '../905/125333'
...
import { AIActionResult, AIActionStatus } from '../905/278499'
import { Z as ICON1 } from '../905/279476'
...
import { FIGJAM_LIBRARY_MODAL, registeredModal } from '../905/300621'
...
import { getWebGPUInitializationStatus, initializeWebGPUContext, initializeWebGPUDevice, logWebGPUInitializationStatus, runWebGPUCompatibilityTests } from '../905/889931'
...
import { genericAtomFamily1, sitesViewSetterAtomFamily } from '../figma_app/115923'
...
import { getCurrentKeyboardLayout, updateKeyboardLayoutPreference } from '../figma_app/475303'
...
import { PROCESS_IDENTIFIER } from '../figma_app/728005'
...
function d(e) {
  ...
  if (!documentStateTsApi || !NodeTsApi)
    throw new Error('Bindings are not initialized')
  let n = documentStateTsApi.getActiveDocument()
  if (NodeTsApi.setGlobalUnstableNodeByID(n, t, i, StateSourceType.REDUX), !NodeTsApi.exists(StateSourceType.REDUX))
...
getInitialOptions().cluster_name !== 'prod' && (window.runGettersBenchmark = function () {
  ...
  if (!e)
    throw new Error('BindingsPerfBench is not initialized')
...
async function ts(e, t) {
  if (!e) {
    ...
    if (!t)
      throw new Error('Must provide a local file key')
    e = t
  }
  if (!t) {
  ...
  if (!i)
    throw new Error('Could not open autosave DB')
...
function ik(e) {
  ...
  let o = useCallback((i) => {
    ...
    i.type === PrimaryWorkflowEnum.COMPONENT
      ? t(insertSharedComponent({
        item: i,
        ...n,
      }))
...
let nf = scopeAwareFunction.testSetup('sticky-typing-scenario', (e, t, i = {}) => {
...
scopeAwareFunction.testSetup('table-typing-scenario', (e, t, i = {}) => {
  let n = i.sessionID ?? Math.floor(2e3 * e()) + 200
  let [r, a] = nA(e, n, window.INITIAL_OPTIONS.user_data.id)
...
function nR() {
  return kiwiParserCodec.decodeNodeChange(interactionTestHelpers.dumpSelectedNode())
}
function nN() {
  return kiwiParserCodec.decodeMessage(interactionTestHelpers.dumpSelectedNode()).blobs
}
function nP() {
...
let nz = {
  ...
  'reactions': function (e, t, i = {}) {
    ...
    for (let t of n.repeatMessages) {
      let i = buildUploadUrl(`${nI(e, reactionWheelItems).image}`)
      r.push([...t, {
...
let nH = (e, t, i = 20, n = 'mouse movement', r = 0, a = 0) => {
  useEffect(() => {
    let s = createPseudoRandomGenerator(0xFA57FA57)
    let o = []
    let l = nz[n]
    for (let t = 0; t < e; t++) {
      o.push(l(s, i, {
    ...
    return (function (e, t) {
      ...
      if (!e.length)
        return null
      if (e.some(t => t.repeatMessages.length !== e[0].repeatMessages.length))
        throw new Error(`All scenarios must have the same length of repeat messages to be compiled together ${e.map(e => e.repeatMessages.length).join(', ')}`)
      let t = (e) => {
      ...
      let n = e => deflateRaw(t(e.map(e => kiwiParserCodec.encodeMessage(e))))
      let r = {
...
function nK() {
  nU ? (unmountComponentAtNode(nU), document.body.removeChild(nU), nU = null) : (nU = document.createElement('div'), document.body.appendChild(nU), createRoot(nU).render(jsx(nW, {})))
}
let nZ = null
let nX = null
class nQ extends KD {
...
let r0 = (e, t) => {
  if (e === ColorProfileEnum.DISPLAY_P3) {
    if (t === 'convert')
      return getI18nString('fullscreen.color_management.document_modal.confirm.title.convert_display_p3')
    if (t === 'assign')
      return getI18nString('fullscreen.color_management.document_modal.confirm.title.assign_display_p3')
  }
  else if (e === ColorProfileEnum.SRGB) {
    if (t === 'convert')
      return getI18nString('fullscreen.color_management.document_modal.confirm.title.convert_srgb')
    if (t === 'assign')
      return getI18nString('fullscreen.color_management.document_modal.confirm.title.assign_srgb')
  }
...
function r6(e, t, i, n, r) {
  switch (r && e(t === ColorProfileEnum.LEGACY
        ...
        children: [jsx(RadioInputOption, {
          ...
          children: jsxs(Fragment, {
            children: [r3(e), e === ColorProfileEnum.SRGB && o === 'convert' && jsxs('div', {
              ...
              children: [jsx(ICON1, {}), renderI18nText('fullscreen.color_management.document_modal.warning.clamp.convert'), jsx(TrackedLink, {
                newTab: !0,
                trusted: !0,
                href: _$$s8,
      ...
      if (l.children?.length) {
        for (let c of l.children) {
          if (zN(c) && PC(c)) {
            try {
              c.type === 'GROUP'
                ? Vy.withMetaScope(`GROUP ${c.id}`, r, () => {
                  d.push(c)
                })
                : c.type === 'FRAME'
                  ? await Vy.withNodeScopeAsync(`FRAME ${c.id}`, r, async () => {
                    if (un(c) || _$$tS(c)) {
                      a[c.id] = c
                    }
                    else if (Oc(c)) {
                  ...
                  : c.type === 'TEXT'
                    ...
                    : _$$a2.includes(c.type)
                      ? await Vy.withNodeScopeAsync(`SHAPE ${c.id}`, r, async () => {
                        if (un(c) || _$$tS(c)) {
                          a[c.id] = c
                        }
                        else if (Oc(c)) {
                      ...
                      : c.type === 'SHAPE_WITH_TEXT'
                        ? Vy.withNodeScope(`SHAPE_WITH_TEXT ${c.id}`, r, () => {
                        ...
                        : c.type === 'TABLE'
                          ...
                          : c.type === 'INTERACTIVE_SLIDE_ELEMENT' && c.flappType === 'YOUTUBE'
                            ? await Vy.withNodeScopeAsync(`INTERACTIVE_SLIDE_ELEMENT ${c.id}`, r, async () => {
...
async function aE(e, t, i) {
  ...
  let [d, c] = await Promise.all([l.size
    ? batchDownloadImages([...l], {
      type: 'figFile',
      fileKey: t.key,
    })
    : Promise.resolve({
  ...
  let g = await Promise.all(n.flatMap(e => e.filter(e => !h || h.has(e)).map((t) => {
    ...
    return i && n
      ? (m[t] = n, Vy.withSlideScopeAsync(e.indexOf(t) + 1, e => ab(o, n, u, p, e)))
      : (console.error('pptx-export', `Could not export slide id ${t}`), Promise.resolve({
        slideId: t,
        slide: null,
        unsupportedNodes: {},
        topLevelActions: [],
  ...
  f.length && (await Promise.allSettled(f.flatMap((e) => {
    ...
    return Object.values(unsupportedNodes).map(e => Vy.withNodeScopeAsync(e.id, null, async (i) => {
      let r = m[slideId]
      if (!r)
        return Promise.reject(new Error(`Slide ${slideId} not found for unsupported node ${e.id}`))
      {
      ...
      children: [jsx(DialogHeader, {
        ...
        children: [jsxs('div', {
          ...
          children: [t && !n && jsxs(Fragment, {
            ...
            children: [jsx(ICON, {}), renderI18nText('fullscreen.export.unable_to_export')],
          })],
...
class aZ extends PureComponent {
  constructor(e) {
      ...
      let i = this.sortedGUIDs.map((i, n) => {
        ...
        try {
          ...
          if (e instanceof SyntaxError)
            r = this.state.matchPattern; else throw e
        }
    ...
    this.getPreviewNames = () => {
      ...
      let i = (t.length <= 5 ? t : [...t.slice(0, 4), t[t.length - 1]]).map(t => jsx(CenterTruncatedText, {
        className: aV,
        text: e[t],
        maxWidth: aH,
      }, t))
    ...
    this.renderLink = () => this.props.nodeType !== 'layer'
      ? null
      : jsx('div', {
  ...
  render() {
    if (this.props.guids.length === 0)
      return null
    let e = this.props.nodeType === 'page'
      ? getI18nString('fullscreen.rename_modal.rename_num_pages_pages', {
...
let aX = registerModal((e) => {
  ...
  let r = useSelector(e => (reportValueError(ServiceCategories.UNOWNED, e.mirror.appModel), e.mirror.appModel.currentPage))
...
class sT {
  constructor(e = 0x4000000) {
    ...
    this._highBytes = 0
    this._numReports = 0
    this._growth = new ProgressiveNotificationHandler(e, 2, 1.2, 2e3, () => {
      this.logStats()
    })
  }
...
class BaseFullscreenMixin { }

/**
 * Mixin to add EXIF and HTML window bindings to a class
 * @param superClass - The class to extend
 * @returns Extended class with EXIF and HTML window bindings
 */

class WithEXIFAndWindowBindings extends superClass {
  /** Image orientation utilities */
  public readonly EXIF = ImageOrientationUtils

  /** Window manager instance */
  public readonly HTMLWindowBindings = windowManagerInstance

  /** JavaScript value memory helper */
  public readonly jsValueHelp = sT.instance()
};

/**
 * Mixin to add fullscreen functionality to a class
 * @param superClass - The class to extend
 * @returns Extended class with fullscreen functionality
 */

class WithFullscreenFunctionality extends WithEXIFAndWindowBindings {
  /** Flag indicating if fullscreen is ready */
  protected _fullscreenIsReady: boolean = false

  /** Promise resolver for when fullscreen is ready */
  protected _readyPromiseResolve: (() => void) | null = null

  /** Flag indicating if font list is loaded */
  protected _fontListLoaded: boolean = false

  /** Event emitter for fullscreen events */
  public readonly fromFullscreen = new EventEmitter('fromFullscreen')

  /** Event emitter for viewport events */
  public readonly viewport = new EventEmitter('viewport')

  /** Container element for fullscreen */
  public containerElement: HTMLElement | null = null

  /** Current viewport information */
  protected _viewportInfo = {
    x: 0,
    y: 0,
    width: 0,
    height: 0,
  ...
  /** Time when file was loaded */
  protected _fileLoadTime: number | null = null

  /** Promise that resolves when fullscreen is ready */
  ...
  /**
   * Set up event listeners for fullscreen functionality
   */
   ...
   */
  public readonly forwardKeyboardEvent = (event: KeyboardEvent): void => {
    forwardKeyboardEvent(event)
  }

  /**
   * Get the ready promise
   * @returns Promise that resolves when fullscreen is ready
   */
  ...
  /**
   * Check if fullscreen is ready
   * @returns True if fullscreen is ready, false otherwise
   */
  public isReady(): boolean {
    return (
      this._fullscreenIsReady
      && fullscreenCrashHandler.getFullscreenCrashState() === 'ok'
      && !!Fullscreen
    )
  }

  /**
   * Check if font list is loaded
   * @returns True if font list is loaded, false otherwise
   */
  public isFontListLoaded(): boolean {
    return this._fontListLoaded
  }

  /**
   * Reparent the root element to the container
   * @param containerElement - The container element
   */
  public reparentRootElement(containerElement: HTMLElement | null): void {
    ...
    if (!this.containerElement) {
      fullscreenRoot.style.visibility = 'hidden'
    ...
    if (
      ...
      && fullscreenRoot.contains(document.activeElement)
    ) {
      activeElement = document.activeElement
    }

    fullscreenRoot.style.visibility = 'visible'
    this.containerElement.appendChild(fullscreenRoot)
   ...
   * @returns True if root element is visible, false otherwise
   */
  public isRootElementVisible(): boolean {
    const fullscreenRoot = document.getElementById('fullscreen-root')
    return (
      this.containerElement != null
      && fullscreenRoot?.parentElement === this.containerElement
    )
  }
   ...
   */
  public documentIsLoaded(): void {
    this._fileLoadTime = window.performance.now()
  }

  /**
   * Get file load time
   * @returns Time when file was loaded
   */
  public fileLoadTime(): number | null {
    return this._fileLoadTime
  }

  /**
   * Trigger an event from fullscreen
   * @param event - Event name
   * @param data - Event data
   */
  public triggerFromFullscreen(event: string, data?: any): void {
    this.fromFullscreen.trigger(event, data)
  }

  /**
   * Start loading dropped image
   */
  public startLoadingDroppedImage(): void {
    imageLoadingManager.startLoadingImage()
  }

  /**
   * Finish loading dropped image
   */
  public finishLoadingDroppedImage(): void {
   ...
   */
  public getViewportInfo(): typeof this._viewportInfo {
   ...
   * @param offsetX - Viewport offset x
   * @param offsetY - Viewport offset y
   * @param zoomScale - Viewport zoom scale
   * @param isZooming - Whether viewport is zooming
   * @param isPanning - Whether viewport is panning
   */
  ...
  /**
   * Handle tiles rendered event
   */
  public onTilesRendered(): void {
  ...
  /**
   * Handle edit table text event
   */
  public onEditTableText(): void {
  ...
  /**
   * Handle tile renderer changed event
   * @param renderer - The new tile renderer
   */
  public onTileRendererChanged(renderer: any): void {
    onTileRendererChanged(renderer)
  }

  /**
   * Handle create sticky event
   */
  public onCreateSticky(): void {
    recordCreateStickyActive()
  }

  /**
   * Handle different section clicked event
   */
  public onDifferentSectionClicked(): void {
    recordDifferentSectionClicked()
  }

  /**
   * Get next query request ID
   * @returns Next session ID
   */
  public getNextQueryRequestId(): number {
   ...
   */
  public afterQueryCompleted(requestId: number): void {
    resolveSessionPromise(requestId)
  }

  /**
   * Reject all scene graph queries
   */
  public rejectAllSceneGraphQueries(): void {
    rejectAllSessionPromises()
  }

  /**
   * Handle node position changed event
   * @param nodeId - Node ID
   * @param position - New position
   */
  public onNodePositionChanged(nodeId: string, position: any): void {
    Om(nodeId, position)
  }
};

/**
 * Class for handling fullscreen user information
 */
class FullscreenUserInfoHandler {
  /** Redux store */
   ...
   * @throws Error if store is not valid
   * @returns Current state
   */
  private get _state(): any {
    if (!this._store) {
      throw new Error('Calling _state without a valid store')
    }
  ...
  public getUserId(): string | null {
    ...
    const currentUser = this._state.multiplayer.allUsers.find(
      (user: any) => this._state.multiplayer.sessionID === user.sessionID,
    )
  ...
  /**
   * Get user locale
   * @returns User locale or default language
   */
   ...
   * @returns True if user has flag, false otherwise
   */
  public hasUserFlag(flag: string): boolean {
    return Object.keys(this._state.userFlags).includes(flag)
  }
}
    ...
    permissionScopeHandler.user('smart-paste', () => {
      function o(e, t) {
        let i = (function (e, t) {
          let i = new Map()
          !(function e(t, i, n) {
            if (n.set(t.guid, i.guid), t.childCount !== i.childCount)
              throw new Error('Mismatched node trees')
            for (let r = 0; r < t.childCount; r++) {
              if (t.childrenNodes[r].type !== i.childrenNodes[r].type)
                throw new Error('Mismatched node trees')
  ...
  handleAction(e) {
    switch (e.name) {
      ...
      case 'toggle-inline-preview':
        let t = e.payload.source === 'toolbar' ? 'button' : e.payload.source
        atomStoreManager.get(hg).modalStatus === _$$bi.OPEN
          ? (this._store.dispatch(trackDefinedFileEventWrapper({
      ...
      case 'present-sites-full-preview':
        if (!getFeatureFlags().sites && !getFeatureFlags().bake)
          break
        or({
  ...
  _applyDrawingToolStylingAction(e) {
    switch (e) {
      case DesignGraphElements.VECTOR_PENCIL:
        let t = atomStoreManager.get(vectorPencilStyleAtom)
        atomStoreManager.set(vectorPencilStyleAtom, {
          ...t,
          strokeWeight: t.strokeWeight === wv ? uM : wv,
        })
        return
      case DesignGraphElements.BRUSH:
        let i = atomStoreManager.get(brushStyleAtom)
        atomStoreManager.set(brushStyleAtom, {
          ...i,
          strokeWeight: i.strokeWeight === wv ? uM : wv,
        })
        return
      case DesignGraphElements.HIGHLIGHTER:
        let n = atomStoreManager.get(highlighterStyleAtom)
        atomStoreManager.set(highlighterStyleAtom, {
          ...n,
...
function oH() {
  if (getFeatureFlags().ee_unsanitized_png_custom)
    return 'web application/x-figma-png'
  if (getFeatureFlags().ee_unsanitized_png)
...
async function lN({
  ...
  if (!c)
    throw new lR('Widget node does not exist')
...
async function lO({
  ...
  if (!s)
    throw new Error('Cannot run widget while logged out')
...
async function lL(e, t, i) {
  ...
  if (a) {
    if (hasLocalFileId(i)) {
      ...
      if (!e)
        throw new Error(`No code found for ${i} version`)
      r = e
    }
...
let lM = new class {
  ...
  findQueueIndex(e) {
    for (let t = 0; t < this.runQueue.length; t++) {
      if (deepEqual(this.runQueue[t], e))
        return t
    }
    return -1
  }
  ...
  runUserInitiatedWidget(e) {
    ...
    if (!i && !n)
      return this.setRunningWidget(e)
    this.pushBack(e)
  }
...
function lU(e, t) {
  ...
  if (!d)
    return fetchedCanvasWidgetVersions[e]?.[t] ? c?.widgets_whitelist_enforced ? lT.DISALLOWED_BY_ORG : lT.OTHER : void 0
  ...
  if (d.org_id && d.org_id !== currentUserOrgId && c)
    return lT.OTHER
  if (d.org_id && !canMemberOrg(currentUserOrgId, debugState.getState()))
    return lT.ORG_MEMBER_ONLY
  if (checkResourceEligibilityDebug(publishedWidgets[e]))
    return lT.REQUIRES_PAYMENT
  let h = getFullscreenViewEditorType()
  if (h && d.manifest.editorType && !d.manifest.editorType.includes(h))
    return h === 'figjam' ? lT.FIGMA_ONLY : lT.FIGJAM_ONLY
}
class lB {
  constructor(e) {
    this.runWidgets = e
...
class lV {
  ...
  trackLocalWidgetsInPlaygroundFile(e) {
    if (this.hasCheckedLocalWidgetsInPlaygroundFile)
      return
    let {
      selectedView,
    } = debugState?.getState()
  ...
  onTextEditEnd(e, t, i) {
    ...
    if (r?.type !== 'TEXT')
      throw new Error('Can\'t have a text event on non text node')
  ...
  clickWidget(e, t, i) {
    if (!isValidWidgetType(e) && !checkCanRunExtensions())
      return PluginManager.instance.handleUpgrade(PluginAction.RUN_WIDGET)
  ...
  addWidgetToRecentlyUsed(e, t) {
    if (e && !this.canInteractWithWidget(t, e))
      return
  ...
  showTooltip(e, t) {
    ...
    deepEqual(n, atomStoreManager.get(someStateAtom)) || someStateAtom.syncFromFullscreen(n)
  }

  hideTooltip() {
    someStateAtom.syncFromFullscreen(null)
  }
  ...
  showNonInteractableWidgetTooltip(e, t) {
    ...
    someStateAtom.syncFromFullscreen(a)
  }
...
getFeatureFlags().ce_aal_enable_debug && (window.AAL_SETTINGS = aalSettingsConfig, window.resetAALConstants = resetAAL)
let l$ = null
async function lZ(e) {
  if (l$ && e === l$.fileKey)
    return l$
  ...
  if (i.status === 200) {
    let t = await i.json()
    if (!t.fileKey || !t.fixedScrolling)
      throw new Error(`Broken-fixed-scrolling metadata for file ${e} is missing 'fileKey' or 'fixedScrolling'.`)
    l$ = t
    return t
  }
...
let lX = class e extends WithFullscreenFunctionality {
  constructor(t, i, n) {
    super()
    ...
    this.handleSketchImportEvent = (e) => {
      if (this.hasUnsavedChanges() || !e.dataTransfer || e.dataTransfer.files.length !== 1)
        return !1
    ...
    this.triggerActionEnum = (e, t) => {
      ...
      if (t != null && typeof t != 'object')
        throw new Error(`Invalid payload for action ${e}`)
    ...
    this.toggleMenu = (e, t, i) => {
      if (e === SettingsAction.SEARCH) {
        Jf(this._state.dropdownShown)
          ? jD(this.dispatch, this._state.dropdownShown, {
  ...
  addStackOverflowListener() {
    ...
    window.addEventListener('error', (e) => {
      isStackOverflowError({
        ...
        : e.message.includes('Out of memory') && fullscreenCrashHandler.showMemoryCrashModal({}, this.openFileKey(), this._store)
    })
  }
  ...
  handleFileThumbnailMenuItem() {
    ...
    this._store.dispatch(setFileThumbnailOptimistic({
      file_key: e,
  ...
  toggleBrowseAllResourcesModal() {
    ...
    let e = this._store.getState().selectedView
    if (!(e && e.view === 'fullscreen' && (e.editorType === FEditorType.Whiteboard || e.editorType === FEditorType.Slides || e.editorType === FEditorType.Cooper)))
      return
    let t = this._store.getState().universalInsertModal
    t.showing && t.pinned === PinningState.NOT_PINNED
      ? t.pinned === PinningState.NOT_PINNED
        ? (fullscreenValue.triggerAction('set-tool-default', {
          source: 'menu',
        }), this.dispatch(closeUniversalInsertModal()))
  ...
  toggleInteractionRecorderVisibility() {
    ...
    isRecordingEnabled() && (this._store.getState().interactionTestDialogShown ? this.dispatch(hideRecorderAction()) : this.dispatch(showRecorderAction()))
  }

  togglePerfHUDVisibility() {
    if (!getFeatureFlags().perf_hud)
      return
    let e = this._store.getState().selectedView
  ...
  getClipboardData(e) {
    ...
    return desktopAPIInstance.getClipboardData([e]).then(({
      ...
      if (i !== e)
        throw new Error(`Asked for clipboard data with MIME type ${e}, got data with MIME type ${i} instead.`)
  ...
  async readClipboardData() {
    ...
    let t = window.FigmaMobile
    if (!(navigator.clipboard && navigator.clipboard.read) && !t?.readClipboardData)
  ...
  async readFilesFromClipboard() {
    if (!navigator.clipboard?.read)
      throw new Error('No Clipboard API')
    if (!this.fileArrayToString)
      throw new Error('fileArrayToString undefined')
  ...
  async sendSerializedClipboardDataToS3(e, t) {
    let i
    let n = this._state
    if (!_$$n3(n.userAnalyticsData, n.user?.email))
      return
    ...
    try {
      ...
      if (e.status === 204)
        o('clipboard_data_upload_success'); else throw new Error(`Failed to upload serialized clipboard data to S3 with status code: ${e.status}`)
    }
    catch (e) {
  ...
  updateViewportInfo(e, t, i, n, r, a, s, o, l) {
    if (super.updateViewportInfo(e, t, i, n, r, a, s, o, l), desktopAPIInstance && !l && !o) {
      ...
      this._viewportQueryParamValue !== e && (this._viewportQueryParamValue = e, debouncedUpdateViewport(this._store, e))
    }
  }
  ...
  showVisualBellLocalized(e, t, i, n) {
    ...
    o?.showToast
      ? this.NOT_LOCALIZED_showMobileNativeToast(s, null)
      : (e === 'copy-as' && handleAtomEvent({
      ...
      if (navigator.onLine) {
        if (this.hasUnsavedChanges()) {
          ...
          if (localStorageRef) {
            ...
            if (localStorageRef.setItem(i, e.toString()), t && parseInt(t) >= e - 12096e5)
              return
          }
  ...
  showAiContentFillSuggestedVisualBell(e, t) {
    if (this._state.user == null || this._state.openFile == null || (this.dismissEphemeralVisualBells(), !this._state.openFile.canEdit))
      return
    let i = getSingletonSceneGraph()
  ...
  showMagicLinkSuggestedVisualBell(e) {
    if (this._state.user == null || this._state.openFile == null)
      return
    let t = getSingletonSceneGraph().getCurrentPage()
    if (!t)
      return
    ...
    let i = getSingletonSceneGraph()
    if ((AppStateTsApi?.propertiesPanelState()?.propertiesPanelTab?.getCopy() ?? DesignWorkspace.DESIGN) !== DesignWorkspace.PROTOTYPE || atomStoreManager.get(_$$xP) && atomStoreManager.get(d_))
      return
    ...
    if (e === 'create_node_transition') {
      ...
      if (l > 0 || !Yh(this._state.mirror.appModel, 'magic-link'))
        return
    }
  ...
  setTimer(e, t, i, n, r, a, s, o) {
    ...
    for (let a of multiplayer.allUsers) {
      if (a.sessionID !== e || a.sessionID === multiplayer.sessionID)
        continue
      ...
      if (timer.time || t > 0 && t === i) {
        let e = null
        timer.time && (t === 0
          ? (PluginCallbacks.timerChange('timerstop'), this.dispatch(setTimerModalThunk({
            state: 'closed',
            userInitiated: !1,
          })))
  ...
  _selectedValidThumbnailNodeIds() {
    let e = this._store.getState()
    if (e.mirror == null || e.mirror.sceneGraphSelection == null || e.openFile == null)
      return []
    let t = []
    ...
    if (n) {
      switch (t) {
        case FigmaSite.SITE:
          ...
          if (!u || getResourceDataOrFallback(c?.status) !== 'published')
            return
          l = IU({
            domain: u,
            path: o,
  ...
  async _documentIsLoaded() {
    if (fullscreenPerfManager.documentIsLoaded(), desktopAPIInstance?.addTabAnalyticsMetadata({
      ...
      e && !isLocalFileKey(e.fileKey)
        ? (t < 0 && logError('Autosave', 'Trying to initialize autosave without a session ID', {
        ...
        : e || t !== 0
          ? !e && t > 0 && this._store.getState().user && logAutosaveError('Autosave should have been initialized for logged in user.')
          : this._figFileLoadPromise.then(async (e) => {
  ...
  static async maybeEnterRecoveryMode(e) {
    ...
    t?.canEdit
      ? t && i.isRecoveryMode && (AppStateTsApi?.uiState().isRecovery.set(!0), trackEventAnalytics('enter_recovery_mode', {
    ...
    if (!this._fullscreenIsReady) {
      ...
      (function () {
        ...
        getFeatureFlags().internal_only_debug_tools && (window.DebuggingHelpers.webReport = (e) => {
          executeInFakeStack(() => logError('bindings', `DebuggingHelpers report: ${e}`))
        }, window.DebuggingHelpers.cppReport = (t) => {
          executeInFakeStack(() => e(t))
        }, window.DebuggingHelpers.fakeNodeError = () => {
          executeInFakeStack(() => t())
      ...
      window.DebuggingHelpers.exportOfflineLog = () => {
        ...
        if (!e)
          throw new Error('You must have the file open')
  ...
  handleSignedOutEditAttempt(e, t) {
    if (!e && this.isInWorkshopMode())
      return
    let i = this._store.getState().selectedView
    if (i.view !== 'fullscreen' || i.editorType === FEditorType.Design && !this.session.user)
      return
  ...
  enableCodegenMcpServer(e) {
    getFeatureFlags().dt_my_cool_plugin && (atomStoreManager.get(codeOptionsAtom) !== 'xml' || getFeatureFlags().dt_my_cool_plugin_xml || atomStoreManager.set(codeOptionsAtom, 'design_to_react'), atomStoreManager.get(codeOptionsAtom) !== 'jsx' || getFeatureFlags().dt_my_cool_plugin_internal || atomStoreManager.set(codeOptionsAtom, 'design_to_react'), desktopAPIInstance?.setEnableMCP(e, PROCESS_IDENTIFIER, () => this.dispatch(postUserFlag({
      ...
      this.dispatch(VisualBellActions.enqueue({
        type: 'codegen-my-cool-plugin-server',
        message: t
          ? getI18nString('my_cool_plugin.codegen_server_failed_to_start_port_in_use', {
            port: PROCESS_IDENTIFIER,
          })
    ...
    if (t.view === 'fullscreen') {
      if (t.editorType === FEditorType.Design || t.editorType === FEditorType.Illustration) {
        ...
        i(setUniversalInsertModalOpen({
          initialX: 0,
          initialY: 0,
          initialTab: TabCategory.STICKERS_AND_COMPONENTS,
        }))
      }
    }
  }
  ...
  toggleTeamLibraryModal() {
    ...
    if (e.view === 'fullscreen' && e.editorType === FEditorType.Whiteboard) {
      this._store.getState().modalShown?.type === FIGJAM_LIBRARY_MODAL
    ...
    this._store.getState().modalShown?.type === LIBRARY_PREFERENCES_MODAL
      ? this.dispatch(hideModal())
      : this.dispatch(showModalHandler({
        type: _$$T2,
        data: {
  ...
  toggleComponentInsertModal() {
    ...
    if (e.selectedView.view === 'fullscreen' && e.selectedView.editorType === FEditorType.Design) {
      ...
      e.universalInsertModal?.showing
        ? this._state.mirror.appModel.showUi && e.universalInsertModal.pinned !== PinningState.NOT_PINNED
          ? (fullscreenValue.triggerAction('clear-tool', {
            source: 'menu',
          }), this.dispatch(handlePinningState({
            initialX: x,
          ...
          : (this.dispatch(closeUniversalInsertModal()), Fullscreen?.triggerAction('set-tool-default', null))
        : (trackEventAnalytics('Inserts menu opened'), fullscreenValue.triggerAction('clear-tool', {
          source: 'menu',
        }), this.dispatch(setUniversalInsertModalOpen({
          initialX: x,
          initialY: y,
  ...
  showPublishDialog(e) {
    ...
    this.isInDrafts() || !hasTeamPaidAccess(n)
      ? this.dispatch(showModalHandler({
        type: $3,
        data: {
          team: n || null,
  ...
  updateSelectedStyleProperties(e, t, i, n, r, a) {
    let s = {
      ...kiwiParserCodec.decodeMessage(e).nodeChanges[0],
    }
  ...
  showViewChangesNotification(e) {
    ...
    let n = t.modalShown
    showCompareChangesNotification(e, this.dispatch, i, n)
  }
  ...
  setHyperlinkPopup(e, t, i, n, r, a) {
    ...
    if (e.length > 0) {
      ...
      if (nodeIdInThisFile && !versionId) {
        ...
        try {
          ...
          o = e.protocol === 'mailto:'
            ? {
              type: AssetLinkType.MAILTO,
              url: e,
            }
            : e.protocol === 'tel:'
              ? {
                type: AssetLinkType.TEL,
                url: e,
              }
              : isFigmaDomain(e.hostname) || isFigmaDomain(e.host)
                ? isSpecialRoutePath(e.pathname)
                  ? {
                    type: AssetLinkType.FIGMA_PROTOTYPE,
                    url: e,
                  }
                  : e.searchParams.has('version-id')
                    ? {
                      type: AssetLinkType.FIGMA_VERSION,
                      url: e,
                    }
                    : {
                      type: AssetLinkType.FIGMA_FILE,
                      url: e,
                    }
                : {
                  type: AssetLinkType.GENERIC,
                  url: e,
                }
        }
        catch {
    ...
    o
      ? this.dispatch(setHyperlinkPopup({
        urlString: e,
        data: o,
        position: t,
        size: i,
  ...
  handleUserClickOnHyperlink(e, t) {
    t === EmailAction.COPY_TO_CLIPBOARD && e.startsWith('mailto:') || e.startsWith('tel:')
      ? copyHyperlinkToClipboard(this.dispatch, e, 'HyperlinkCanvas')
      : this._store?.dispatch(setupHyperlinkHandler({
        rawInput: e,
      }))
  }
  ...
  setCanvasMentionPopup(e, t, i, n, r, a, s) {
    ...
    o
      ? this.dispatch(setCanvasMentionPopup({
        data: o,
        position: i,
        size: n,
        mouse: r,
  ...
  syncHighlighterStyle(e) {
    ...
    Number.isFinite(strokeWeight) && paints && highlighterStyleAtom.syncFromFullscreen({
      paints,
      strokeWeight,
    })
  }
  ...
  syncToolStyles(e) {
    ...
    toolStylesAtom.syncFromFullscreen({
      shapeWithTextType,
      connectorToolLineStyle,
      connectorToolEndCap,
      washiTapePaint,
    })
    shapeColorAtom.syncFromFullscreen(shapeColor)
    shapeStrokeStyleAtom.syncFromFullscreen(shapeWithTextStrokeStyleType)
    getFeatureFlags().figjam_track_stroke_color && strokeColorAtom.syncFromFullscreen(strokeColor)
    stickyColorAtom.syncFromFullscreen(stickyColor)
    connectorColorAtom.syncFromFullscreen(connectorToolColor)
    sharedStateAtom.syncFromFullscreen(platformShapeSelected)
  }

  syncCurrentToolSetSource(e) {
    shapeSidebarModeAtom.syncFromFullscreen(e)
  }
  ...
  _validateParsedCsvDataAndCreateTable(e, t) {
    return e.meta && e.meta.aborted
      ? (this._maybeShowCSVError(t, 'whiteboard.csv.parse_failed'), !1)
      : e.data.length === 0
        ? (this._maybeShowCSVError(t, 'whiteboard.csv.file_empty'), !1)
        : (this.dispatch(_$$n2.set({
  ...
  generateFlappFromPastedText(e) {
    return !(function (e) {
      ...
      for (let e of a4.templates) {
        if (t.match((e.startsWith('^') ? '' : '^(?:www.)?') + e))
          return !0
      }
      return !1
    }(e))
      ? !!isProtoViewerUrl(e) && (fullscreenValue.triggerActionInUserEditScope('insert-interactive-element-into-active-slide', {
        type: 'EMBED',
        url: e,
  ...
  generateWidgetFromURL(e, t) {
    return processPasteUrls(this._store.dispatch, e, t).some(e => e.valid)
  }
  ...
  changeDocumentColorProfile(e, t) {
    let i = this._store.getState().selectedView;
    (i && i.view === 'fullscreen' && isDesignOrIllustration(i.editorType) || this._state.openFile?.canEdit) && e !== t && (e === ColorProfileEnum.LEGACY
      ? r6(this.dispatch, e, t, 'assign', this._state.openFile?.key)
      : this.dispatch(showModalHandler({
        type: r7,
        data: {
  ...
  autoRenameLayers(e, t) {
    let i = {
      ...
      source: getActionSourceString(e),
  ...
  closeLeftRailOverlay() {
    atomStoreManager.set(genericAtomFamily1, void 0)
  }

  setSitesInsertsOverlay() {
    isSitesFeatureEnabled() && atomStoreManager.set(genericAtomFamily1, $e.INSERT)
  }

  setSitesFindOverlay() {
    isSitesFeatureEnabled() && atomStoreManager.set(genericAtomFamily1, $e.FIND)
  }
