...
export async function $$_0({
  ...
  try {
    for (;;) {
      ...
      if (t.aborted) break;
      if (value?.createNodes) for (let e of value.createNodes) {
        if (Lo("handleTextToTemplate", "Create or update node", e), "INSTANCE" === e.node.type && e.node.componentType && e.node.componentKey) {
        ...
        } else if ("WIDGET" === e.node.type) {
          if (e.node.pluginData && e.node.pluginData.pluginSvgPreviewUrl) try {
