  ...
  useEffect(() => {
    let e = new MutationObserver(() => {
      s((e) => {
        ...
        if (compareThemePreferences(e, t))
          return e
...
function B(e) {
  (async () => {
    if (desktopAPIInstance && e.getState().selectedView.view === 'fullscreen' && (await fullscreenValue.onReady(), fullscreenValue.isReady())) {
      let t = e.getState()
      !(function (e, t, i) {
        if (!desktopAPIInstance || e.view !== 'fullscreen')
          return
        let n = {
          selectedView: e,
...
async function el() {
  try {
    ...
    if (function () {
      try {
        Cookie.set(es, 'enabled', {
          sameSite: 'None',
          secure: !0,
        })
        let e = Cookie.get(es) === 'enabled'
        Cookie.remove(es, {
          sameSite: 'None',
          secure: !0,
        })
...
class ey {
  constructor(e) {
    ...
    this.handleWindowMoveStart = (e) => {
      if (this.checkAndMarkEventSeen(this.seenMoveStartEvents, e))
        return
      let t = eA(e)
      t && (this.mouseClientOffset = t)
    }
    this.handleWindowMoveCapture = (e) => {
      if (this.checkAndMarkEventSeen(this.seenMoveCaptureEvents, e))
        return
      let {
        moveStartSourceIds,
      } = this
...
class e$ extends PureComponent {
  ...
  render() {
    ...
    if (!(this.state.error instanceof eX))
...
let i6 = e => t => function (i) {
  ...
  else if (userTeamFlagPost.matches(i)) {
...
let ng = e => t => function (i) {
    ...
    return t(i)
  {
    ...
    }), e.dispatch(componentClearLocal()), resetAllAsyncPromises(), e.dispatch(notificationActions.clearAll()), n.modalShown && e.dispatch(hideModal()), n.universalInsertModal.showing && e.dispatch(closeUniversalInsertModal()), e.dispatch(clearState({})), fullscreenValue.onReady().then(() => {
      e.dispatch(setVotingSessionInfo({
...
let nS = e => t => function (i) {
  if (isChromebookTabbed() && selectViewAction.matches(i)) {
    let n = e.getState()
    if (n.selectedView === _$$o4 || i.payload === _$$o4)
      return t(i)
  ...
  if (selectViewAction.matches(i)) {
    ...
    if (r.view === 'desktopNewTab' && (n.selectedView.view === 'fullscreen' || n.selectedView.view === 'prototype'))
      return
...
function rZ({
  ...
  return jsx(TableRow, {
    ...
    children: (i ? r$ : rq).map(i => jsx(SortableHeaderCell, {
      className: classNames()(i === 'alpha' ? 'dsa_subscription_list_file_rows--nameCol--pVdjf' : 'dsa_subscription_list_file_rows--numCol--mF91c library_modal_stats--numCol---FbhI', {
...
function r7({
  ...
  if (x.status === 'loaded' && x.data.is_currently_migrating)
    return jsx(_$$l7, {})
...
let as = registerModal((e) => {
  ...
  if (useEffect(() => {
    c && e.tab === navigationRoutes.FONTS && (t(hideModal()), t(selectViewAction({
...
let am = e => t => function (i) {
  if (selectViewAction.matches(i)) {
    ...
    hasStoredValue() && t !== n && handlePluginError()
  }
  return t(i)
}
let af = keyBy
...
let aj = new RealtimeSubscriptionManager({
  ...
  convertLivegraphMessage: (e, t, i, n) => (function (e, t) {
    ...
    for (let s of t.team?.componentUpdates || []) {
      ...
      if (s.updateType === 'delete' && !d || !s?.component && s.updateType !== 'delete' || s?.component && d >= aM(s.component?.updatedAt?.toISOString() || ''))
        continue
      let c = aV(s)
...
let aU = new RealtimeSubscriptionManager({
  ...
  convertLivegraphMessage: (e, t, i, n) => (function (e, t) {
    ...
    for (let a of t.project?.componentUpdates || []) {
      ...
      if (a.updateType === 'delete' && !c || !a?.component && a.updateType !== 'delete' || a?.component && c >= aM(a.component?.updatedAt?.toISOString() || ''))
        continue
      let u = aV(a)
...
let aB = new RealtimeSubscriptionManager({
  ...
  convertLivegraphMessage: (e, t, {
    ...
    for (let o of (i.file || s.push({
      ...
      if (o.updateType === 'delete' && !c || !o?.component && o.updateType !== 'delete' || o?.component && c >= aM(o.component?.updatedAt?.toISOString() || ''))
        continue
      let u = aV(o)
...
let az = new RealtimeSubscriptionManager({
  ...
  convertLivegraphMessage: (e, t, i, n) => (function (e, t) {
    ...
    for (let s of t.team?.stateGroupUpdates || []) {
      ...
      let d = aM(l?.[a]?.[o]?.[t]?.updated_at || '')
      if (s.updateType === 'delete' && !d || !s?.stateGroup && s.updateType !== 'delete' || s?.stateGroup && d >= aM(s.stateGroup?.updatedAt?.toISOString() || ''))
        continue
      let c = aG(s)
      s.updateType === 'post' ? i.push(c) : s.updateType === 'put' ? n.push(c) : s.updateType === 'delete' && r.push(c)
    }
...
let aH = new RealtimeSubscriptionManager({
  ...
  convertLivegraphMessage: (e, t, i, n) => (function (e, t) {
    ...
    for (let a of t.project?.stateGroupUpdates || []) {
      ...
      if (a.updateType === 'delete' && !c || !a?.stateGroup && a.updateType !== 'delete' || a?.stateGroup && c >= aM(a.stateGroup?.updatedAt?.toISOString() || ''))
        continue
      let u = aG(a)
      a.updateType === 'post' ? i.push(u) : a.updateType === 'put' ? n.push(u) : a.updateType === 'delete' && r.push(u)
    }
...
let aW = new RealtimeSubscriptionManager({
  ...
  convertLivegraphMessage: (e, t, {
    ...
    for (let o of (i.file || s.push({
      ...
      let c = aM(l?.[d]?.[s]?.[t]?.updated_at || '')
      if (o.updateType === 'delete' && !c || !o?.stateGroup && o.updateType !== 'delete' || o?.stateGroup && c >= aM(o.stateGroup?.updatedAt?.toISOString() || ''))
        continue
      let u = aG(o)
      o.updateType === 'post' ? n.push(u) : o.updateType === 'put' ? r.push(u) : o.updateType === 'delete' && a.push(u)
    }
...
let aQ = new RealtimeSubscriptionManager({
  ...
  convertLivegraphMessage: (e, t, i, n) => (function (e, t) {
    if (!t.team || !t.team.fontFileUpdateForTeam)
      return []
...
let aJ = new RealtimeSubscriptionManager({
  ...
  convertLivegraphMessage: (e, t, i, n) => (function (e, t) {
    if (!t.org || !t.org.fontFileUpdateForOrg)
      return []
...
let a2 = new RealtimeSubscriptionManager({
  ...
  livegraphArgs: e => ({}),
  convertLivegraphMessage: (e, t, i, n) => (function (e, t) {
    if (e.getState().user?.id !== t.currentUser.id)
      return []
    let i = []
    ...
    for (let e of t.currentUser.allBaseOrgUsers || []) {
      ...
      t
        ? t.updated_at < e.updatedAt?.toISOString() && i.push({
          method: 'put',
          type: 'org_user',
          org_user: a5(e),
        })
...
function a3(e, t) {
  switch (t.method) {
    ...
    case 'delete':
      ...
      i && i.id && e.dispatch(mergePublishedPluginThunk({
        publishedPlugins: [i],
        src: 'realtime',
      }))
  }
...
function sl(e) {
  return (t = {}, i) => {
    if (!(i.payload && typeof i.payload == 'object' && 'role' in i.payload))
      return t
  ...
  matchMessage(e, t) {
    if (e.type && t.type && e.type === t.type && e.method && t.method && e.method === t.method) {
      for (let i of Object.keys(e.objects)) {
        if (String(e.objects[i].id) === String(t.objects[i].id))
          return !0
      }
    }
    return !1
...
let sS = new RealtimeSubscriptionManager({
  ...
  convertLivegraphMessage: (e, t, i, n) => (function (e, t) {
    if (!t.currentUser.teamRoleRequestUpdates)
      return []
...
let sk = new RealtimeSubscriptionManager({
  ...
  convertLivegraphMessage: (e, t, i, n) => (function (e, t) {
    if (!t.currentUser.eduGracePeriodUpdate)
      return []
    let i = []
...
let sO = new RealtimeSubscriptionManager({
  ...
  livegraphArgs: e => ({}),
  convertLivegraphMessage: (e, t, i, n) => (function (e, t) {
    if (!t.currentUser || !t.currentUser.userTeamFlags)
      return []
...
let sj = new class {
  ...
  handleRealtimeSubscription(e, t) {
    if (getFeatureFlags().skip_lg_shim_in_bell_view && t.getState().selectedView?.view === 'feed')
      return
...
let sW = getInitialOptions().disable_realtime
  ? e => e => e
  : e => t => function (i) {
    let n = t(i)
            ...
            switch (t.method) {
              ...
              case 'put':
                if (i && i.id) {
                  ...
                  if (delete t.file_key, e.dispatch(processHubFilesThunk({
                    hubFiles: [t],
                    src: 'realtime',
                  })), i.file_key) {
                ...
                break
              case 'delete':
                i.file_key && e.dispatch(delFigFilePublishedAsHubFile({
                  fileKey: i.file_key,
                }))
            }
          }
    ...
    if (deleteAllPlugins.matches(i)) {
      ...
      if (t && r.user_id === t.id) {
        if (r.resource_type === FResourceCategoryType.FILE && e.getState().openFile?.key === r.resource_id_or_key)
          return n
...
let oo = e => t => function (i) {
  ...
  if (searchResultClicked.matches(i)) {
    ...
    if (n.parameters.query !== i.payload.parameters.query)
      return t(i)
...
function og(e, t) {
  ...
  if (resetTileSelection.matches(t))
    return om()
...
let oE = class e {
  ...
  expectAccessibleVisualBell(e, t = 1) {
    ...
    let n = i.filter((t) => {
      if (typeof e == 'string')
        return t.message?.toLocaleLowerCase() === e.toLocaleLowerCase()
      if (typeof e == 'object')
...
let oG = combineReducers({
  activeThread(e = null, t) {
    return resetComments.matches(t) || resetActiveCommentId.matches(t)
      ? null
      : setActiveComment.matches(t)
  ...
  threads(e = {}, t) {
    ...
    if (deleteCommentAction.matches(t)) {
      ...
      if (t.payload.attachment && !t.payload.attachment.isUploading && !i?.reply.attachments?.[t.payload.attachment.id])
        return e
  ...
  newComment(e = DEFAULT_THREAD_STATE, t) {
    ...
    if (setNewCommentAttachment.matches(t)) {
      if (t.payload.attachment && !t.payload.attachment.isUploading && !e.attachments[t.payload.attachment.id])
        return e
  ...
  editingComment(e = null, t) {
    if (resetComments.matches(t) || stopEditingComment.matches(t) || setCommentContentAction.matches(t))
      return null
    ...
    if (setEditingAttachment.matches(t)) {
      if (t.payload.attachment && !t.payload.attachment.isUploading && !e?.attachmentUpdates?.attachments[t.payload.attachment.id])
        return e
      if (t.payload.attachment) {
        return {
          attachmentUpdates: {
  ...
  emojiPicker(e = null, t) {
    return resetComments.matches(t) || setActiveComment.matches(t) || setNewCommentActive.matches(t)
      ? null
  ...
  savingCommentUuids(e = oV, t) {
    if (setCommentSaving.matches(t))
      return new Set(e).add(t.payload.commentUuid)
...
let oQ = combineReducers({
  ...
  feeds(e = oq, t) {
    if (resetCommentState.matches(t))
      return oq
    if (setComments.matches(t)) {
...
let o8 = combineReducers({
  logs(e = [], t) {
    return setActivityLogsAction.matches(t) ? t.payload.logs : e
  },
  cursor(e = Object.create(null), t) {
    return setActivityLogsAction.matches(t) ? t.payload.cursor : clearCursorActivityLogsAction.matches(t) ? Object.create(null) : e
  },
  isLoading(e = !1, t) {
    return !setActivityLogsAction.matches(t) && (!!getActivityLogsAction.matches(t) || e)
  },
  isNewQuery(e = !1, t) {
    return getActivityLogsAction.matches(t) ? t.payload.newQuery : e
  },
  lastQuery(e = {
    date: {
      start: null,
    ...
    if (getActivityLogsAction.matches(t)) {
      let {
        emails,
        date,
        eventName,
...
function lp(e, t) {
  ...
  if (addFileImportToQueue.matches(t)) {
    t.payload.forEach((t) => {
      if (isPdfFile(t.name) && e.step === ImportEventType.FILE_IMPORT_WITH_CANCELED_PDF)
        return
    ...
    if (e.step === ImportEventType.CONFIRM_PDF_IMPORT || e.queue.length === 0)
      return e
...
let lg = composeFn((e = {}, t) => {
  ...
  if (addFileFavoriteAction.matches(t) || removeFileFavorite.matches(t)) {
    ...
    for (let t of i) {
      t.key in e
        ? n[t.key] = fileReducer(e[t.key], filePutAction({
          file: t,
        }))
        : n[t.key] = t
    }
...
let ly = oH(hubFilePublishActionCreators)
function lb(e = {}, t) {
  if (hubFilePutHubFileRemix.matches(t)) {
    let {
      hubFileId,
      from,
      to,
...
let lx = combineReducers({
  ...
  idpUsers(e = {}, t) {
    ...
    if (!deleteOrgInviteAction.matches(t))
      return e
    {
      let i = {
        ...e,
...
function lD(e) {
  return (t = Object.create(null), i) => {
    ...
    if (componentClearPublishedItems.matches(i) && i.payload.type === e)
      return {}
    ...
    if (!deleteTeamAction.matches(i) || !(i.payload.team.id in t))
      return t
    {
      let e = {
        ...t,
...
let lj = combineReducers({
  ...
  thumbnails: (e = {}, t) => {
    if (componentClearLocal.matches(t))
      return {}
...
function lW(e, t) {
  if (!e || prototypeResetRecents.matches(t))
    return []
  ...
  if (recentPrototypeUnmarkViewed.matches(t))
    return e.filter(e => e.file_key !== t.payload.fileKey || e.page_id !== t.payload.pageId)
...
let lY = bindWithIgnore((e, t) => putRepoOptimist.matches(t) || putRepoPermissions.matches(t)
...
function lJ(e = [], t) {
  if (initRecentRepos.matches(t)) {
    if (t.payload.recent_repos)
      return t.payload.recent_repos
  }
  else if (createRepoDeleteAction.matches(t)) {
    ...
    if (t.payload.repo.trashed_at)
      return e.filter(e => t.payload.repo.id !== e.repo.id)
  }
  else if (postRecentRepo.matches(t) || putRecentRepo.matches(t)) {
...
let dd = combineReducers({
  config(e = null, t) {
    return orgSamlConfigSet.matches(t) ? t.payload.orgSamlConfig : e
  },
  isFetching(e = !1, t) {
    return !orgSamlConfigSet.matches(t) && (!!orgSamlConfigGet.matches(t) || e)
  },
})
function dc(e = {}, t) {
...
function dK(e, t) {
  if (!e || resetCanvasSearch.matches(t))
    return dW
  if (setCanvasSearchQuery.matches(t)) {
    return {
      ...e,
      query: t.payload,
    }
  }
  if (toggleCanvasSearchCategoryFilter.matches(t)) {
    let i
    e.filters[t.payload]
      ? (i = {
        ...e.filters,
  ...
  if (clearCanvasSearchCategoryFilters.matches(t)) {
    let t = dY(e.filters)
    return {
      ...e,
      filters: t,
  ...
  if (setCanvasSearchScope.matches(t)) {
    return {
      ...e,
      scope: t.payload,
    }
  }
  if (setCanvasSearchMode.matches(t)) {
    return {
      ...e,
      mode: t.payload,
    }
...
function dJ(e = Object.create(null), t) {
  ...
  if (!exportPickerCheckAction.matches(t))
    return e
  {
    let i = {
      ...e.checked,
...
function cd(e = d9(), t) {
  ...
  if (setTimerModal.matches(t))
    return ci(e, t.payload)
  if (setTimerAudioEnabled.matches(t))
    return ct(e, t.payload)
  if (setMusicVolume.matches(t))
    return cn(e, t.payload)
  if (setSelectedSongId.matches(t))
    return cr(e, t.payload)
  if (resetLocalTimer.matches(t))
    return ca()
...
function cu(e = cc, t) {
  if (dismissJoinConfirmation.matches(t)) {
    return {
      ...e,
      hasDismissedJoinConfirmation: !0,
    }
  }
  if (hideJoinVotingSessionModal.matches(t)) {
    return {
      ...e,
      hasDismissedJoinConfirmation: !1,
    }
  }
  if (setTitle.matches(t)) {
    return {
      ...e,
      votingParams: {
        ...e.votingParams,
  ...
  if (setVotesPerUserLimit.matches(t)) {
    return {
      ...e,
      votingParams: {
        ...e.votingParams,
  ...
  else if (unsetHoveredInModalVotePin.matches(t)) {
...
let cE = combineReducers({
  ...
  selectionProperties(e = {}, t) {
    ...
    if (updateMirror.matches(t) && t.payload.selectionProperties) {
      ...
      let n = i.message
      if (n == null && isEmptyObject(i.derivedProperties.changes) && isEmptyObject(i.derivedProperties.mixed))
        return e
  ...
  return initAction.matches(t)
    ...
    : fullscreenOpen.matches(t)
      ...
      : newFileLoaded.matches(t)
        ? e != null && e.key === t.payload.file.key ? e : t.payload.file
        : closeFullscreenAction.matches(t)
          ? null
...
function cH(e = cz, t) {
  return initAction.matches(t) || attemptClose.matches(t)
    ? cz
    : updateMultiplayerState.matches(t)
      ? t.payload
...
function c$(e, t) {
  return e == null ? 'library' : setQuickAccessToolActionCreator.matches(t) ? t.payload.toolType : e
}
function cZ(e = !1, t) {
  return setImageDialogActionCreator.matches(t) ? t.payload.open : e
}
function cX(e = !1, t) {
  return !!showRecorderAction.matches(t) || !hideRecorderAction.matches(t) && e
}
function cQ(e = !1, t) {
  return setDelightfulToolbarOverflowMenuActionCreator.matches(t) ? t.payload.open : e
}
function cJ(e = !1, t) {
  return setIsMakeSomethingV2ActiveActionCreator.matches(t) ? t.payload.isActive : e
}
...
function c4(e = null, t) {
  return setHyperlinkPopup.matches(t)
    ? t.payload
...
function c3(e = null, t) {
  return setCanvasMentionPopup.matches(t)
    ? t.payload
...
function c6(e = [], t) {
  if (recentlyUsedQuickCommands.matches(t))
    return t.payload
...
let uL = oH(PublishActionCreators)
let uM = {
  showProgressBar: !0,
  showComments: !1,
...
let uB = combineReducers({
  byId(e = {}, t) {
    ...
    if (createPublicUserPutManyAction.matches(t)) {
      let {
        users,
      } = t.payload
      return {
...
async function p2(e, t, i) {
  ...
  return n.status === 'error'
    ? (pX('fetch_error_currently_deployed'), 'canceled')
    : n.data
...
class p4 {
  ...
  async check() {
    try {
      let e = Date.now()
      if (e - this._lastReloadCheckTime < pz)
        return
