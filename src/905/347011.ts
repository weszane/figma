...
import { extractFavoritedResource, isHighLimit } from '../figma_app/863319';
 ...
 */
export function useFavoriteResource(resourceId: string, resourceType: string, orgId: string | null = null, teamId: string | null = null): FavoriteHookResult {
  ...
  const favoriteData = subscriptionResult.data ? extractFavoritedResource(subscriptionResult.data, resourceType) : undefined;

  // Determine if maximum favorites limit has been reached
  const hasReachedMaxFavorites = !!subscriptionResult.data?.favoritedResources?.numFavorites && isHighLimit(subscriptionResult.data?.favoritedResources.numFavorites);

  // Get user sidebar sections with fallback
  const userSidebarSections = subscriptionResult.data?.currentUser ? getResourceDataOrFallback(subscriptionResult.data.currentUser.userSidebarSections) : null;
