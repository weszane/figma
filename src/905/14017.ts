 ...
 */
function MonetizationLabel({ resource }: { resource: any }): JSX.Element {
 ...
 */
function ResourceMonetizationLabel({ resource }: { resource: any }): JSX.Element | null {
  const profile = useSelector((state: any) =>
    'authedActiveCommunityProfile' in state ? state.authedActiveCommunityProfile : null,
  )
  if (!resource)
    return null

  if (
    !(profile && isOrgOrTeamExport(profile))
    && hasMonetizedResourceMetadata(resource)
    && resource.community_resource_payment
    && isSubscriptionActive(resource.community_resource_payment)
  ) {
  ...
  if (hasMonetizedResourceMetadata(resource)) {
    return jsx(MonetizationLabel, { resource })
  }
...
export function UsageCountDisplay({
  ...
  return jsxs('div', {
    ...
    children: [jsx(SvgComponent, { svg: _$$A }), formatNumber(usageCount)],
  })
}
 ...
 */
export function ResourceTile(e: { resource: any }): JSX.Element | null {
  let t: string, i: string, a: string
  const { resource } = e
  if (hasResourceType(resource)) {
    t = resource.name
    i = resource.resource_type
    a = buildCommunityUrl({ resource }) || new URL(resource.rdp_url).pathname
  }
  else {
    t = getCurrentVersion(resource).name
    i = mapResourceTypePlural(resource)
    a = buildCommunityPathById({ resource })
  }
  ...
  const publisherDisplay = multiplePublishers
    ? jsx(So, {
        className: classNames(LF, _W),
        preview: jsx(r5, { authors: publishers }),
        children: publisherName,
      })
    : jsx('span', { className: _W, children: publisherName })
          ...
          jsxs('div', {
            ...
            children: [
              jsx(Link, {
                ...
                to: new ProfileRouteState({ profileHandle: firstPublisher.profile_handle }).to,
              ...
              jsxs('span', {
                ...
                children: [
                  '\xA0\xA0\u2022\xA0\xA0',
                  jsx(ResourceMonetizationLabel, { resource }),
                ],
              }),
            ],
          }),
