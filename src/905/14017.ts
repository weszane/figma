...
function v({
  ...
  return e
    ? !(t && cs(t)) && hasMonetizedResourceMetadata(e) && e.community_resource_payment && isSubscriptionActive(e.community_resource_payment)
        ? jsx(A, {})
        : hasFreemiumCode(e)
          ? jsx(y, {})
          : hasMonetizedResourceMetadata(e)
            ? jsx(_, {
                resource: e,
              })
            : isThirdPartyMonetized(e) ? jsx(y, {}) : jsx(b, {})
    : null
}
export function $$O1({
  likeCount: e,
...
export function $$L0(e) {
  ...
  hasResourceType(resource)
    ? (t = resource.name, i = resource.resource_type, a = buildCommunityUrl({
        resource,
      }) || new URL(resource.rdp_url).pathname)
    : (t = getCurrentVersion(resource).name, i = mapResourceTypePlural(resource), a = buildCommunityPathById({
        resource,
      }))
