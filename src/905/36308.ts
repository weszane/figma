...
/**
 * Determines the appropriate mask action based on the current selection state.
 *
 * @returns 'mask-selection' if no mask or mixed state, 'remove-mask' if masked
 * @example
 * // When selection is not masked
 * $$nR1() // returns 'mask-selection'
 *
 * // When selection is masked
 * $$nR1() // returns 'remove-mask'
 */
export function getMaskState(): 'mask-selection' | 'remove-mask' {
  const selectionMaskState = AppStateTsApi.editorState().selectionIsMask.getCopy();
  switch (selectionMaskState) {
    ...
    case SwitchState.ON:
      return 'remove-mask';
    default:
      // Ensures exhaustive checking - should never reach here with proper enum values
      throw new Error(`Unexpected mask state: ${selectionMaskState}`);
  }
}
export function generateFullscreenMenuItems(e) {
  let t;
  let {
    fileMenuArgs,
...
export const p = generateFullscreenMenuItems;
export const P = getMaskState;