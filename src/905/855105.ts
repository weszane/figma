...
export class OrgViewHandler {
   ...
   */
  pathToSelectedView(params: { currentUserOrgId?: string }, pathSegments: string[]): {
    view: string
    orgId: string
    orgViewTab: string
  } | null {
    ...
    if (pathSegments[1] === 'files' && pathSegments.length >= 3) {
      ...
      return matchedTab ? { ...result, orgViewTab: matchedTab } : null
    }
  ...
  requireHistoryChange(
    prevView: { view?: string, orgId?: string },
    nextView: { view?: string, orgId?: string },
  ): boolean {
   ...
   */
  selectedViewName(
    view: { view?: string, orgId?: string, orgViewTab?: string },
    context: { orgById: Record<string, { name: string }> },
  ): string | null {
    if (view.view !== 'org')
      return null
    const org = context.orgById[view.orgId!]
    if (view.orgViewTab === X.HOME)
      return org ? org.name : ''
    if (view.orgViewTab === X.PLUGINS)
      return getI18nString('org_view.view_selector.plugins')
    return 'Org'
  }
   ...
   */
  selectedViewToPath(
    view: { view?: string, orgViewTab?: string },
    context: { user?: { id: string } },
  ): string | null {
    if (view.view !== 'org')
      return null
    const orgPath = j(context)
    if (!orgPath)
      return null
    let path = `/files${orgPath}`
    if (view.orgViewTab)
      path += `/${view.orgViewTab}`
    if (context.user)
      path += `?fuid=${context.user.id}`
    return path
  }
