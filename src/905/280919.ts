...
export function $$k1({
  ...
  let j = function (e) {
    let t = getInitialDynamicConfig('datadog_rum_plan_rollout_config');
    let i = function () {
      let e = getUserPlan();
      return e && C.includes(e) ? e : 'fallback';
    }();
    let n = t.get('fallback', w);
    return e === 'user-employee' ? t.get('employee', n) : t.get(i, n);
  }(M);
  let U = isDebugFlowActive();
  if (!(a = a || U) && (!user_data || T(R(user_data.email)) && j.rolloutPercent !== 100 && murmurHash(user_data.id, 7) % 100 >= j.rolloutPercent)) return;
  ...
  let K = [];
  datadog_rum_trace_header && getInitialDynamicConfig('datadog_rum_trace_config').get('allowed_urls', []).forEach(e => {
    try {
      K.push(new RegExp(e));
    } catch (t) {
  ...
  $$D0.configure({
    ...
    onCircuitBreakerError: () => {
      datadogRum.addError(new Error(`${P}: Rate limit exceeded, disabling send events`), {
        rumCircuitBreaker: !0,
...
let O = class e {
  constructor() {
    ...
    this.debouncedCountIncrement = debounce(t => {
      ...
      this._enableEventWeighting && t.type === 'resource' && (e.staticResourceEventTypes.has(t.resource?.type) || t.resource?.type === 'other' && t.resource?.url.endsWith('.min.js.br')) && (i = 0.1);
    ...
    this.trackSendEventForCircuitBreaker = e => !this._overallCircuitBreakerSendsDisabled && (this._numWeightedEventsInBucket > this._overallCircuitBreakerRatePerSecond ? (this._overallCircuitBreakerSendsDisabled = !0, this._onCircuitBreakerError(), defer(() => this._onDisableDatadog()), !1) : (this.debouncedCountIncrement(e), !0));
  }
  static getInstance() {
  ...
  beforeSend(e, t) {
    if (e.type === 'error' && e.error?.message?.startsWith(P)) return !0;
    if (!this.shouldSendEvent(e, t) || !this.trackSendEventForCircuitBreaker(e) && e.type !== 'view') return !1;
    try {
  ...
  shouldSendEvent(e, t) {
    ...
    if (this._collectErrors && e.type === 'error') {
      if (e.error.message.startsWith('[Figment]')) return !1;
    } else if (e.type === 'resource') {
      for (let t of I) {
        if (e.resource.url.includes(t)) return !1;
      }
    ...
    } else if (e.type === 'action' && e.action.type === 'click' && (e.action.target?.name ?? '').startsWith('Screenreader support')) {
      return !1;
    }
    return !!this._collectErrors || e.type !== 'error';
  }
  modifyContextWithCustomEventProcessing(e, t) {
    if (e.context || (e.context = {}), this.sanitizeResourceUrl(e), getFeatureFlags().additional_rum_context_logging && e.type === 'long_task') {
      if (e.long_task.scripts) {
        for (let t of e.long_task.scripts) t.source_url?.endsWith('.br') && (t.source_url = t.source_url.substring(0, t.source_url.length - 3));
      }
      let t = e.context;
      if (debugState) {
    ...
    if (this._collectErrors && e.type === 'error' && (e.error.stack ? e.error.stack = N(e.error.stack) : e.error.stack = N(e.error.handling_stack), e.error.causes?.forEach(e => {
      e.stack = N(e.stack);
    })), e.type === 'resource' && 'performanceEntry' in t && t.performanceEntry && t.performanceEntry.serverTiming) {
      let i = {
        request: {
          upstream: {
            duration: {}
      ...
      for (let {
        ...
        description,
        duration
      } of t.performanceEntry.serverTiming) {
        switch (name) {
          case 'cdn-cache-hit':
            i.cache = 'hit';
            break;
...
function L(e) {
  ...
  setSentryContext('datadog_rum_link', {
    session_link: `https://go/${cluster_name === 'gov' ? 'dd-gov' : 'dd'}/rum/sid/${e}`
  });
}
export const Tf = $$D0;
export const bj = $$k1;
