...
export function $$k1({
  ...
  if (!(a = a || U) && (!user_data || T(R(user_data.email)) && j.rolloutPercent !== 100 && murmurHash(user_data.id, 7) % 100 >= j.rolloutPercent))
    return
...
let O = class e {
  ...
  beforeSend(e, t) {
    if (e.type === 'error' && e.error?.message?.startsWith(P))
      return !0
    if (!this.shouldSendEvent(e, t) || !this.trackSendEventForCircuitBreaker(e) && e.type !== 'view')
      return !1
    try {
  ...
  shouldSendEvent(e, t) {
    if (!getFeatureFlags().datadog_rum_allow_send || !this.isSendEventsEnabled())
      return !1
    if (this._collectErrors && e.type === 'error') {
      if (e.error.message.startsWith('[Figment]'))
        return !1
    }
    else if (e.type === 'resource') {
      for (let t of I) {
        if (e.resource.url.includes(t))
          return !1
      }
