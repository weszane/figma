import type { AnnotationCategoryData, ExportSetting, InteractionReaction, NavigationAction, ProcessedRegion, SolidPaint, TriggerEvent } from '../types';
import { $D } from '../11';
...
import { checkIncrementalUnsafeMember, ensurePluginPageLoaded, loadAllPluginPages, loadInternalCanvasMemoized, markPageLoaded } from '../816197';
...
import { CUU, Egt, Ez5, glU, L5V, mgy, NFK, NfO, NUh, sAE, SpR, tbx, v$l, w3z, y0x, ZHy, zIx, zkO } from '../../figma_app/763686';
import { QC } from '../../figma_app/913823';
import { AdvancedNavigationProcessor, arrayToTransformMatrix, convertInternalPaintToExternal, normalizeBlendMode, PaintConversionUtils, transformMatrixToArray } from '../modules';
import { processImageFilters } from '../modules/paint-management';
import { createAdvancedVariableExpressionProcessor, createAdvancedVariableParser } from '../modules/variable-expression-processing';
import { convertGridLayoutConfig, convertListOption, convertTextDecorationThicknessFromLegacy, convertTextDecorationThicknessToLegacy, getNodeById, handlePaintProperty, hasResizeToFit, iC, ik, iR, isInImmutableContext, iT, iw, processEffect, processEffectWithValidation, processGridLayout, processUrlData, processVectorData, processWidgetSyncData, t$, tq, validateImmutableFrame, validateNamespace, variableDefinitions } from './helper';
...
export function normalizeTextCase(vm, fallback, textCase, smallCaps) {
  if (textCase === 'mixed') return fallback;
    ...
    options: {
      ...
      set(val) {
        ...
        for (let idx = 0; idx < parsedArr.length; idx++) {
          ...
          if (!item) throw new Error(`Grid track size is undefined at index ${idx}`);
          permissionScopeHandler.plugin('plugin-grid-track-size', () => {
            node.setGridTrackType(dimension, idx, sAE[item.type]);
            if (item.value) node.setGridTrackSize(dimension, idx, item.value);
          });
        }
        return vm.undefined;
      }
      ...
      options: {
        ...
        set(val) {
          ...
          if (parsed != null) {
            ...
            if (node.type === 'SLIDE') throw new Error(`Cannot set ${propName} on slide nodes`);
          ...
          getFeatureFlags().dse_min_max_plugin_behavior ? Egt?.setNodeTransformProperties(node.guid, {
            [propName]: parsed
...
export function validateResizeDimensions(node, widthHandle, heightHandle, vm) {
  ...
  if (node.type === 'SECTION' && node.sectionContentsHidden) throw new Error('Cannot resize hidden sections');
  if (node.type === 'SLIDE') throw new Error('Cannot resize slide nodes');
  return {
    width,
    height
  };
...
async function getNodePublishStatus(node) {
  ...
  if (node.type === 'SYMBOL') {
    ...
    if (!symbol || symbol.unpublished_at) return 'UNPUBLISHED';
...
export const NodeAPI = {
  ...
  clone({
    ...
    defineVmFunction({
      ...
      cb() {
        ...
        if (editorType === _$$nT.Sites && nodeType === 'CANVAS') throw new Error('Cannot add pages to a Site');
        if (node.isResponsiveSet) throw new Error('Cannot clone a webpage');
  ...
  cloneWidget({
    ...
    r({
      ...
      cb(r, o) {
        if (!n) return e.undefined;
        let l = a(this);
        if (l.widgetId !== t) return e.undefined;
        let d = l.clone();
  ...
  checkInternalOnlyNode(node) {
    ...
    while (currentNode) {
      if (currentNode.isInternalOnlyNode) return true;
  ...
  getTopLevelFrame({
    ...
    i({
      ...
      cb() {
        if (r !== _$$nT.Design && r !== _$$nT.Illustration) throw new Error('getTopLevelFrame is only available in Figma Design');
        let i = n(this);
      ...
      options: {
        ...
        get() {
          try {
            ...
            if (t instanceof ApplicationError) return e.newBoolean(!0);
            throw t;
          }
        }
      },
      ...
      cb(i) {
        let s = (n, r) => {
          ...
          for (let n = 0; n < l; n++) {
            ...
            if (r && d.isInternalOnlyNode || isInImmutableContext(e, d)) continue;
            let c = t().createNode(d.guid, 'node.findOne');
            let u = e.callFunction(i, e.$$null, c);
            if (u.type === 'FAILURE') throw new Error(`"findOne" callback crashed: ${u.error.message}`);
            if (e.toBoolean(u.handle)) return c;
            let p = s(d, !1);
    ...
    i({
      ...
      cb(i) {
        ...
        for (let n = 0; n < d; n++) {
          ...
          if (o && a.isInternalOnlyNode) continue;
          let s = t().createNode(a.guid, 'node.findChild');
          let c = e.callFunction(i, e.$$null, s);
          if (c.type === 'FAILURE') throw new Error(`"findChild" callback crashed: ${c.error.message}`);
          if (e.toBoolean(c.handle)) return s;
        }
        return e.$$null;
      },
      isAllowedInReadOnly: !0,
  ...
  loadAsync({
    ...
    defineVmFunction({
      ...
      cb() {
        const node = getNode(this);
        if (node.type !== 'CANVAS') throw new Error('Cannot call loadAsync on a non-page node.');
        const {
          promise,
          resolve,
          reject
  ...
  appendChild({
    ...
    defineVmFunction({
      ...
      cb(childHandle) {
        const node = getNode(this);
        if (node.isOrInResponsiveSet && !enableResponsiveSetHierarchyMutations) throw new Error('Cannot modify children of nodes in a webpage.');
        documentAccessState.checkAllowedPage(node, {
          method: 'appendChild'
        });
      ...
      cb(indexHandle, childHandle) {
        ...
        if (vm.getStringProp(this, 'id') === '0:0') {
          ...
          for (let i = 0; i < len && i < index; i++) {
            if (getNodeById(children[len - i - 1], sceneGraph).isInternalOnlyNode) index++;
          }
        }
  ...
  constraints({
    ...
    t({
      ...
      options: {
        ...
        get() {
          let t = n(this);
          if (hasResizeToFit(t) || t.type === 'BOOLEAN_OPERATION') return e.undefined;
          let i = e.deepWrap({
    ...
    defineVmProp({
      ...
      options: {
        ...
        set(value) {
          ...
          if (node.stackWrap === 'WRAP' && mode !== 'HORIZONTAL') node.stackWrap = 'NO_WRAP';
          if (mode === 'GRID') {
  ...
  rotation({
    ...
    t({
      ...
      options: {
        ...
        set(t) {
          let n = i(this);
          if (n.type === 'SLIDE') return e.undefined;
          let r = _$$u({
            vm: e,
            handle: t,
            zSchema: _$$N.Float,
    ...
    t({
      ...
      options: {
        ...
        get() {
          ...
          if (!t || t.status === zIx.NONE) return e.$$null;
          let n = e.deepWrap({
        ...
        set(t) {
          ...
          if (a?.type === convertBuildStatusToString(zIx.BUILD)) {
            ...
            if (!t || t.status === zIx.NONE) return e.undefined;
            setNodeStatus({
              nodeIds: [n.guid],
              status: zIx.NONE,
              sourceForLogging: 'plugin',
  ...
  cornerRadius({
    ...
    t({
      ...
      options: {
        ...
        get() {
          let t = n(this);
          if (hasResizeToFit(t)) return e.undefined;
        ...
        set(t) {
          ...
          if (i.type === 'SLIDE') throw new Error('Cannot set cornerRadius on slide nodes');
          let r = _$$u({
            vm: e,
            handle: t,
            zSchema: _$$N.PositiveFloat,
    ...
    t({
      ...
      options: {
        ...
        set(t) {
          ...
          if (n.type === 'SLIDE') throw new Error('Cannot set cornerSmoothing on slide nodes');
  ...
  widgetHoverStyle({
    ...
    if (!(vm instanceof ScopedNoOpVm)) return;
    defineVmProp({
      handle,
  ...
  strokeWeight({
    ...
    defineVmProp({
      ...
      options: {
        ...
        get() {
          const node = getNode(this);
          if (!node.borderStrokeWeightsIndependent) return vm.newNumber(node.strokeWeight);
          const {
            borderTopWeight,
            borderBottomWeight,
            borderLeftWeight,
    ...
    t({
      ...
      options: {
        ...
        set(t) {
          ...
          if (n.length > NfO.getMaxDashPattern()) throw new Error('A maximum of 8 dash patterns is supported');
  ...
  swapComponent({
    ...
    t({
      ...
      cb(t) {
        let n = i(t);
        if (n.type !== 'SYMBOL') throw new Error('Cannot swap to non-component node');
  ...
  detachInstance({
    ...
    defineVmFunction({
      ...
      cb() {
        const node = getNode(this);
        if (node.type !== 'INSTANCE') throw new Error('Cannot detach a non-instance');
  ...
  detachedInfo({
    ...
    defineVmProp({
      ...
      options: {
        ...
        get() {
          const node = getNode(this);
          if (node.type === 'INSTANCE' || node.type === 'SYMBOL') return vm.$$null;
          const info = node.detachedInfo;
          if (!info) return vm.$$null;
  ...
  cellAt({
    ...
    t({
      ...
      cb(t, r) {
        let a = n(this);
        if (a.type !== 'TABLE') throw new Error('Can only get cells from a table node');
        let s = _$$u({
          vm: e,
          handle: t,
        ...
        let l = a.cellAt(s, o);
        if (l) return i().createNode(l, 'node.cellAt');
  ...
  insertRow({
    ...
    t({
      ...
      cb(t) {
        let n = i(this);
        if (n.type !== 'TABLE') throw new Error('Can only get cells from a table node');
        let r = _$$u({
          vm: e,
          handle: t,
  ...
  moveRow({
    ...
    t({
      ...
      cb(t, n) {
        let r = i(this);
        if (r.type !== 'TABLE') throw new Error('Can only get cells from a table node');
        let a = _$$u({
          vm: e,
          handle: t,
  ...
  moveColumn({
    ...
    defineVmFunction({
      ...
      cb(fromHandle, toHandle) {
        const node = getNode(this);
        if (node.type !== 'TABLE') throw new Error('Can only get cells from a table node');
        const fromIndex = _$$u({
          vm,
          handle: fromHandle,
  ...
  deleteCharacters({
    ...
    t({
      ...
      cb(t, n) {
        ...
        if (r > a) throw new Error('deleteCharacters must have (start <= end)');
  ...
  flowStartingPoints({
    ...
    t({
      ...
      options: {
        ...
        get() {
          let t = i(this);
          if (t.type !== 'CANVAS') return e.$$null;
  ...
  selection({
    ...
    defineVmProp({
      ...
      options: {
        ...
        set(val) {
          if (!vm.isArray(val)) throw new Error('The selection must be an array');
      ...
      options: {
        ...
        set(val) {
          ...
          if (vm.isNull(val)) {
            ...
            if (t.length === 1 && t[0].type === 'TEXT') Y5.triggerAction('leave-edit-mode');
            return vm.undefined;
          }
          const r = _$$u({
            vm,
          ...
          if (!r) return;
          if (r.end < r.start) throw new Error('selectedTextRange must have (start <= end)');
      ...
      options: {
        ...
        get() {
          ...
          const wrapped = vm.deepWrap(settings.map(setting => {
            ...
            if (setting.colorProfile) result.colorProfile = setting.colorProfile;
            if (type === 'TEXT') result.useAbsoluteBounds = !!setting.useAbsoluteBounds;
    ...
    defineVmFunction({
      ...
      cb(fieldHandle, variableHandle) {
        ...
        const getVariableId = ({
          ...
          if (vm.isNull(handle) || vm.isUndefined(handle)) return null;
  ...
  pluginRelaunchData({
    ...
    t({
      ...
      cb(t) {
        if (n) throw new Error('Cannot set relaunch data in a widget');
  ...
  textRangeFunctions({
    ...
    defineVmFunction({
      ...
      cb(start, end) {
        ...
        if (val === 'mixed') return mixedSentinel;
    ...
    defineVmFunction({
      ...
      cb(fieldsHandle, start, end) {
        ...
        if (fields.includes('textCase')) fields.push('fontVariantCaps');
        ...
        if (fields.includes('textDecorationColor')) fields.push('textDecorationFills');
        let segments;
        ...
        for (const [idx, seg] of segments.entries()) {
          ...
          if (seg.fontSize !== undefined) vm.setProp(obj, 'fontSize', vm.newNumber(seg.fontSize));
          ...
          if (seg.fontWeight !== undefined) vm.setProp(obj, 'fontWeight', vm.newNumber(seg.fontWeight));
          if (seg.textDecoration !== undefined) vm.setProp(obj, 'textDecoration', vm.newString(seg.textDecoration));
          ...
          if (seg.lineHeight !== undefined) vm.setProp(obj, 'lineHeight', vm.deepWrap(iC(seg.lineHeight)));
          ...
          if (seg.indentation !== undefined) vm.setProp(obj, 'indentation', vm.newNumber(seg.indentation));
          ...
          if (seg.listSpacing !== undefined) vm.setProp(obj, 'listSpacing', vm.newNumber(seg.listSpacing));
  ...
  variantProperties({
    ...
    t({
      ...
      options: {
        ...
        get() {
          let t = i(this);
          if (!t || t.type !== 'INSTANCE' && !t.isState) return e.$$null;
  ...
  componentPropertyDefinitions({
    ...
    i({
      ...
      cb(t, i, r, a) {
        ...
        s = d === 'VARIANT' ? o.addVariantComponentPropertyDefinition(l, u) : o.addComponentPropertyDefinition(l, function (e) {
          switch (e) {
            case 'BOOLEAN':
              return 'BOOL';
            case 'TEXT':
      ...
      options: {
        ...
        get() {
          ...
          for (const o of overrides) {
            ...
            if (idx >= 0) o.overriddenFields[idx] = 'stokeTopWeight';
          }
      ...
      defineVmProp({
        ...
        options: {
          ...
          set(value) {
            ...
            if (!sH(endpointValue.endpointNodeId)) throw new Error('Invalid endpointNodeId');
  ...
  measurements({
    ...
    const mapMeasurementToVm = (measurement, metricsKey) => {
      ...
      const toSide = measurement.toSameSide ? fromSide : (() => {
        switch (fromSide) {
          case 'TOP':
            return 'BOTTOM';
          case 'BOTTOM':
    ...
    const findStartNode = (id, pageGuid) => {
      ...
      if (!measurement) throw new Error('Couldn\'t find measurement on the current page');
    ...
    defineVmFunction({
      ...
      cb(target) {
        ...
        if (targetNode.containingCanvas !== node.guid) throw new Error('Node is not on the current page');
    ...
    if (editorType === _$$nT.DevHandoff || editorType === _$$nT.Design || editorType === _$$nT.Illustration) {
      ...
      defineVmFunction({
        ...
        cb(start, end, options) {
          if (getFeatureFlags().plugins_annotations_seat_check && !fb()) throw new Error('A Full seat is required to add measurements');
          const node = getNode(this);
          const startObj = _$$u({
            vm,
            handle: start,
          ...
          if (startNode.containingCanvas !== node.guid) throw new Error('Start node is not on the current page');
          if (endNode.containingCanvas !== node.guid) throw new Error('End node is not on the current page');
          ...
          if (startNode.guid === endNode.guid && startObj.side === endObj.side) throw new Error('Measurement can\'t point to itself');
      ...
      defineVmFunction({
        ...
        cb(id, newValue) {
          if (getFeatureFlags().plugins_annotations_seat_check && !fb()) throw new Error('A Full seat is required to edit measurements');
          const node = getNode(this);
          ...
          if (value.freeText !== undefined) update.freeText = value.freeText;
      ...
      defineVmFunction({
        ...
        cb(id) {
          if (getFeatureFlags().plugins_annotations_seat_check && !fb()) throw new Error('A Full seat is required to delete measurements');
          const node = getNode(this);
          const measurementId = _$$u({
            vm,
            handle: id,
      ...
      options: {
        ...
        get() {
          ...
          if (r) {
            ...
            if (!n.stackMode || n.stackMode === 'NONE') return e.$$null;
            t = e.deepWrap({
    ...
    t({
      ...
      options: {
        ...
        set(t) {
          let n = i(this);
          let r = i(t);
          if (r.type !== 'SLIDE') throw new Error('focusedSlide must be a Slide');
