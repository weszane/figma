...
class MultiplayerRoundTripReporter {
  ...
  private report = debounce(() => {
    if (this.roundTripTimes.length === 0) return;
...
class RenderingTracer {
  ...
  saveRenderingTrace(): void {
    if (renderingTraceHtml.length === 0) return;
...
class FullscreenHandler {
  ...
  reportQuantizedColorEqualsUse(r1: number, g1: number, b1: number, a1: number, r2: number, g2: number, b2: number, a2: number): void {
    const data = {
      ...splitBufferIntoRawStacks(new Error().stack),
  ...
  startPerfTimer(event: string): void {
    if (timerEventNames.has(event)) reactTimerGroup.start(event);
    if (gpuFullscreenEventNames.has(event)) fullscreenPerfManager.startFs(event);
  }
  stopPerfTimer(event: string): void {
    if (timerEventNames.has(event)) reactTimerGroup.stop(event);
    if (gpuFullscreenEventNames.has(event)) fullscreenPerfManager.stopFs(event);
  }
  startOpsTimer(name: string, key: string): void {
    globalPerfTimer.start(name, {
      key
    });
  }
