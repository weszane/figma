...
import { templateSourceAtom, TemplateSourceType } from "../figma_app/755939";
...
export async function loadAndOpenFileInFullscreen(store: any, fileKey: string, editorType: any, shouldUseOfflineMode: boolean): Promise<void> {
  ...
  const fileMetadataResponse = (await (shouldUseOfflineMode ? (async () => {
...
async function handleNewFileCreation(store: any, newFileParams: any, fileCreationController: any, shouldHideUI: boolean, autosaveManager: any): Promise<{
  ...
  if (newFileParams.editorType === "cooper" && templateSourceAtom && TemplateSourceType) {
    ...
    newFileParams.cooperTemplateLibraryKey ? atomStoreManager.set(templateSourceAtom, {
      type: TemplateSourceType.TEMPLATES,
      libraryKey: newFileParams.cooperTemplateLibraryKey,
      parentView: {
        type: TemplateSourceType.ALL
      }
    }) : orgTemplateView && atomStoreManager.set(templateSourceAtom, {
