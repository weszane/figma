...
export const ParentSubpathItem = memo<ParentSubpathItemProps>(({
  ...
  const { setKeyboardNavigationElement } = useKeyboardNavigationItemForParentSubpath(!visible)
  ...
  const { listItemHeight } = useContext({} as any) // DrilldownContext would be passed here
  ...
  return jsxs(ButtonPrimitive, {
    className: g()(
      'drilldown_item--parentSubpathContainer--wbu7T drilldown_item--_drilldownItemBase--YOfRd text--fontPos11--2LvXf text--_fontBase--QdLsd',
      Qx
    ),
    style: {
    ...
    htmlAttributes: {
      tabIndex,
      'onKeyDown': shouldForwardKeyDownToFullscreen ? (e: any) => jr(e.nativeEvent) : undefined,
