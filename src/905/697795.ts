...
export function $$M2(e, t) {
  ...
  if (t.view === 'fullscreen' && t.tryPluginName) {
    e.openFile
      ? L(e.openFile.name, i)
      : L(getI18nString('community.try.plugin_name_with_community', {
  ...
  if (t.view === 'prototype') {
    let n
    let r = ''
    if (isSlidesFile(t.file.editor_type)) {
      n = r = `${t.isPresenterView ? getI18nString('slides.general.presenter') : getI18nString('slides.general.slide_deck')} \xB7 ${i} - ${getI18nString('slides.general.slides')}`
    }
    else {
      let t = (function (e) {
        let t = e.prototype
        for (let e of t.pages) {
          if (e.nodeId === t.currentPageId)
            return e.name
        }
        return ''
      }(e))
  ...
  if (t.view === 'fullscreen' && !e.openFile) {
    let e = GT()
...
export let $$j1 = createOptimistThunk(e => customHistory.listen((t) => {
  t === 'POP' && (function (t) {
    if (t) {
      ...
      if (n.view === 'fullscreen' && !fullscreenValue.isReady()) {
        customHistory.reload('User navigated before fullscreen fully initialized')
        return
      }
      if (s.view === 'fullscreen' && !s.fileKey) {
        window.history.back()
        return
      }
      if (n.view === 'fullscreen' && (s.view !== 'fullscreen' || n.fileKey !== s.fileKey)) {
        e.dispatch(VB({
          oldSelectedView: n,
          newSelectedView: s,
        }))
        ...
        if (e.dispatch(hideModal()), s.view === 'communityHub' && s.subView === 'searchAndBrowse' && OE(s.data)) {
          let t = getCurrentSearchSessionId(i)
          t && e.dispatch(searchRestoreSessionAction({
            sessionId: t,
          }))
        }
  ...
  hasViewProperty(customHistory.location.state) || (function () {
    ...
    i !== null && e.dispatch(selectViewAction({
      ...i,
      fromPopstate: !0,
    }))
  }())
