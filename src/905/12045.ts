...
export function getPluginParameterEntry(event: any) {
  if (event.type !== 'run-installed-plugin' && event.type !== 'run-local-plugin' || !event.parameterEntry || event.parameterEntry.parameters.length === 0) {
    return null;
  }

  // Map parameters to expected format
  const parameters = event.parameterEntry.parameters.map(({
    valueType,
    ...rest
  }) => ({
    ...rest,
...
export function runPluginParameterEntry(event: any, triggeredFrom: string, showDropdown: (action: any) => void) {
  ...
  showDropdown(showDropdownThunk({
