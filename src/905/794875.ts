...
class et extends RecordingPureComponent {
  constructor(e) {
    ...
    this.resizeDropdown = () => {
      let e
      if (this.hiddenInput && this.props.preventHiddenInput == null && this.hiddenInput.focus(), this.container && (this.containerRect = this.container.getBoundingClientRect()), !this.containerRect)
        return
      ...
      if (this.props.neverConstrain || BrowserInfo.windows) {
        ...
        if (this.props.targetDomNode && this.props.targetDomNode !== document.body)
          throw new Error("If you are parenting your <Select /> into a node that is NOT document.body via the targetDomNode prop, please ensure neverConstrain is also set to true. There are known issues with scrolling positioning with a specified targetDomNode.")
        if (this.shouldConstrainTop = this.containerRect.top + i < d, this.shouldConstrainTop) {
          ...
          for (let t = s.length - 2; t >= 0; t--) {
            ...
            if (Math.abs(r) <= 8) {
              if (s[t].type.__CLASS_NAME__ === "SelectDivider" || t === this.optionIndexByFormattedValue[o])
                break
              let e = r
        ...
        if (this.shouldConstrainBottom = l() > u, this.shouldConstrainBottom) {
          ...
          for (let i = 1; i < t.length; i++) {
            ...
            if (Math.abs(r - n) <= 8) {
              if (t[i - 1].type.__CLASS_NAME__ === "SelectDivider" || i === this.optionIndexByFormattedValue[o])
                break
              n += r - n
    ...
    this.updatePreview = debounce((e) => {
      if (!this.props.enablePreview || !this.isDropdownShown(this.props))
        return
      ...
      if (e && this.props.optionsWithDisabledPreviews?.includes(e))
        return
      let n = void 0 === this.state.previousPreviewValue
      if (n) {
        if (this.props.property === e)
          return
        this.setState({
    ...
    this.onScroll = (e, t, i) => {
      ...
      if (void 0 === dropdownScrollTop || void 0 === dropdownHeight)
        return
    ...
    this.onKeyDown = hookForKeyboard(this, "keydown", (e) => {
      ...
      if (this.dropdownShown()) {
        ...
        switch (e.keyCode) {
          ...
          {
            let t
            if (this.props.disableSelectFocus && e.metaKey)
              break
            let a = i ?? -1
            let s = Children.toArray(this.props.children)
        ...
        if (this.props.isInFPLGridCell && e.code === "Space" && e.shiftKey)
          return;
        (this.props.openOnKeyPressed ? this.props.openOnKeyPressed.includes(e.keyCode) : J.includes(e.keyCode))
          ? (e.preventDefault(), e.stopPropagation(), this.showDropdown(), this.setState({
              allowSelection: !0,
            }))
          : this.addInputChar(e.keyCode)
      }
  ...
  componentDidUpdate(e, t) {
    if (super.componentDidUpdate(this.props, this.state), this.props.triggerDropdown && (this.showDropdown(), this.setState({
      ...
      if (this.state.currentDropdownAccepted
        ? this.setState({
            currentDropdownAccepted: !1,
          })
  ...
  positionForFormattedValue(e) {
    let t = 0
    let i = 0
    if (this.props.neverConstrain && (i = this.state.topPositionOffsetForNeverConstrain || 0), this.props.openBelowTarget)
      return -i
  ...
  scrollValueOverInput(e) {
    if (!this.dropdownEl || !this.containerRect || void 0 === this.state.dropdownScreenTop)
      return
  ...
  scrollValueIntoView(e) {
    if (!this.dropdownEl || void 0 === this.state.dropdownHeight || this.dropdownContentHeight <= this.state.dropdownHeight)
      return
...
export class $$en1 extends RecordingPureComponent {
  ...
  render() {
    ...
    return jsxs("div", {
      ...
      }), this.props.children || !this.props.truncateInMiddle
        ? jsx("span", {
