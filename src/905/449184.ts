...
class h {
  ...
  userAgeInDays() {
    ...
    if (e)
      return ~~((new Date().getTime() - new Date(e).getTime()) / 864e5)
  }
...
class x {
  ...
  constructor(e) {
    ...
    if (!this._analytics)
      throw new Error('Unable to use Segment, "analytics" is undefined.')
    this.identifyUser(getInitialOptions().user_data)
    this._analytics.user && (window[y] = this._analytics.user().anonymousId())
  }

  get userId() {
    if (this._analytics.user)
      return this._analytics.user().id() || void 0
  }
...
class S {
  constructor(e) {
    ...
    !(function () {
      ...
      ? this._anonymousId = e.user().anonymousId()
      : this._anonymousId = window.parent[y]
  }
...
class j {
  ...
  eventTracked(e) {
    try {
      if (!getFeatureFlags().figment_rate_limit_events)
        return !0
...
class V {
  ...
  _makeRequest({
    ...
    let r = null
    if (e === 'monitor' && getFeatureFlags().figment_debugger && (r = this._prepareEventVisualizerPayload(t), trackFigmentRequest(r, NetworkState.PENDING)), this._disableTracking)
    ...
    let a = {}
    getFeatureFlags().internal_only_debug_tools
      ? a = {
...
class AnalyticsEventManager {
  ...
  shouldSendDefinedEvent = (eventName, def) => {
    if (def.featureFlag && !getFeatureFlags()[def.featureFlag])
      return false
    if (!def.throttle)
      return true
