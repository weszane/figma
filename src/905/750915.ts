...
function PrePurchaseUserSelectorContent({
  ...
  const authedActiveCommunityProfile = useSelector<ObjectOf>(state => state.authedActiveCommunityProfile);
  ...
  const associatedProfiles = useMemo(() => getAssociatedUserProfiles({
  ...
  const {
    usersCanPurchase,
    publishers
  } = useMemo(() => partitionUsersByPurchaseEligibility(associatedProfiles, resource), [associatedProfiles, resource]);

  // Build options for users who can purchase
  let options = usersCanPurchase.map(user => {
    const hasActiveSubscription = purchaseMeta[user.id] && isSubscriptionActive(purchaseMeta[user.id]);
    return {
      id: user.id,
      title: jsxs('div', {
        className: workspaceTitleWrapper,
        children: [user.name, !isOrgOrTeam(authedActiveCommunityProfile) && hasActiveSubscription && jsx('div', {
      ...
      onClick: () => {
        if (!hasActiveSubscription) onUserSelect(user);
      },
  ...
  options = options.concat(publishers.map(publisher => ({
    id: publisher.id,
 ...
 * Original export: $$T0
 */
export const PrePurchaseUserSelectorModal = registerModal((props: {
  onUserSelect: (user: any) => void;
  resource: any;
}) => {
   ...
   */
  const ModalContent = useMemo(() => function ModalContentInner({
    onUserSelect,
    resource
  }: {
    onUserSelect: (user: any) => void;
    ...
    return jsx(_$$A2, {
      ...
      children: jsxs('div', {
        ...
        children: [jsx(PrePurchaseUserSelectorContent, {
          onUserSelect,
          resource
        }), jsx('button', {
