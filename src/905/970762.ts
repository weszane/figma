...
export function A(t, i, p, m) {
  ...
  for (let _ of i.parsedDefs) {
      ...
      if (typeof t == 'object') {
        ...
        switch (i.valueType) {
          case 'BOOLEAN':
            ...
            return
          case 'STRING':
            if (typeof n == 'string' || typeof n == 'object' && n?.type === 'JSXExpressionContainer')
              break
            return
        }
        return n
    ...
    if (_.devFriendlyProp.type === 'ARRAY' || _.def.type === 'GROUPED_INSTANCE_SWAP' || _.def.type === 'NESTED_INSTANCE' && _.isOptional) {
      for (let e of wn(i.componentId, _, m)) h.nestedInstancesVisibility[e] = !y
    }
    if (!y) {
      switch (_.def.type) {
