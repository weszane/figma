...
async function ea(e, t, i) {
  ...
  if (t.dispatch(setNeedsUpgrade({
    ...
    let e = function (e, t) {
      for (let i of [t, e]) {
        for (let n in i) {
          if (!!e[n] != !!t[n]) return `${n} changed to ${!!t[n]}`;
        }
      }
      return null;
    }(l, c);
  ...
  if (selectedView.view !== 'fullscreen') throw new Error(er);
...
let $$ej0 = createOptimistThunk(async (e, {
  ...
  if (!n) return;
  let a = function (e, t) {
    if (!isEditorTypeEnabled(e.editorType) || e.mode && e.mode !== 'auto') return e;
    ...
    if (!canAccessFullDevMode(n) && a && i === FEditorType.DevHandoff) {
      ...
      if (i.view !== 'fullscreen') throw new Error('Redirect new editor type failed.');
      return i;
    }
    ...
    if (i !== e.editorType) {
      ...
      if (n.view !== 'fullscreen') throw new Error('Redirect new editor type failed.');
      return n;
    }
    return e;
  }(t, e);
  ...
  if (!u) throw new Error('Could not get file from liveStore');
...
let eH = createOptimistThunk(async (e, {
  ...
  try {
    ...
    desktopAPIInstance ? desktopAPIInstance.reportFatalError(t.key, i === 0 ? {
      type: 'load',
      code: -106,
