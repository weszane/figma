...
async function ea(e, t, i) {
  ...
  if ("fullscreen" !== selectedView.view) throw Error(er);
  if (null != selectedView.commentThreadId || selectedView.showCommentPreferencesPicker) {
...
let $$ej0 = createOptimistThunk(async (e, {
  ...
  let a = function (e, t) {
    ...
    if (i !== e.editorType) {
      ...
      if ("fullscreen" !== n.view) throw Error("Redirect new editor type failed.");
      return n;
    }
    return e;
  }(t, e);
  ...
  if (!u) throw Error("Could not get file from liveStore");
...
let eH = createOptimistThunk(async (e, {
  ...
  try {
    ...
    try {
      ...
      let e = Error(`Calling "/api/file_metadata/:file_key" failed with status ${t && t.status}`);
      e.cause = t;
      return e;
    }
    await ea(r, e, c);
