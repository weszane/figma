 ...
 */
async function handleFileMetadataResponse(response: any, store: any, editorType: FEditorType): Promise<void> {
  const state = store.getState();
  const metadata = response.data.meta;

  // Validate metadata presence
  if (!metadata) {
    reportError(ServiceCategories.FRONTEND_PLATFORM, new Error('We are expecting FileMetadata but got undefined'), errorContext => errorContext.setExtra('response', response));
    return;
  }
  ...
  if (serverFeatureFlags && (currentFeatureFlags.reload_when_feature_flags_change || serverFeatureFlags.reload_when_feature_flags_change)) {
  ...
  if (selectedView.showInspectPanel) {
    ...
    setPropertiesPanelTab(metadata.can_edit ? DesignWorkspace.DESIGN : DesignWorkspace.COMMENT);
  }
  ...
  if (selectedView.compareLatest) {
    store.dispatch(maybeShowVersionDiffNotification({
      openFileKey: metadata.file_key
    }));
  }

  // Handle Google Classroom permissions modal
  if (selectedView.showPermissionsModalFromGoogleClassroomIntegration && selectedView.fileKey) {
...
registerLegacyModal(LICENSE_AGREEMENT_MODAL_TYPE, props => jsx(LicenseAgreementModal, {
  ...props,
...
class LicenseAgreementModal extends PureComponent<{
  ...
  private accept = () => {
    ...
    if (selectedView.landingState === 'DUPLICATE' && this.props.user && selectedFile != null) {
  ...
  render() {
    return jsxs('div', {
      children: [jsx('p', {
        ...
        children: 'This License constitutes the entire agreement between you and Apple relating to the use of the Apple UI Design Resources licensed hereunder and supersedes all prior or contemporaneous understandings regarding such subject matter. No amendment to or modification of this License will be binding unless in writing and signed by Apple. The parties hereto confirm that they have requested that this Agreement and all related documents be drafted in English. Les parties ont exigé que le présent contrat et tous les documents connexes soient rédigés en anglais.'
      }), jsx('p', {
        children: 'EA1441'
      }), jsx('p', {
        children: '12/02/2016'
 ...
 */
export const processFileView = createOptimistThunk(async (store, {
  newSelectedView
}: ProcessFileViewParams, {
  liveStore
}) => {
...
const initializeFileForFullscreen = createOptimistThunk(async (store, params) => {
  ...
  try {
    ...
    if (BrowserInfo.isIpadNative && isASTCCompressionSupported() && statsigClient.getFeatureGate('figjam_ipad_compressed_textures')) {
    ...
    try {
      metadataResponse = pendingMetadataPromise && pendingFileKey === file.key ? await pendingMetadataPromise : await fetchFileMetadata(file.key);
    ...
    trackDefinedFileEvent('scenegraph_and_sync.fullscreen_open_error', file.key, state, {
      error: error instanceof Error ? error.cause ? `${error.message}: ${error.cause.message}` : error.message : JSON.stringify(error),
 ...
 */
export async function executeTeamUpgrade(teamData: any, folderData: any, userData: any, store: any, file: any) {
