...
import { isAndroidOrIphoneNotFigmaMobile, isFigmaMirrorAndroid, isInFigmaMobile } from '../figma_app/778880'
import { TrackingProvider, withTrackedClick } from '../figma_app/831799'
import { LargeLoadingSpinner, LoadingOverlay } from '../figma_app/858013'
...
function v(e) {
  ...
  let i = useRef(null)
  let a = useCallback(() => {
    if (i.current && !e.noAutofocus) {
      for (let e = 0; e < i.current.elements.length; e++) {
  ...
  let s = useCallback(() => {
    if (i.current) {
      for (let t = 0; t < i.current.elements.length; t++) {
  ...
  return jsx('form', {
    ...
    'children': e.isLoading
      ? jsx(LoadingOverlay, {})
      : jsxs(Fragment, {
          children: [e.header || e.subtitle || e.auth.hint || !e.hideError && e.auth.error
            ? jsx(Fragment, {
                children: jsxs('div', {
...
function k(e) {
  ...
  let s = y()('brand_buttons--brandButton--eCsuS auth_view--brandButton--Le4wb auth_view--brandButtonBase--RAO-j auth_brand--figmaSans--aXdNw', {
    ...
    'brand_buttons--brandButtonHoverIconEffects--N6V8n': e.use !== 'hollow' && !loading && !e.disabled,
    'brand_buttons--brandButtonLoadingIconEffects--BxiQD': e.use !== 'hollow' && loading,
    'brand_buttons--hollowButton--tvLh4 auth_view--brandButton--Le4wb auth_view--brandButtonBase--RAO-j auth_brand--figmaSans--aXdNw': e.use === 'hollow',
  ...
  return jsx('button', {
    ...
    children: jsxs('span', {
      className: y()('brand_buttons--brandButtonContent--HPB3u', e.use !== 'hollow' && 'brand_buttons--standardButtonContent--5k3-s'),
      children: [e.use === 'hollow' || !loading && e.disabled
        ? null
        : loading
          ? jsx(SvgComponent, {
              svg: _$$A3,
...
function j(e) {
  ...
  let E = async () => {
    e.auth.postVerificationAction === null ? await x() : t(e.auth.postVerificationAction)
    o(!1)
    w()
  }
  let x = async () => {
    ...
    onSubmit: () => {
      ...
      if (n) {
        W8(n, {
          ...
          i.type === 'login' && (t(AUTH_SET_REDIRECT_URL({
    ...
    children: [jsx(gK, {
      ...
      children: [e.auth.smsOkay && !i && jsxs(Fragment, {
        ...
        onClick: () => {
          ...
          e.auth.ssoMethod === AuthProvider.SAML
            ? t(changeAuthFormState({
...
function Q(e) {
  ...
  if (!c || !d || !u)
    return null
  let m = e?.auth.origin === 'start_using_plugins_button_click'
    ? jsx('div', {
        className: X,
...
function eb(e) {
  ...
  let g = (t) => {
    ...
    i = appAuthDesktopProtocol === 'vscode' ? 'vscode://figma.figma-vscode-extension/app_auth_redeem' : appAuthDesktopProtocol === 'vscode-insiders' ? 'vscode-insiders://figma.figma-vscode-extension/app_auth_redeem' : appAuthDesktopProtocol === 'vscode-cursor' ? 'cursor://figma.figma-vscode-extension/app_auth_redeem' : h && e.auth.appAuthMsftTeamsUrl ? e.auth.appAuthMsftTeamsUrl : `${e.auth.appAuthDesktopProtocol || 'figma'}://app_auth/redeem`
  ...
  i = appAuthAppType === ClientPlatform.MOBILE || appAuthAppType === ClientPlatform.FIGJAM_MOBILE
    ? getI18nString('auth.continue-with-the-specific-app', {
        appDescriptor: t,
      })
    : appAuthAppType === ClientPlatform.VSCODE || appAuthAppType === ClientPlatform.VSCODE_INSIDERS || appAuthAppType === ClientPlatform.VSCODE_CURSOR
      ? getI18nString('auth.open-figma-for-vs-code')
      : getI18nString('auth.open-specific-app', {
          appDescriptor: t,
        })
  s = appAuthAppType === ClientPlatform.VSCODE || appAuthAppType === ClientPlatform.VSCODE_INSIDERS || appAuthAppType === ClientPlatform.VSCODE_CURSOR
    ? getI18nString('auth.choose-account-to-add-to-figma-for-vs-code')
    : getI18nString('auth.choose-account-to-add-to-specific-app', {
        appDescriptor: t,
      })
  ...
  let C = w
    ...
    : null
  if (!c && e.auth.appAuthId && e.auth.appAuthUsers) {
    o = w
      ? jsxs(Fragment, {
          children: [jsx('h1', {
            className: ec,
    ...
    let e
    let i
    e = appAuthAppType === ClientPlatform.MOBILE || appAuthAppType === ClientPlatform.FIGJAM_MOBILE
      ? getI18nString('auth.generic-go-back-and-try-again')
      : appAuthAppType === ClientPlatform.VSCODE || appAuthAppType === ClientPlatform.VSCODE_INSIDERS || appAuthAppType === ClientPlatform.VSCODE_CURSOR
        ? getI18nString('auth.open-figma-for-vs-code')
        : getI18nString('auth.open-specific-app', {
            appDescriptor: t,
          })
    i = appAuthAppType === ClientPlatform.VSCODE || appAuthAppType === ClientPlatform.VSCODE_INSIDERS || appAuthAppType === ClientPlatform.VSCODE_CURSOR
      ? getI18nString('auth.try-again-in-figma-for-vs-code')
      : getI18nString('auth.try-again-on-specific-app', {
          appDescriptor: t,
        })
    o = jsxs(Fragment, {
      children: [jsx(_$$T, {
...
function ev(e) {
  ...
  let r = () => {
    ...
    XHR.post('/api/session/app_auth/redeem', {
      ...
      desktopAPIInstance
        ? desktopAPIInstance.finishAppAuth(e.auth.redirectUrl)
        : setTimeout(() => {
            let t = e.auth.redirectUrl || (isFigmaMirrorAndroid() || isInFigmaMobile() ? '/mobile-app' : null)
            t != null ? (console.log(`Opening: ${t}`), customHistory.redirect(t)) : customHistory.reload('AppAuthRedeemForm')
          }, 500)
    }).catch((e) => {
      console.error(e)
      i()
...
function ex({
  ...
  let p = c
    ? o
      ? renderI18nText('auth.account-picker-join-text', {
...
function eB(e) {
  ...
  return e.onForgotPassword || e.onSwitchToSaml || e.onChangeFormClick
    ? jsxs('div', {
        className: ew,
...
function ez(e) {
  ...
  useEffect(() => {
    auth.loading || auth.errorType !== AuthErrorCode.UNAUTHORIZED || auth.googleIdToken == null || t(AUTH_SET_GOOGLE_ID_TOKEN({
      googleIdToken: null,
    }))
...
function eH(e) {
  ...
  return jsx(ez, {
    ...
    onGoogleAuth: () => {
      o(null)
      e.auth.shouldUseRedirectInstead
        ? Bs(e.auth.redirectUrl, e.auth.fromMsTeams, e.auth.origin)
        : Rm({
            dispatch: t,
            origin: e.auth.origin,
            tosAccepted: l,
          }).then((e) => {
            e.type === 'login' && t(AUTH_COMPLETE({
              userId: e.user.id,
            }))
          }, (e) => {
...
function eJ(e) {
  ...
  let d = e.auth.existingSession
    ? getI18nString('auth.sso-gate.back-with-session', {
        orgName: e.auth.orgName,
      })
  ...
  return jsxs(v, {
    ...
    children: [e.auth.ssoMethod === AuthProvider.GOOGLE && jsx(P, {
      ...
      onClick: () => {
        xI(t, e.auth.origin).then(e => e.type === 'login' && i(), (e) => {
          r(e.message)
        })
      },
        ...
        cta: jsx(LinkPrimitive, {
          ...
          onClick: () => {
            ...
            _$$k2.resendCode().then(() => {
              ...
              i.reason === 'invalid_session' && customHistory.redirect('/')
...
function te() {
  ...
  let v = async (i) => {
    ...
    try {
      ...
      if (t?.data?.i18n?.id) {
        ...
        e(FlashActions.error(getI18nString('auth.default-error')))
      }
      f(!1)
      return
    }
  ...
  return jsxs('div', {
    ...
    children: [jsx('div', {
      ...
      className: c,
      children: d
        ? renderI18nText('auth.magic_link_check_email.body_login_v2', {
            emailAddress: A,
          })
    ...
    let t = () => {
      UserAPIHandlers.getUnverified().then((n) => {
        if (n?.data?.meta?.email_validated_at) {
          ...
          e = setTimeout(t, 7e3 + 3e3 * Math.random())
        }
      })
    }
    t()
    let n = () => {
      document.visibilityState === 'hidden' && e && (clearTimeout(e), e = null)
      document.visibilityState === 'visible' && e === null && t()
    }
...
function tl(e) {
  ...
  let r = () => {
    ...
    switch (window.INITIAL_OPTIONS.cluster_name !== 'gov' || s === AuthFlowStep.VALIDATE_CODE || window.origin.startsWith('https://admin.') || (s = AuthFlowStep.SAML_START), s) {
      case AuthFlowStep.SIGN_IN:
        t(AUTH_SIGN_IN({
          formId: r,
        }))
  ...
  if (e.auth.formState === AuthFlowStep.ACCOUNT_PICKER && e.auth.accountPicker) {
    return jsx(ex, {
  ...
  if (window.INITIAL_OPTIONS.cluster_name === 'gov' && e.auth.formState !== AuthFlowStep.VALIDATE_CODE && !window.origin.startsWith('https://admin.')) {
    return jsx(ej, {
      onFormSubmit: r,
      ...e,
    })
  }
  if (e.auth.formState === AuthFlowStep.SIGN_IN) {
    return jsx(TrackingProvider, {
      name: 'Sign In',
      children: jsx(eH, {
        onFormSubmit: r,
  ...
  if (e.auth.formState === AuthFlowStep.SIGN_UP) {
    let t = new URLSearchParams(customHistory.location.search).get('with_community_header') === 'true'
    return jsx(TrackingProvider, {
      name: 'Sign Up',
      children: jsx(eQ, {
        onFormSubmit: r,
  ...
  if (e.auth.formState === AuthFlowStep.GOOGLE_SSO_SIGNUP_ACTION_REDIRECT) {
    return jsx(TrackingProvider, {
  ...
  if (e.auth.formState === AuthFlowStep.RESET_PASSWORD) {
    return jsx(en, {
      onFormSubmit: r,
      ...e,
    })
  }
  else if (e.auth.formState === AuthFlowStep.VALIDATE_CODE) {
    return jsx(e2, {
      onFormSubmit: r,
      ...e,
    })
  }
  else if (e.auth.formState === AuthFlowStep.SAML_START) {
    return jsx(ej, {
      onFormSubmit: r,
      ...e,
    })
  }
  else if (e.auth.formState === AuthFlowStep.EMAIL_ONLY) {
    return jsx(eP, {
      onFormSubmit: r,
      ...e,
    })
  }
  else if (e.auth.formState === AuthFlowStep.JOIN_ORG) {
    return jsx(Q, {
      onFormSubmit: r,
      ...e,
    })
  }
  else if (e.auth.formState === AuthFlowStep.VERIFY_HUMAN) {
    return jsx(j, {
      ...e,
    })
  }
  else if (e.auth.formState === AuthFlowStep.TWO_FACTOR) {
    return jsx(H, {
      onFormSubmit: r,
      ...e,
    })
