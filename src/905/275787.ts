...
export const LibraryItemTileContextMenu = memo(({
  ...
  const { isFlyoutOpen } = useComponentFlyoutModal()
  ...
  const shouldHideMenu = useMemo(() => {
    ...
    dropdownShownData?.type,
    selectedFile,
    isSameLibrary,
    hideForLocalComponents,
    defaultPublishedLibraryKeys,
    component,
    isLibraryFileLinkAvailable,
  ])
  ...
  const viewDetailsItem = useMemo(() => {
    ...
    return jsx(MenuItem, {
      ...
      onClick: () => onOpenFlyout(
        component,
        dropdownShownData?.data?.sectionPosition,
        dropdownShownData?.data?.sectionNameForTracking,
      ),
...
function TogglePublishableMenuItem({ localNodeId }: TogglePublishableMenuItemProps) {
  ...
  const toggleText = isPublishable
    ? renderI18nText('design_systems.publishing_modal.hide_when_publishing')
    : renderI18nText('design_systems.publishing_modal.show_when_publishing')
