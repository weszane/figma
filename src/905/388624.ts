 ...
 */
function createCommunityProfileOption(
  profile: any,
  user: any,
  isCommunityHubView: boolean,
  isActive: boolean,
  bellStates: any,
  view: any,
  dispatch: any,
  ...
  return {
    ...
    icon: jsxs('div', {
      ...
      children: [
        bellStates[profile.id] && jsx('div', { className: RED_DOT_CLASS }),
        jsx(renderAvatar, {
          userId: user.id,
    ...
    itemBadge: isOrgOrTeamExport(profile)
      ? jsx(Badge, {
...
function generateDropdownItems(
  authedUsers: any,
  authedProfilesById: any,
  permissionsState: any,
  user: any,
  orgById: any,
  authedTeamsById: any,
  authedActiveCommunityProfile: any,
  isCommunityHubView: boolean,
  hasOrgOrTeamProfiles: boolean,
  communityProfileBellStates: any,
  selectedView: any,
  dispatch: any,
) {
  ...
  authedUsers.orderedIds.forEach((userId: string) => {
    ...
    const planItems = authedUser.plans?.map((plan: any) => {
      ...
      return {
        ...
        callback: () => dispatch(switchAccountAndNavigate({ workspace: workspaceIdentity, view })),
        activeClassName: ACTIVE_OPTION_CLASS,
      }
    }) || []
    items = items.concat(planItems).concat([{ displayText: '', disabled: true, separator: true }])
  })
  ...
  const communityItems = (() => {
    ...
    Object.keys(authedUsers.byId).forEach((userId) => {
      ...
      if (communityProfileId) {
        ...
        if (!profile || seenProfiles.has(communityProfileId))
          return
        ...
        const isActive = isCommunityHubView && isPrimary
        const profile = findProfile({ authedProfilesById, userId: authedUser.id })
        userProfiles.push({
          ...
          icon: jsx('div', {
            className: ICON_CONTAINER_CLASS,
            children: jsx(renderAvatar, { userId: authedUser.id, forceAvatar: true }),
          }),
          isActive,
          isChecked: isActive,
          className: COMMUNITY_OPTION_CLASS,
    ...
    Object.keys(orgById).forEach((orgId) => {
      ...
      if (communityProfileId) {
        ...
        const profile = findProfile({ authedProfilesById, orgId: org.id })
        if (profile) {
          orgProfiles.push({
            ...
            icon: jsx('div', {
              className: ICON_CONTAINER_CLASS,
              children: jsx(renderAvatar, { userId: user.id, orgId: org.id, forceAvatar: true }),
            }),
            isActive,
            itemBadge: jsx(Badge, {
              className: ADMIN_BADGE_CLASS,
  ...
  if (communityItems.length > 1) {
    const headerItem = {
      ...
      optionHeight: USER_OPTION_HEIGHT,
      itemBadge: hasOrgOrTeamProfiles
        ? jsxs('div', {
            className: 'workspace_switcher--browseCommunityRowContainer--X6NW8',
            children: [
              renderI18nText('navbar.community.browse_community_workspace_switcher_label'),
              jsx('a', {
                href: 'https://help.figma.com/hc/articles/4404108672663',
                target: '_blank',
                className: 'workspace_switcher--infoBadgeContainer--5BRbv',
                children: jsx(SvgComponent, { svg: SVG, className: 'workspace_switcher--infoBadge--kvJnm' }),
              }),
            ],
          })
        : undefined,
    ...
    return [
      {
        ...
        'icon': jsxs('div', {
          className: ICON_CONTAINER_CLASS,
          children: [hasNotifications && jsx('div', { className: RED_DOT_CLASS }), jsx(MediaQuerySvgComponent, { svg: SVG3, fallbackSvg: SVG2 })],
        }),
...
function WorkspaceSwitcherDropdown() {
  ...
  const hasOrgOrTeamProfiles = useSelector((state: any) =>
    Object.keys(state.authedProfilesById).some(id => !!state.authedProfilesById[id].org_id || !!state.authedProfilesById[id].team_id),
  )
 ...
 * @returns The JSX element.
 */
export function WorkspaceSwitcherButton(props: { dataTestId?: string, text: string }) {
      ...
      jsx('button', {
        ...
        'onClick': (event) => {
          ...
          isDropdownShown
            ...
            : rect && dispatch(showDropdownThunk({ type: DROPDOWN_TYPE, data: { targetRect: rect } }))
        },
