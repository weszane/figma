...
import { EV, SceneGraphNode } from '../figma_app/404307'
...
function createMixinNodes() {
  ...
  class ComponentPropRefsNode extends createEmptyMixin7(StateGroupNode) {
    ...
    setComponentPropertyReferences(e) {
      this.setGlobalNodeID()
      if (SceneNodeCpp)
        return SceneNodeCpp.setComponentPropertyReferences(e)
    }
  ...
  class GuidesNode extends createEmptyMixin16(TopLevelFrameNode) {
    ...
    set guides(e) {
      ...
      if (!Mm(e, i))
        throw new Error('Invalid format for guides')
      this.setGlobalNodeID()
  ...
  class InstanceNode extends createEmptyMixin14(GuidesNode) {
    ...
    swapComponent(e) {
      if (!this.isInstance)
        throw new Error('Can only perform swaps on instances')
      this.setGlobalNodeID()
    ...
    detachInstance() {
      if (!this.isInstance)
        throw new Error('Can only detach instances')
      this.setGlobalNodeID()
    ...
    get exposedInstances() {
      if (!this.isInstance)
        throw new Error('Can only get exposed instances of an instance')
      this.setGlobalNodeID()
  ...
  class JsxNode extends createEmptyMixin26(InteractiveSlideNode) {
    ...
    setJSXProps(e) {
      ...
      if (t)
        throw new Error(`Cannot set JSX node props: ${t}`)
    }
    ...
    clearJSXOverrides() {
      ...
      if (e)
        throw new Error(`Cannot clear JSX overrides: ${e}`)
    }
  };
  // Expandable node (X)
  ...
  class OverlayNode extends createOverlayMixin(CountNode) {
    ...
    set prototypeInteractions(e) {
      ...
      if (!Mm(e, i))
        throw new Error('Invalid format for prototypeInteractions')
      this.setGlobalNodeID()
    ...
    set objectAnimations(e) {
      ...
      if (!Mm(e, i))
        throw new Error('Invalid format for objectAnimations')
      this.setGlobalNodeID()
  ...
  class StateGroupVariantNode extends createEmptyMixin27(LayoutNodeWithMixins) {
    ...
    variantProperties(e = Confirmation.NO) {
      if (!this.isInstance && !this.isState)
        throw new Error('Can only get variant properties of a variant or variant instance')
      this.setGlobalNodeID()
  ...
  class TextNode extends createTextMixin(TableNode) {
    get fontSize() {
      if (this.setGlobalNodeID(), !TextFacetTsApiGenerated)
        throw new Error('TextFacetTsApiGenerated is not defined')
  ...
  class ArcDataNode extends createArcDataMixin(VariableCollectionNode) {
    ...
    set vectorDataInfo(e) {
      ...
      if (t == null || t < 0 || t >= e.blobs.length)
        throw new Error('Invalid input for vectorData')
      this.setGlobalNodeID()
  ...
  class WidgetNode extends createWidgetMixin(BooleanOperationNode) {
    ...
    removeWidgetWithoutSafetyChecks() {
      if (this.type !== 'WIDGET')
        throw new Error('Can only remove widgets')
      this.setGlobalNodeID()
...
export class SceneNode extends createMixinNodes() {
  ...
  set exportSettings(e) {
    ...
    if (!Mm(e, i))
      throw new Error('Invalid format for export settings')
    this.setGlobalNodeID()
  ...
  set layoutGrids(e) {
    ...
    if (!Mm(e, i))
      throw new Error('Invalid format for layoutGrids')
    this.setGlobalNodeID()
  ...
  set effects(e) {
    ...
    if (!Mm(e, i))
      throw new Error('Invalid format for effects')
    this.setGlobalNodeID()
  ...
  set videoPlayback(e) {
    if (this.setGlobalNodeID(), void 0 === e.autoplay || void 0 === e.mediaLoop || void 0 === e.muted)
      throw new Error('Missing playback setting')
  ...
  set dashPattern(e) {
    ...
    if (!Mm(e, i))
      throw new Error('Invalid format for dashPattern')
    this.setGlobalNodeID()
  ...
  export(e) {
    this.setGlobalNodeID()
    if (!e)
      return SceneNodeCpp.exportAsImage('', this.sceneGraph.nodeContext)
    {
  ...
  getStyledTextSegments(start: number, end: number = -1, altEnd: number = -1) {
    ...
    const decodePaints = (data: any, key: string, decodeFn: (k: string, v: any) => any, errMsg: string) =>
      data ? assertNotNullish(decodeFn(key, data), errMsg) : data

    const segments = rawSegments.map((segment) => {
      const { fills, textDecorationFills, ...rest } = segment
      ...
      if ('textDecorationFills' in segment) {
        result.textDecorationFills = decodePaints(
          textDecorationFills,
          'textDecorationFillPaints',
          dG,
          'Figma Internal Error: textDecorationFillPaints not found',
        )
      }
      return result
    })
  ...
  componentPropertyDefinitions(e = Confirmation.NO) {
    if (!this.isLooseComponent && !this.isStateGroup && !this.isCodeComponent)
  ...
  componentProperties(e = Confirmation.NO) {
    if (!this.isInstanceOrCodeInstance)
      throw new Error('Can only get component properties of an instance')
  ...
  addComponentPropertyDefinition(e, t, i, r) {
    if (!this.isLooseComponent && !this.isStateGroup)
      throw new Error('Can only set component property definitions on a product component')
  ...
  editComponentPropertyDefinition(e, t = '', i, n) {
    if (!this.isLooseComponent && !this.isStateGroup)
      throw new Error('Can only edit component property definitions on a product component')
    this.setGlobalNodeID()
  ...
  deleteComponentPropertyDefinition(e) {
    if (!this.isLooseComponent && !this.isStateGroup)
      throw new Error('Can only delete component property definitions on a product component')
    this.setGlobalNodeID()
  ...
  getSitesJSON(e) {
    this.setGlobalNodeID()
    if (!SitesBindingsCpp)
      throw new Error('SitesBindingsCpp is not available in getSitesJSON')
  ...
  get codeFileCanvasNode() {
    if (this.setGlobalNodeID(), !this.isCodeFile)
      return null
  ...
  setInheritedStyleKey(e, t) {
    if (t && !$P(t))
      throw new Error(`Invalid style key ${t.key} and version ${t.version}`)
    let i = t ?? {
      key: 'INVALID',
      version: 'INVALID',
    }
    ...
    if (n)
      throw new Error(`Cannot set style successfully: ${n}`)
  }
  ...
  setRangeInheritedStyleKey(e, t, i, n) {
    if (n && !$P(n))
      throw new Error(`Invalid style key ${n.key} and version ${n.version}`)
    let r = n ?? {
      key: 'INVALID',
      version: 'INVALID',
    }
    ...
    if (a)
      throw new Error(`Cannot set style successfully: ${a}`)
  }
  ...
  setGridRowCount(e, t = !1) {
    ...
    if (i)
      throw new Error(`Cannot set grid row count: ${i}`)
  }
  ...
  setGridColumnCount(e, t = !1) {
    ...
    if (i)
      throw new Error(`Cannot set grid column count: ${i}`)
  }
  ...
  setGridChildPosition(e, t) {
    ...
    if (i)
      throw new Error(`Cannot set grid child position: ${i}`)
  }
  ...
  appendChildToGridAtPosition(e, t, i) {
    ...
    if (n)
      throw new Error(`Cannot append child to grid at specified position: ${n}`)
  }
  ...
  setSpanAndUpdateChildPositions(e, t) {
    ...
    if (i)
      throw new Error(`Cannot set span and update child positions: ${i}`)
  }
  ...
  setGridTrackSize(e, t, i) {
    ...
    if (n)
      throw new Error(`Cannot set grid track size: ${n}`)
  }
  ...
  setGridTrackType(e, t, i) {
    ...
    if (n)
      throw new Error(`Cannot set grid track sizing type: ${n}`)
  }
