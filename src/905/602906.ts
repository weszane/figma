 ...
 * (original: c, u)
 */
const fpsDataMap = new WeakMap<DurationVital, Array<{ timestamp: number, fps: number }>>()
...
class ObservabilityClient {
  ...
  addAction(
    ...
    if (
      options?.forceEnable
      || (this.isEnabled && getFeatureFlags().datadog_rum_fs_action_logger)
    ) {
  ...
  logVital(
    ...
    if (this.isEnabled && (level !== LogLevelStr.DEBUG || isDebugFlowActive())) {
      if (startTime < performance.timeOrigin) {
        ...
        console.warn(
          'You appear to be logging a relative timestamp and not an epoch timestamp. Try adding performance.timeOrigin to the timestamp.',
        )
      }
...
class UserFlow {
  ...
  measureDuration<T>(
    name: string,
    callback: () => T,
    options?: { level?: LogLevelStr, description?: string },
  ): T {
...
class DurationVital {
  ...
  private startFrameRateMonitoring() {
    ...
    const monitor = () => {
      ...
      if (elapsed >= 1000) {
        ...
        fpsStats && fpsStats.push({ timestamp: now, fps })
        this.frameCount = 0
   ...
   * Get FPS data array.
   */
  getFpsData(): Array<{ timestamp: number, fps: number }> | undefined {
  ...
  start() {
    if (this.started) {
      observabilityClient.addError(
        new Error(`DurationVital ${this.name} already started`),
        ServiceCategories.UNOWNED,
      )
      return
    }
  ...
  stop() {
    if (this.stopped || !this.started) {
      observabilityClient.addError(
        new Error(
          `DurationVital ${this.name} ${this.stopped ? 'already stopped' : 'stopped but never started'}`,
        ),
        this.team,
      )
      return
...
function calculateFrameTimeStats(
  ...
  const stddev = Math.sqrt(
    data.map(v => (v - avg) ** 2).reduce((sum, v) => sum + v, 0) / data.length,
  )
