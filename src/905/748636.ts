...
interface DraggableModalRef {
  ...
  getCurrentContentSize: () => { x: number, y: number } | undefined
...
interface DraggableModalInternalState {
  ...
  startingDragPosition: { elementPosition: Point, mousePosition: Point } | null
...
class DraggableModalInternal extends Component<DraggableModalInternalProps, DraggableModalInternalState> {
  ...
  getOnScreenPositionOfHeader = (element: HTMLElement) => {
    const { top, width, left } = element.getBoundingClientRect()
    const rect = new DOMRect(left, top, width, HEADER_HEIGHT)
    return this.getOnScreenPositionOfBoundingRect(rect)
  }

  getOnScreenPositionOfElement = (element: HTMLElement) =>
    this.getOnScreenPositionOfBoundingRect(element.getBoundingClientRect())
  ...
  getOnScreenPositionOfBoundingRect = (rect: DOMRect) => {
    ...
    if (top < this.minTopMargin()) {
      ...
      y = this.state.constraints.y === 'top'
        ? Math.max(this.minTopMargin(), window.innerHeight - (height + this.minBottomMargin()))
    ...
    if (this.props.allowPartialHeaderOnScreen) {
      if (right < this.minLeftMargin() + 40) {
        ...
        x = this.state.constraints.x === 'left'
          ? window.innerWidth - this.minRightMargin() - 40
          : window.innerWidth - this.minRightMargin() - width - 40
      }
    }
    else {
      if (left < this.minLeftMargin()) {
        ...
        x = this.state.constraints.x === 'left'
          ? window.innerWidth - this.minRightMargin() - width
          : window.innerWidth - this.minRightMargin()
      }
    }
  ...
  onDrag = (event: any) => {
    if (!this.state.startingDragPosition)
      return
  ...
  componentDidUpdate(prevProps: DraggableModalInternalProps, prevState: DraggableModalInternalState, snapshot: any) {
    ...
    if (this.state.hasCustomPosition) {
      ...
      if (prevProps.initialConstraints?.x !== this.props.initialConstraints?.x
        || prevProps.initialConstraints?.y !== this.props.initialConstraints?.y) {
    ...
    if ((this.props.alwaysEnsureModalOnScreen || this.props.alwaysEnsureHeaderModalOnScreen)
      && constraintsChanged && !this.state.shouldCallEnsureModalIsOnScreen) {
      this.setState({
        shouldCallEnsureModalIsOnScreen: true,
      })
    }
...
export const DraggableModal = forwardRef<DraggableModalRef, DraggableModalManagerProps>((props, ref) =>
