...
async function shouldLoadAllPages(plugin: any, queryMode: boolean): Promise<boolean> {
  ...
  if (!pluginId || (plugin && hasTextReviewCapability(plugin)) || $A(debugState.getState().selectedView)) {
    return false
  }
  if (queryMode)
    return false
 ...
 */
export async function loadAllPagesWithUI(plugin: any, event: any, modalType: PluginModalType): Promise<{ isCancelled: boolean }> {
  ...
  const message = plugin && manifestContainsWidget(plugin)
    ? getI18nString('plugins.loading_pages_for_widget', { numUnloadedPages: unloadedPagesCount })
    : getI18nString('plugins.loading_pages_for_plugin', { numUnloadedPages: unloadedPagesCount })
  const dequeueLoadingBell = () => {
    fullscreenValue.dispatch(VisualBellActions.dequeue({ matchType: 'loading-pages-for-plugin' }))
  }
  ...
  if (plugin && manifestContainsWidget(plugin) && event?.widgetAction === 'insert') {
    fullscreenValue.dispatch(VisualBellActions.dequeue({ matchType: 'plugins-status' }))
  }
 ...
 * @returns {Promise<{isCancelled: boolean}>} Result indicating if cancelled.
 */
export async function waitForAllPagesForPlugin(event: any): Promise<{ isCancelled: boolean }> {
