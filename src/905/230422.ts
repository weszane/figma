...
interface FollowsListModalState {
  followerState: {
    followers: any[]
    loading: boolean
    pagination?: any
...
interface FollowsListModalProps {
  ...
  currentSelectedView: {
    profileTab?: UserProfileTab
  }
  currentUserProfileId: string
  authedActiveCommunityProfileId: string
  dispatch: Fn
}

class FollowsListTab extends TabWithRecording {
...
class FollowsListModal extends Component<FollowsListModalProps, FollowsListModalState> {
   lastProfileRef = createRef<HTMLDivElement>()
   modalContainerRef = createRef<HTMLDivElement>()
   scrollListener: () => void

  constructor(props: FollowsListModalProps) {
    super(props)

    // Initialize debounced scroll listener
    this.scrollListener = debounce(this.handleScroll.bind(this))

    // Redirect to followers tab if following tab is selected but not available
    if (!props.profile.primary_user_id && props.currentSelectedView.profileTab === UserProfileTab.FOLLOWING) {
      props.dispatch(selectViewAction({
        ...props.currentSelectedView,
  ...
  componentDidUpdate(prevProps: FollowsListModalProps) {
    if (prevProps.currentSelectedView.profileTab !== this.props.currentSelectedView.profileTab) {
      this.fetchNextFollows()
    }
  }
   ...
   unlockBodyScroll() {
    ...
    window.scrollTo(0, -1 * parseInt(scrollTop || "0"))
  }

   handleScroll() {
    if (this.lastProfileRef.current && this.modalContainerRef.current) {
      const lastProfileTop = this.lastProfileRef.current.getBoundingClientRect().top
      const containerBottom = this.modalContainerRef.current.getBoundingClientRect().bottom

      if (lastProfileTop <= containerBottom) {
        this.fetchNextFollows()
      }
    }
  }
   ...
   fetchNextFollows() {
    const { profileTab } = this.props.currentSelectedView
    if (profileTab === UserProfileTab.FOLLOWERS) {
      this.fetchFollowers()
    }
    else {
      this.fetchFollowing()
   ...
   async fetchFollowers() {
    ...
    try {
      const response = await fetchPaginatedData(
        `/api/followers/${this.props.profile.id}`,
        DEFAULT_PAGE_SIZE,
        this.state.followerState,
        PAGINATION_NEXT,
      )
      ...
      this.setState({
        followerState: {
          followers: [...this.state.followerState.followers, ...response.followers],
          loading: false,
   ...
   async fetchFollowing() {
    ...
    try {
      const response = await fetchPaginatedData(
        `/api/following/${this.props.profile.id}`,
        DEFAULT_PAGE_SIZE,
        this.state.followingState,
        PAGINATION_NEXT,
      )
      ...
      this.setState({
        followingState: {
          following: [...this.state.followingState.following, ...response.following],
          loading: false,
          pagination: response.pagination,
        },
        didFetchFail: false,
   ...
   handleTabClick(tab: UserProfileTab) {
   ...
   hideModal() {
    ...
    delete updatedView.profileTab
    this.props.dispatch(selectViewAction(updatedView))
  }

   getFollowsListHeightStyle(tab: UserProfileTab) {
    const maxHeight = isMobileUA ? window.innerHeight : window.innerHeight - 128 - 48
    const count = tab === UserProfileTab.FOLLOWERS
      ? this.props.profile.follower_count
      : this.props.profile.following_count
    const calculatedHeight = 80 * Math.min(count, DEFAULT_PAGE_SIZE)

    return {
      height: Math.min(Math.max(calculatedHeight, 328), maxHeight),
    }
  }
   ...
   renderTabs() {
    ...
    return jsxs("div", {
      ...
      children: [
        jsxs(FollowsListTab, {
          onClick: () => this.handleTabClick(UserProfileTab.FOLLOWERS),
        ...
        this.props.profile.primary_user_id && jsxs(FollowsListTab, {
          onClick: () => this.handleTabClick(UserProfileTab.FOLLOWING),
   ...
   renderView() {
    const { profileTab } = this.props.currentSelectedView
    const currentState = profileTab === UserProfileTab.FOLLOWERS
      ? this.state.followerState
      : this.state.followingState
    const profiles = profileTab === UserProfileTab.FOLLOWERS
      ? this.state.followerState.followers
      : this.state.followingState.following

    if (profiles.length > 0 || currentState.loading) {
      return jsx(TrackingProvider, {
        ...
        children: jsxs("div", {
          ...
          style: this.getFollowsListHeightStyle(profileTab),
          onScroll: this.scrollListener,
          children: [
            profiles.map((profile, index) =>
              jsx(ProfileRowTile, {
                profile,
                tileRef: index === profiles.length - 1 ? this.lastProfileRef : undefined,
                rightSide: this.props.authedActiveCommunityProfileId !== profile.id
                  ? jsx(ProfileFollowButton, {
                    profile,
                    trackingProperties: {
                      modalTab: profileTab,
                  ...
                  : undefined,
                hideFollowsYouBadge: this.props.profile.id === this.props.currentUserProfileId && profileTab === UserProfileTab.FOLLOWERS,
              }, profile.profile_handle),
            ),
            currentState.loading && jsx(LoadingSpinner, {
  ...
  static displayName = "FollowsListModal"
}


export const registerFollowsListModal = registerModal(FollowsListModal, "FollowsListModal")

export const w = registerFollowsListModal
