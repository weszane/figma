...
export const SvgComponent: React.FC<SvgComponentProps> = (props) => {
  ...
  const buildSvgHtml = (svg: any, props: SvgComponentProps): string => {
    ...
    let styleStr = "";
    if (props.svgWidth) styleStr += `width: ${props.svgWidth}; `;
    if (props.svgHeight) styleStr += `height: ${props.svgHeight}; `;
    if (styleStr) svgAttrs += `style="${styleStr}" `;
    if (props.title) svgAttrs += `title="${props.title}" `;
    if (props.role !== "button" && props.ariaLabel) svgAttrs += `aria-label="${props.ariaLabel}" `;
    if (props.ariaHidden) svgAttrs += 'aria-hidden="true" ';
  ...
  const prepareSpanProps = (props: SvgComponentProps) => {
    ...
    if (props.className) spanClass += ` ${props.className}`;
    if (autosize) {
      spanClass += " svg--autoscale---9awM";
      rest.style = { ...rest.style, height: props.height, width: props.width };
    }
    if (props.onClick && props.recordingKey) rest.onClick = onClick;
    if (props.onMouseDown && props.recordingKey) rest.onMouseDown = onMouseDown;
    if (props.onPointerUp && props.recordingKey) rest.onPointerUp = onPointerUp;
  ...
  return jsx("span", {
    ref: (el: HTMLSpanElement | null) => {
      spanRef.current = el;
      if (props.innerSpanRef) props.innerSpanRef.current = el;
    },
