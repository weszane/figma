...
export class DraggableModalView extends RecordingComponent {
  ...
  contextValue: (isResizing: boolean, isDragging: boolean) => {
    isResizing: boolean;
    isDragging: boolean;
  };
  constructor(props) {
    ...
    this._onMouseDown = event => {
      ...
      if (event.button === 2 || this.props.disableDragging) return;
      let target = event.target;
      const isInteractiveElement = target && (target.tagName.toLowerCase() === 'input' || target.tagName.toLowerCase() === 'label' || target.tagName.toLowerCase() === 'img' || target.tagName.toLowerCase() === 'a');
      const hasNotDraggableAttribute = target && typeof target.closest == 'function' && target.closest('[data-not-draggable]');
    ...
    this.documentMouseMove = handleMouseEvent(this, 'mousemove', event => {
      if (this.props.hidden || this.dragState === 'None') return SKIP_RECORDING;
    ...
    this.onKeyDown = event => {
      ...
      if (event.event.keyCode === KeyCodes.ESCAPE && !this.props.ignoreCloseShortcut && this.props.onClose) {
  ...
  calculatePixelPerfectDimensions({
    ...
    return [[Math.round, Math.round], [Math.floor, Math.floor], [Math.ceil, Math.ceil], [Math.round, Math.ceil], [Math.floor, Math.round], [Math.floor, Math.ceil]].map(([roundWidth, roundHeight]) => ({
      ...
      roundedWidth,
      roundedHeight
    }) => roundedWidth / roundedHeight <= aspectRatio).reduce(({
      roundedWidth,
      roundedHeight
    }, {
      roundedWidth: newWidth,
        ...
        'children': jsxs(ContainerComponent, {
          ...
          children: [jsxs('div', {
            ...
            children: [this.props.showArrow && this.props.arrowRelativeX && jsx('div', {
              ...
              children: this.props.title != null ? jsx(HeaderModalContainer, {
                className: hv,
