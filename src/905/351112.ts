...
function $$A(e) {
  return jsxs('div', {
    ...
    }, `DragImageItem${i}`)).reverse(), !e.multiselectDisabled && e.selectedItems.length
      ? jsx('div', {
...
function N(e) {
  ...
  let G = useHandleKeyboardEvent('listViewKeyDown', 'keydown', (t) => {
    switch (t.key) {
      case 'a':
        if (!t.ctrlKey && !t.metaKey || e.multiselectDisabled)
          break
        O(new Set([...Array.from({ length: e.items.length }).keys()]))
        p(e.items.length - 1)
        h(0)
        t.preventDefault()
        t.stopPropagation()
