...
import { isRightKey, useKeyboardNavigationItem, isLeftKey } from '../figma_app/119475';
...
export function TreeRow({
  ...
  const isButtonTarget = useCallback((e: React.MouseEvent) => e.target instanceof Element && e.target.tagName === 'BUTTON', []);

  // Helper: isExpansionToggleTarget (original: L)
  const isExpansionToggleTarget = useCallback((e: React.MouseEvent) => !!(e.target instanceof Element && (e.target.classList.contains(p) || e.target.parentElement?.classList.contains(p))), []);
  ...
  return jsx('button', {
    ...
    onKeyDown: (e: React.KeyboardEvent<HTMLButtonElement>) => {
      if (isLeaf && isLeftKey(e.code, true) || !isLeaf && isRightKey(e.code, true)) {
        toggleExpanded(e.metaKey);
      }
    },
    ...
    children: jsxs(AutoLayout, {
      children: [jsx(TreeIndent, {
        indent,
        isLeaf,
        isExpanded
      }), children, togglePinned && jsx(PinToggle, {
        isPinned: !!isPinned,
        togglePinned
      }), changeType && jsx(renderChangeBadge, {
        type: changeType
      })]
    })
  });
...
export function PinToggle({
  ...
  return jsx('span', {
    ...
    className: isPinned ? 'tree_row--unpinToggle--CU4-x tree_row--_pinToggleBase--W1djM' : 'tree_row--pinToggle--wbULT tree_row--_pinToggleBase--W1djM',
