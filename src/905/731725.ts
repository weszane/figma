...
import { paginatedFolderQuery, useFolderQuery } from '../figma_app/199513';
 ...
 * Refactored from $$c0.
 */
export const setupFolderPreviewHandler = getFeatureFlags().folder_preview_file_pagination ? function handlePaginatedFolderPreview({
  folderId,
  ...
  const [resourceAtom, resourceActions] = setupResourceAtomHandler(paginatedFolderQuery({
    folderId,
  ...
  const shouldFetchNextPage = previewFiles.status === 'loaded' && previewFiles.data.length < 8 && resourceAtom.hasNextPage && !resourceAtom.isFetchingNextPage;
  ...
  // resourceUtils.useTransform
  const previewFiles = resourceUtils.useTransform(useFolderQuery(folderId, true, shouldShowOnlyTrashedFiles), atom => atom.previewFiles);
