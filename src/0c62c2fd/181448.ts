...
export function $$U1({
  ...
  let et = useDispatch();
  let er = "whiteboard" === getCurrentFileType();
  let [ea, es] = useState(!1);
  let ei = useAtomWithSubscription(_$$l2);
  let en = useAtomWithSubscription(Yh);
  let eo = useDeepEqualSceneValue((e, t, r) => getFeatureFlags().gate_memory_usage_computations && !r ? null : e.get(t)?.nodeMemoryUsage, r, z);
  let el = useDeepEqualSceneValue((e, t, r) => getFeatureFlags().gate_memory_usage_computations && !r ? null : e.get(t)?.rootMemoryUsage, r, z);
  let ed = useMemo(() => getNodeStatus(X, r) === DataLoadStatus.LOADED, [r, X]);
