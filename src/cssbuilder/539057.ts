...
import {
  ...
  modalZ,
  secondaryModalZ,
  templateModalTeamName,
  topBarZ,
} from '../figma_app/786175'
...
export type CssPropertyMap = typeof cssPropertyMap
export type CssValues = CssPropertyMap[keyof CssPropertyMap][number]
export type CssValueName = CssValues[0]
export type CssValue = CssValues[1]

/**
 * Base style builder class.
 * Original class: a
 */
...
/**
 * Dynamically define property getters for each CSS property value.
 * Original code: Object.entries(t).forEach(...)
 */
Object.entries(cssPropertyMap).forEach(([property, values]) => {
  Object.defineProperties(
    StyleBuilderBase.prototype,
    Object.fromEntries(
...
type Extended = { [P in CssValueName]: StyleBuilder }

export function StyleBuilderFactory(rules: Record<string, string | number> = {}): StyleBuilder & Extended {
  return new StyleBuilder(rules) as StyleBuilder & Extended
}
 ...
 * Original class: $$n0
 */
export class StyleBuilder extends StyleBuilderBase {
  rules: Record<string, string | number>

  constructor(rules?: Record<string, string | number>) {
    super()
    this.rules = rules
  }
   ...
   * Original method: add
   */
  add = (input: Record<string, string | number> | StyleBuilder): StyleBuilder => {
    const rules = input instanceof StyleBuilder ? input.rules : input
  ...
  /**
   * Get style rules object.
   * Original getter: $
   */
  get $() {
    return this.rules as React.CSSProperties
  }

  /**
   * Get style rules as CSS string.
   * Original method: str
   */
  str(): string {
  ...
  /**
   * Prevent using toString directly.
   ...
   * Original method: if
   */
  if(condition: boolean, thenStyle: Record<string, string | number> | StyleBuilder, elseStyle?: Record<string, string | number> | StyleBuilder): StyleBuilder {
    return condition ? this.add(thenStyle) : elseStyle ? this.add(elseStyle) : this
  }

  /**
   * Case style addition.
   * Original method: case
   */
  case(cases: Array<[boolean, Record<string, string | number> | StyleBuilder]>, defaultStyle?: Record<string, string | number> | StyleBuilder): StyleBuilder {
    for (const [cond, style] of cases) {
      if (cond)
        return this.add(style)
    }
    return defaultStyle !== undefined ? this.add(defaultStyle) : this
  }

  /**
   * Match style addition.
   * Original method: match
   */
  match<K extends string | number>(
    key: K,
    map: Record<K, Record<string, string | number> | StyleBuilder>,
  ): StyleBuilder {
    const style = map[key]
    return style ? this.add(style) : this
  }
}

/**
 * Export with original variable name.
 * Original export: export const r = $$n0;
 */
export const r = StyleBuilder
