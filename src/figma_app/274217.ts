...
export function LinkPreviewWidget({
  ...
  widget.register(() => {
    ...
    function getTextStates({
      ...
      return { title, description, fullUrlProvider }
    }
    ...
    function handleOpenLink() {
      ...
      debugState.dispatch(setupHyperlinkHandler({ rawInput: url }))
    }
      ...
      if (text) {
        return widget.h(Text, {
          ...
          fill: {
            ...
            color: { r: 0, g: 0, b: 0, a: hasTitle ? 0.5 : 0.8 },
          },
        }, text)
      }
      return null
    ...
    if (orientation === 'vertical') {
      return renderVerticalLayout(
        !hasThumbnail && widget.h(Frame, { width: 304, height: 0.01 }),
        hasThumbnail && renderThumbnail(
          304,
          thumbnailAspectRatio ? Math.max(Math.min(304 / thumbnailAspectRatio, 608), 160) : 160,
          thumbnailImageHash,
        ),
        hasTitleOrDescription
        && widget.h(AutoLayout, {
        ...
        }, renderTitle(title), renderDescription(hasTitle, description)),
        widget.h(AutoLayout, {
      ...
      return renderHorizontalLayout(
        hasThumbnail,
        !hasThumbnail && widget.h(Frame, { width: 0.01, height: 0.01 }),
        hasThumbnail && renderThumbnail(112, 112, thumbnailImageHash),
        widget.h(AutoLayout, {
          direction: 'vertical',
          horizontalAlignItems: 'center',
          verticalAlignItems: 'start',
