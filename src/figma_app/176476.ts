...
import { wP, n5, Q3, vY } from "../905/457102";
let m = [{
...
class f extends PureComponent {
  ...
  append(e) {
    ...
    if ((null === this.displayedNumber || d - this.lastDisplayedUpdateTime > 150) && (this.displayedNumber = e, this.lastDisplayedUpdateTime = d), r.fillText(`sample: ${this.displayedNumber.toFixed(1)}`, 0, 0), r.fillText(`${n.toFixed(1)} - ${i.toFixed(1)}`, 0, 10), r.fillStyle = this.props.bg, this.props.autoResize) {
      if (e > this.yAxisMaxValue) {
        let n = Math.pow(2, Math.ceil(Math.log2(e / this.yAxisMaxValue)));
        ...
        let e = Math.pow(2, Math.ceil(Math.log2(this.yAxisMaxValue / Math.max(this.props.max, i))));
...
class y extends PureComponent {
  constructor(e) {
    ...
    this.updateTimers = () => {
      if (this.tsmerMetrics.onFrame(), null == this.lastFrameTime) this.lastFrameTime = performance.now();else {
      ...
      t && t.append(E(hMR.getTotalUsedHeapMemory()));
      let r = this.sparklines.get("gpu-memory");
      r && hMR?.getMemStatsTotal && r.append(E(hMR.getMemStatsTotal(YOB.RENDERER)));
      let n = this.sparklines.get("cpp-malloc-call-count");
      n && n.append(hMR.getAllocCallCount());
      let i = this.sparklines.get("cpp-malloc-size-totals");
      i && i.append(hMR.getAllocTotal() / 1024);
      hMR.resetAllocCountsAndTotal();
      let a = this.sparklines.get("render-job-queue-length");
      a && a.append(CeL.getRendererJobQueueLength());
      ...
      c && c.append(glU.estimatedViewportRenderCoverage(pWM.DEFINITELY_COVERED_ONLY));
...
export function $$T0(e) {
  b ? (unmountComponentAtNode(b), document.body.removeChild(b), b = null) : (b = document.createElement("div"), document.body.appendChild(b), H(b).render(jsx(y, {
    isFigjamFile: e
  })));
}
export const v = $$T0;