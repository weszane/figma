...
export function ModalView(props) {
  ...
  const handleBackgroundMouseDown = useCallback(() => {
    if (!disableClickOutsideToHide) hide('click_outside');
  ...
  const handleKeyDown = useCallback(e => {
    if (e.keyCode === KeyCodes.ESCAPE && (!disableClickOutsideToHide || enableEscapeToClose)) {
      hide('press_escape');
      e.preventDefault();
      e.stopPropagation();
      return;
  ...
  useEffect(() => {
    if (disableFocusOnMount) return;
    if (backgroundRef.current.matches(':focus-within')) return;
...
export function ConfirmationModal2(props) {
  ...
  const closeButtonHandler = useHandleMouseEvent(recordingKey, 'click', handleCloseButtonClick);

  // Handles confirm action
  const handleConfirm = useCallback(e => {
    e.preventDefault();
    if (hideOnConfirm ?? true) handleClose();
    onConfirm();
  }, [handleClose, hideOnConfirm, onConfirm]);

  // Playback handler for confirm
  const confirmPlaybackHandler = useSetupPlayback(recordingKey, 'confirm', handleConfirm, playbackHandlerStub);

  // Handles cancel action
  const handleCancel = useCallback(e => {
    e.preventDefault();
    if (hideOnCancel ?? true) handleClose();
    onCancel?.();
  }, [handleClose, hideOnCancel, onCancel]);

  // Playback handler for cancel
  const cancelPlaybackHandler = useSetupPlayback(recordingKey, 'cancel', handleCancel, playbackHandlerStub);

  // Choose button type
  const ConfirmButton = destructive ? ButtonNegativeTracked : ButtonBasePrimaryTracked;
  ...
  return jsxs(ModalContainer, {
    ...
    children: [showCloseButton && jsx(ModalUpperRightCorner, {
      ...rest,
      className: closeClass,
      onClick: closeButtonHandler
    }), jsxs(TabLoop, {
      displayAs: TabLoopDisplayAs.Block,
      children: [!hideConfirmationTitle && jsx('div', {
        className: titleClass ?? DD,
        children: jsxs('h2', {
          children: [' ', confirmationTitle ?? renderI18nText('modal.are_you_sure')]
        })
      }), children || jsx('div', {
        className: jE,
        children: content
