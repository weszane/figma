...
let $$K10 = createOptimistThunk((e, t) => {
  ...
  r = uuid ? getCurrentLiveGraphClient().optimisticallyDeleteWithUUID({
    Comment: {
      [uuid]: null
    }
  }, d) : getCurrentLiveGraphClient().optimisticallyDelete({
    Comment: {
      [id]: null
    }
  }, d);
...
let $$Y34 = createOptimistThunk((e, {
  ...
  if (n && containsDash(r)) {
    ...
    n ? getCurrentLiveGraphClient().optimisticallyUpdateWithUUID({
      Comment: {
        [n]: {
          uuid: n,
          resolvedAt: i ? new Date() : null
...
let $$$33 = createOptimistThunk((e, {
  ...
  let a;
  n && containsDash(r) ? (a = getCurrentLiveGraphClient().getIdFromUuid('Comment', n).then(e => sendWithRetry.put(`/api/file/${t}/comments/${e}`, {
    client_meta: i
  })), trackEventAnalytics('Livegraph Optimistic Comment Update', {
...
let $$q27 = createOptimistThunk((e, t) => {
  ...
  if (s ? getCurrentLiveGraphClient().optimisticallyUpdateWithUUID({
    Comment: {
      [s]: {
        uuid: s,
        messageMeta: o,
  ...
  }, i) : getCurrentLiveGraphClient().optimisticallyUpdate({
    Comment: {
      [t.comment.id]: {
        messageMeta: o,
        messageMetaStylized: o
    ...
    e ? getCurrentLiveGraphClient().optimisticallyDelete({
      CommentAttachment: e
    }, i) : getCurrentLiveGraphClient().emitEvent({
      type: 'EMPTY_SYNC',
      source: 'comment_delete'
    });
    let r = Mu(Object.values(t.attachmentUpdates.attachments), t.comment.key, t.comment.id);
    r ? getCurrentLiveGraphClient().optimisticallyCreate({
      CommentAttachment: r
    }, i) : getCurrentLiveGraphClient().emitEvent({
      type: 'EMPTY_SYNC',
...
let $$ea23 = createOptimistThunk((e, t) => {
  ...
  return !a && _ ? (e.dispatch(eS(_.id)), !1) : (r.selectedView.view === 'fullscreen' && Fullscreen.setActiveCommentAnchorData({
