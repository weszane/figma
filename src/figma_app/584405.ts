...
import { yU, F7 } from "../figma_app/908460";
import { v4, Ut, u2, Kk } from "../figma_app/761118";
import { yA, tS, Iv } from "../figma_app/327683";
...
export function $$I3() {
  return md(u2) !== td.GROUPING_COMPLETE;
}
export function $$S0() {
  let e = Xr(_M);
  let t = sf();
...
export function $$A4() {
  return useCallback(async () => {
    let e = r8();
    if (!e) throw Error("Linter is not initialized");
    try {
      await e.requestDetection();
    } catch (e) {
      if (e instanceof yU) {
        zl.set(_M, FR.LANDING_PAGE_LIMIT_EXCEEDED);
        return;
      }
      if (e instanceof F7) {
        zl.set(_M, FR.LANDING_PAGE_NO_SELECTION);
        return;
      }
...
export function $$x7(e) {
  ...
  return useCallback(() => {
    t();
    let r = zl.get(IZ).map(e => e.violationId);
