...
import { isBigmaEnabledSimple, getCurrentUserOrg } from '../figma_app/336853';
...
export function isSelectedViewMissingOrgAdminResources({
  ...
  const hasBigma = getCurrentUserOrg(permissions);
...
export function isSelectedViewMissingOrgAdminSettingsResources({
  ...
  const hasBigma = getCurrentUserOrg(permissions);
  const currentUserOrgUser = getCurrentUserOrgUser(permissions);
  return !hasBigma || !currentUserOrgUser || !hasBigma.bigma_enabled || view.subView === UserGroupRole.ADMIN && !isAdminOrg && !currentUserOrgUser.license_admin;
}
...
export function isSelectedOrgAdminSettingsMissingResources({
  ...
  const hasBigma = getCurrentUserOrg(permissions);
  const currentUserOrgUser = getCurrentUserOrgUser(permissions);
  const hasOrg = permissions.currentUserOrgId && isBigmaEnabledSimple(permissions.orgById[permissions.currentUserOrgId]);
  const hasAdmin = isAdminOrg || currentUserOrgUser?.license_admin || currentUserOrgUser?.workspace_users?.some((u: any) => u.permission === 'admin');
  const isWorkspaceTab = orgAdminSettingsViewTab !== DashboardSection.WORKSPACES && orgAdminSettingsViewSecondaryTab !== WorkspaceTab.WORKSPACES;
  const validWorkspace = !permissions.currentUserOrgId || hasOrg;
  if (!(hasBigma && currentUserOrgUser && (isWorkspaceTab || validWorkspace) && hasAdmin) || orgAdminSettingsViewTab === DashboardSection.BILLING && orgAdminSettingsViewSecondaryTab === BillingSectionEnum.BILLING_GROUPS && !hasOrg) {
    return true;
  }
  return false;
}
...
export function isSelectedTeamAdminConsoleMissingResources({
  ...
  return (teamAdminConsoleViewTab === DashboardSections.BILLING || teamAdminConsoleViewTab === DashboardSections.DASHBOARD) && (!isAdminTeam || !teams[teamId] || teams[teamId].student_team || !hasTeamPaidAccess(teams[teamId]));
}
...
export const viewConfig = {
  ...
  team: {
    ...
    hasMissingResource: (state: any, view: any) => {
      ...
      const missing = view.teamViewTab === NavigationRoutes.BILLING && (!canAdminTeam(teamId, state) || !state.teams[teamId] || !state.teams[teamId].pro_team);
  ...
  teamAdminConsole: {
    ...
    hasMissingResource: () => false,
    areDifferent: (a: any, b: any) => a.teamId !== b.teamId || a.teamAdminConsoleViewTab !== b.teamAdminConsoleViewTab || a.teamAdminConsoleViewSecondaryTab !== b.teamAdminConsoleViewSecondaryTab
  },
  addCollaborators: {
  ...
  teamUpgrade: {
    slug: 'upgrade-team',
    hasMissingResource: (state: any, view: any) => !getFeatureFlags().redirect_starter_team_loophole && view.teamId === null && isUpgradeExistingTeam(view.teamFlowType) || !!view.teamId && !(view.teamId in state.teams),
    missingResourceType: EntityType.TEAM,
  ...
  workspace: {
    ...
    hasMissingResource: (state: any, view: any) => {
      ...
      const hasBigma = getCurrentUserOrg(permissions);
      const currentUserOrgUser = getCurrentUserOrgUser(permissions);
      if (!hasBigma || !currentUserOrgUser || !hasBigma.bigma_enabled) return true;
      switch (view.subView) {
    ...
    areDifferent: (a: any, b: any) => a.subView !== b.subView || a.workspaceId !== b.workspaceId || a.selectedTab !== b.selectedTab
  },
  licenseGroup: {
    slug: 'billing-group',
    hasMissingResource: () => false,
    areDifferent: (a: any, b: any) => a.subView !== b.subView || a.licenseGroupId !== b.licenseGroupId || a.selectedTab !== b.selectedTab
  },
  ...
  orgAdminSettings: {
    ...
    hasMissingResource: () => false,
    areDifferent: (a: any, b: any) => a.orgAdminSettingsViewTab !== b.orgAdminSettingsViewTab || a.orgAdminSettingsViewSecondaryTab !== b.orgAdminSettingsViewSecondaryTab || !isEqual(a.orgAdminMembersTabFilters, b.orgAdminMembersTabFilters) || !isEqual(a.orgAdminMembersTabSort, b.orgAdminMembersTabSort)
  },
  search: {
    slug: 'search',
    hasMissingResource: () => false,
...
export function reportInvalidResourceId({
  ...
  reportError(ServiceCategories.WAYFINDING, new Error(`Invalid ${resourceType} ID while parsing selected view ${matchingViewName} from path`), {
    extra: {
      matchingViewName,
      ...extra
    }
      ...
      case 'teamAdminConsole':
        {
          ...
          if (teamAdminConsoleViewTab === DashboardSections.CONTENT && teamAdminConsoleViewSecondaryTab === MemberSections.CONNECTED_PROJECTS && query && parseQuerySimple(query).rciid != null) {
            showResourceConnectionInviteModal = parseQuerySimple(query).rciid;
          }
          return {
            view: viewKey,
      ...
      case 'teamUpgrade':
        {
          ...
          const loopholeEnabled = !!getFeatureFlags().redirect_starter_team_loophole && !getFeatureFlags().close_starter_team_loophole_v2;
          const teamFlowType = Object.values(CreateUpgradeAction).includes(parts[4] as CreateUpgradeAction) ? parts[4] : CreateUpgradeAction.UPGRADE_EXISTING_TEAM;
          ...
          const paymentStep = Object.values(UpgradeSteps).includes(parts[4]) ? parts[4] : UpgradeSteps.CHOOSE_PLAN;
          const step = parts[5] || paymentStep;
          const planType = params.get('planType') ? parseInt(params.get('planType')) : teamFlowType === CreateUpgradeAction.UPGRADE_EXISTING_TEAM ? TeamType.TEAM : undefined;
          ...
          return {
            ...
            ...(params.get('entryPoint') ? {
              entryPoint: parseInt(params.get('entryPoint'))
            } : {}),
            ...(params.get('billingPeriod') ? {
              billingPeriod: parseInt(params.get('billingPeriod'))
            } : {}),
      ...
      case 'orgAdminSettings':
        {
          ...
          if (orgAdminSettingsViewTab === DashboardSection.DASHBOARD && query && parseQuery(query).dashEntryPoint != null) {
          ...
          const sortConfig = parseSortConfig(query);
          if (orgAdminSettingsViewTab === DashboardSection.MEMBERS && query && parseQuery(query).initialSort === 'date-upgraded') {
          ...
          if (orgAdminSettingsViewTab === DashboardSection.MEMBERS && query && parseQuery(query).orgJoinRequest != null) {
          ...
          if (orgAdminSettingsViewTab === DashboardSection.RESOURCES && query && (orgAdminSettingsViewSecondaryTab === FigResourceType.APPROVED_PLUGINS || orgAdminSettingsViewSecondaryTab === FigResourceType.APPROVED_WIDGETS)) {
            result.selectedExtensionId = parseQuery(query).eid;
          }
          if (orgAdminSettingsViewTab === DashboardSection.CONTENT && orgAdminSettingsViewSecondaryTab === WorkspaceTab.CONNECTED_PROJECTS && query && parseQuerySimple(query).rciid != null) {
      ...
      case 'search':
        {
          ...
          queryParams = {
            ...
            ...(searchParams.fileTypeFilter && {
              file_type: searchParams.fileTypeFilter
            })
          };
  ...
  selectedViewName(view: any, state: any): string | null {
    ...
    switch (view.view) {
      ...
      case 'user':
        ...
        if (view.userViewTab === InterProfileType.INTERNAL_PROFILE || view.userViewTab === InterProfileType.INTERNAL_PROFILE_POSTS) {
