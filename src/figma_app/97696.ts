...
export function $$l2(e) {
  let t = e?.filter(e => e.type !== K$p.TOOL_MESSAGE);
  return t ? t.reduce((e, t) => {
    let r = t.type === K$p.ASSISTANT_MESSAGE;
    let a = t.toolCalls.length > 0;
    let o = 0 === H5(t.textContent || "").plainText.length;
    if (r && a && o) {
      let r = e[e.length - 1];
      r?.type === K$p.ASSISTANT_MESSAGE && (r.toolCalls = s()([...r.toolCalls, ...t.toolCalls], "toolCallId"));
      return e;
    }
    return [...e, t];
  }, []) : [];
...
export function $$c0(e) {
  let t = e.slice().reverse().find(e => e.type === K$p.USER_MESSAGE);
  return t ? t.id : null;
}
