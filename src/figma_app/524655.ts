...
export function $$I7(e, t, r, n, i) {
  ...
  }), d.every(e => 0 === e.children.length)) ? (AppStateTsApi?.canvasGrid().setAreChildrenManuallyIndented(!1), [d.map(e => e.guid)]) : (1 === d.length && AppStateTsApi?.canvasGrid().setAreChildrenManuallyIndented(!0), d.map(e => [e.guid, ...e.children.map(e => e.guid)]));
}
export function $$S10(e, t) {
  if (!AppStateTsApi || !SceneGraphHelpers) return "";
  let r = AppStateTsApi.singleSlideView().focusedNodeId.getCopy();
  let {
    row,
    col
  } = AppStateTsApi.getInsertGridCoord({
    x: -1 / 0,
    y: -1 / 0
  });
  let s = Sz(row, col, t, !1);
  if (!s) return "";
  AppStateTsApi.singleSlideView()?.isFocusedNodeViewEnabled() && AppStateTsApi.singleSlideView().focusNodeInFocusedNodeView(s, !0);
  let o = e.get(s);
  let l = e.get(r);
  o && l?.sourceLibraryKey && (o.sourceLibraryKey = l.sourceLibraryKey);
  SceneGraphHelpers.replaceSelection([s], !0);
  return s;
}
...
export function $$C6() {
  return {
    width: (SlideConstantsCppBindings?.singleSlideThumbnailWidth() ?? 0) * 2,
    height: (SlideConstantsCppBindings?.singleSlideThumbnailHeight() ?? 0) * 2
  };
}
export const $u = $$x0;
export const Cd = $$N1;
