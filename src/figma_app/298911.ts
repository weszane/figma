...
const defaultNavigationConfig = {
  ...
  animationTimingFunction: (progress: number) =>
...
interface ViewportInterface {
  ...
  getCommentsWrapperOffset: (e: any) => { x: number, y: number }
...
class ViewportNavigator {
   ...
   */
  getCommentDestinationForCanvasPosition = (e: any, t: any, r: any) =>
    this.viewport.getCommentDestinationForCanvasPosition(e, t, r)
  ...
  private shouldSkipAnimation(options: NavigationOptions): boolean {
    return reducedMotionMediaQuery.matches || nl() || options.duration === 0
  }
   ...
   */
  private performAnimatedNavigation(
    initialViewport: any,
    targetPosition: any,
    targetZoom: number,
    ...
    const completeAnimation = (cancelled: boolean) => {
      ...
      cleanupCancellation()
      deferredPromise.resolve({ cancelled })
    }
  ...
  private shouldCancelAnimation(lastViewport: any, cancelled: boolean, options: NavigationOptions): boolean | undefined {
    const currentViewport = this.getViewportInfo()
    const viewportChanged = this.config.isGetViewportAtomic && (
      lastViewport.offsetX !== currentViewport.offsetX
      || lastViewport.offsetY !== currentViewport.offsetY
      || lastViewport.zoomScale !== currentViewport.zoomScale
    )
   ...
   */
  private performFrameUpdate(
    initialViewport: any,
    zoomDelta: number,
    initialScaledOffset: any,
    positionDelta: any,
    progress: number,
  ): void {
    const currentViewport = this.getViewportInfo()
  ...
  private reportAnimationMetrics(frameTimings: number[], cancelled: boolean, options: NavigationOptions): void {
    ...
    if (frameTimings.length) {
      averageFrameDuration = Math.round(
        frameTimings.reduce((sum, duration, index) => {
  ...
  private reportReducedMotionMetrics(options: NavigationOptions): void {
    if (reducedMotionMediaQuery.matches && options.metricsCallback) {
      options.metricsCallback({ prefersReducedMotion: true })
    }
  }
  ...
  private reportInstantNavigationMetrics(options: NavigationOptions): void {
    options.metricsCallback?.({ prefersReducedMotion: false })
  }
...
const defaultViewportNavigator = new ViewportNavigator({
  ...
  getCommentsWrapperOffset: () => ({ x: 0, y: 0 }),
...
const viewportNavigatorContext = createContext(defaultViewportNavigator) // Originally $$h11
const viewportXContext = createContext(defaultViewportState.x) // Originally $$m10
const viewportYContext = createContext(defaultViewportState.y) // Originally $$g13
const viewportWidthContext = createContext(defaultViewportState.width) // Originally $$f1
const viewportHeightContext = createContext(defaultViewportState.height) // Originally $$E16
const viewportOffsetXContext = createContext(defaultViewportState.offsetX) // Originally $$y9
const viewportOffsetYContext = createContext(defaultViewportState.offsetY) // Originally $$b15
const viewportZoomContext = createContext(defaultViewportState.zoomScale) // Originally $$T17
const viewportPanningContext = createContext(defaultViewportState.isPanning) // Originally $$I12
const viewportZoomingContext = createContext(defaultViewportState.isZooming) // Originally $$S6
const frameCountContext = createContext(0) // Originally $$v7
const activeStateContext = createContext(false) // Originally $$A4
const currentViewportRefContext = createContext({ current: defaultViewportState }) // Originally $$x0
const previousViewportRefContext = createContext({ current: defaultViewportState }) // Originally $$N3
