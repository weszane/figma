...
let $$u1 = nF(e => {
  glU?.setIsInCursorChat(!1);
  e.dispatch($$c2());
});
let $$p9 = NC("MULTIPLAYER_EMOJI_START_CHATTING");
let $$_0 = nF((e, t) => {
  let r = e.getState();
  0 > [NLJ.HAND, NLJ.SELECT, NLJ.HAND_SELECT].indexOf(r.mirror.appModel.currentTool) && glU?.triggerAction("set-tool-default", null);
  q().then(e => {
    e || (glU?.setIsInCursorChat(!0), ds("Started chatting", r.openFile?.key, r, {
      source: t.source
    }));
  });
  e.dispatch($$p9(t));
