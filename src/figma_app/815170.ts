...
import { setActiveComment } from '../figma_app/770088';
      ...
      if (node) {
        ...
        if (isDevHandoffFullscreen && state.mirror.appModel.currentPage === Fullscreen?.getPageIdFromNode(node.guid)) {
          ...
          if (isSlidesFullscreen && AppStateTsApi) {
            ...
            slideView.isFocusedNodeViewEnabled() ? slideView.focusNodeInFocusedNodeView(nodeIdInThisFile, true) : slideView.panToNode(nodeIdInThisFile, 0);
          }
        ...
        if (commentThreadId) {
          requestAnimationFrame(() => {
            dispatchApi.dispatch(setActiveComment({
  ...
  if (desktopAPIInstance) {
    if (nextView && nextView.view === 'fullscreen') {
      ...
      if (nextView.versionId) {
        dispatchApi.dispatch(setActiveVersion({
    ...
    if (nextView && nextView.view === 'prototype') {
      dispatchApi.dispatch(openUrlInContext({
        url
      }));
      return;
    }
