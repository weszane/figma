...
import { eD, hX } from "../figma_app/876459";
...
export let $$T0 = nF(function (e, {
  ...
  let I = "fullscreen" === y.selectedView.view && y.selectedView.editorType === nT.Slides;
  let S = "fullscreen" === y.selectedView.view && y.selectedView.editorType === nT.Sites;
  let {
    url,
    nodeIdInThisFile,
    commentThreadId,
  ...
  eD || hX || ce() ? Ay.unsafeRedirect(url, "_blank") : Ay.postRedirect(`/exit?url=${encodeURIComponent(url)}`, "_blank");
  sx("Hyperlink Click", R);
});
export const RK = $$T0;