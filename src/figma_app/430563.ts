...
let $$I0 = createOptimistThunk((e, {
  ...
  if (!r || isSubscribedLibrary(e.getState().library.defaultPublished, t))
    return
  let n = getSelectedFile(e.getState())
  if (n) {
    let a = n.editor_type ?? 'design';
    (!0 === r || a === 'design' && r.design || a === 'whiteboard' && r.figjam) && (e.getState().leftPanel.activeTab === UserInterfaceElements.ASSETS || e.getState().universalInsertModal.showing
...
let $$S3 = createOptimistThunk(async (e, t) => {
  ...
  })), fileSubscribedLibraryKeys.has(library_key) && is_subscribed || !userInitiated) {
    return
  }
  ...
  id
    ...
    : is_subscribed && publishedLibrary && WB()?.optimisticallyCreate({
      LibraryFileSubscription: {
...
let $$C2 = createOptimistThunk(async (e, t) => {
  ...
  if (e.dispatch($$I0({
    ...
    if (t.currentSubscription) {
      ...
      !(t.currentSubscription?.design || t.currentSubscription?.figjam) && e
