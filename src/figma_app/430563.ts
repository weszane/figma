 ...
 */
export const handleOptimisticLibrarySubscription = createOptimistThunk((e, {
  libraryKey,
  subscriptions
}) => {
  if (!subscriptions || isSubscribedLibrary(e.getState().library.defaultPublished, libraryKey)) {
    return;
  }
  const selectedFile = getSelectedFile(e.getState());
  if (selectedFile) {
    const editorType = selectedFile.editor_type ?? 'design';
    const shouldUpdate = subscriptions === true || editorType === 'design' && subscriptions.design || editorType === 'whiteboard' && subscriptions.figjam;
    if (shouldUpdate) {
      const state = e.getState();
      if (state.leftPanel.activeTab === UserInterfaceElements.ASSETS || state.universalInsertModal.showing) {
        e.dispatch(getPublishedComponentsForLibraryThunk({
          libraryKey
        }));
      } else {
 ...
 */
export const handleFileLibrarySubscription = createOptimistThunk(async (e, t) => {
  const {
  ...
  if (openFileKey === file_key) {
    e.dispatch(handleOptimisticLibrarySubscription({
      libraryKey: library_key,
      subscriptions: is_subscribed
    }));
  }
  if (fileSubscribedLibraryKeys.has(library_key) && is_subscribed || !userInitiated) {
    return;
  }
 ...
 */
export const handleUserLibrarySubscription = createOptimistThunk(async (e, t) => {
  const {
  ...
  updateSubscriptionOptimistically(existingSubscription, LibrarySubscriptionType.USER, subscriptions.design, subscriptions.figjam, subscriptions.slides, subscriptions.buzz, optimisticPayload, postPromise);
  try {
    if (!library_key) throw new Error('Library key is required for user subscription');
    await postPromise;
 ...
 */
export const handleTeamLibrarySubscription = createOptimistThunk(async (e, t) => {
  const {
  ...
  updateSubscriptionOptimistically(existingSubscription, LibrarySubscriptionType.TEAM, subscriptions.design, subscriptions.figjam, subscriptions.slides, subscriptions.buzz, {
    libraryKey: library_key,
    teamId: team_id,
    library: {
      libraryKey: library_key,
  ...
  try {
    if (!library_key) throw new Error('Team subscription requires a library key');
    await postPromise;
    analyticsEventManager.trackDefinedMetric('library_preferences_modal.library_subscription_toggle.outcome', {
      designSubscribed: subscriptions.design,
      figjamSubscribed: subscriptions.figjam,
 ...
 */
export const handleWorkspaceLibrarySubscription = createOptimistThunk(async (e, t) => {
  const {
  ...
  try {
    ...
    e.dispatch(VisualBellActions.enqueue({
      message: err?.message ?? getI18nString('design_systems.subscriptions.unable_to_change_setting'),
      error: true
    }));
  }
});
 ...
 * @param params - Contains libraryWorkspaceSubscription, libraryName, workspaceName
 */
export const handleDeleteWorkspaceLibrarySubscription = createOptimistThunk(async (e, t) => {
  const {
    libraryWorkspaceSubscription,
    libraryName,
    workspaceName
 ...
 */
export const handleOrgLibrarySubscription = createOptimistThunk(async (e, t) => {
  const {
  ...
  if (userInitiated) {
    if (currentSubscription) {
      ...
      if (!(currentSubscription?.design || currentSubscription?.figjam) && designEnabled) {
