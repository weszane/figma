...
let u = atom(null, (e, t, r) => {
  ...
  if (r.parentNodeId && !n[r.parentNodeId]) {
    logWarning("scene node overlay", `trying to add child overlay to ${r.parentNodeId} but parent overlay does not exist`);
    return;
  }
  t(c, e => ({
    ...e,
...
let _ = atom(e => Object.keys(e(c)).length > 0);
let h = createRemovableAtomFamily(e => atom(t => Object.values(t(c)).filter(({
  parentNodeId: t
}) => t === e)));
let m = atom(e => Object.values(e(c)).filter(({
  parentNodeId: e
}) => !e));
...
function b({
  ...
  let o = useAtomWithSubscription(h(t.nodeId));
