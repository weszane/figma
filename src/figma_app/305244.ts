...
import { UpdateVisibilityStatus } from '../905/687477';
...
export const websocketManager = {
  ...
  connect(url: string): void {
    ...
    if (preconnectSock && areUrlsEquivalent(preconnectSock.url, url) && (preconnectSock.readyState === WebSocket.CONNECTING || preconnectSock.readyState === WebSocket.OPEN)) {
      ...
      if (preconnectSock) {
        ...
        if (preconnectSock.readyState !== WebSocket.CONNECTING && preconnectSock.readyState !== WebSocket.OPEN) {
          ...
          if (preconnectSock.readyState === WebSocket.CLOSED) readyStateStr = 'closed';else if (preconnectSock.readyState === WebSocket.CLOSING) readyStateStr = 'closing';
    ...
    websocket.onerror = () => {
      if (navigator.onLine) consecutiveFailureCount++;else consecutiveFailureCount = 0;
