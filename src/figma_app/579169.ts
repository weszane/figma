...
import { _s, rg, ZS } from "../figma_app/33126"
...
let $$_9 = mapAndAggregateResources([UserForRcs.Query({}), ZS], ([e, t]) => e.currentUser.id != null && t?.canRead ? OrgPersonal.ORG : OrgPersonal.PERSONAL)
...
let $$g1 = mapAndAggregateResources([rg, UserForRcs.Query({})], ([e, t], r) => {
  let n = r(_s)
  if (!n)
    return t.currentUser.draftsFolderId
  {
...
let $$f15 = transformAtom(UserForRcs.Query({}), e => e.currentUser.studentValidatedAt != null)
...
let $$b7 = transformAtom($$y8, e => e?.map(e => e.team).filter(e => e != null))
let T = createReduxSubscriptionAtomWithState(e => e.isFreeUser)
let I = mapAndAggregateResources([rg, $$b7], ([e, t]) => (!e?.length || !(e.length > 0)) && (t == null || !t.some(e => e.canEdit && hasTeamPaidAccess({
...
let $$L5 = mapAndAggregateResources([UserForRcs.Query({}), ZS], ([e, t]) => {
  let r = e.currentUser.externalRestrictedOrgId.status === "loaded" ? e.currentUser.externalRestrictedOrgId.data : null
