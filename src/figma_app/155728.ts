...
import { f as isSlidesEditorType } from '../figma_app/252485'
...
export function useSubscribedLibraries() {
  ...
  const { subscription, ensureSubscription } = assertNotNullish(
    context,
    'Must call `useSubscribedLibraries` from within <SubscribedLibrariesContext>',
  )
  useEffect(() => {
    ensureSubscription()
  }, [ensureSubscription])
...
export function useUntransformedSubscribedLibraries() {
  const context = useContext(SubscribedLibrariesContext)
  const { untransformedSubscription, ensureSubscription } = assertNotNullish(
    context,
    'Must call `useSubscribedLibraries` from within <SubscribedLibrariesContext>',
  )
  useEffect(() => {
...
export function getGroupedSubscribedLibraryKeys() {
  const context = useContext(SubscribedLibrariesContext)
  const { untransformedSubscription } = assertNotNullish(
    context,
    'Must call `useSubscribedLibraries` from within <SubscribedLibrariesContext>',
  )
  if (untransformedSubscription.status !== 'loaded')
    return {}
  const result: Record<string, Set<string>> = {}
  const { file } = untransformedSubscription.data
 ...
 */
export function SubscribedLibrariesProvider({ children }: { children: React.ReactNode }) {
  const file = selectCurrentFile()
  const hasTeam = !!file?.teamId
  const hasProjectRoot = hasRootPath(file?.project)
  useAtomWithSubscription(LibraryPresetSubscriptionsV2.Query({ group: getProviderConfigType() }))
  ...
  const isSlides = isSlidesEditorType(file?.editorType ?? null)
  ...
  const editorType = useMemo(
    () =>
      isSlides
        ...
        : isCooper
          ? 'cooper'
          : file?.editorType === 'whiteboard'
            ? 'whiteboard'
            : 'design',
    [isCooper, isSlides, file?.editorType],
  )
   ...
   */
  const finalSubscription = useTransformSubscription(
  ...
  useEffect(() => {
    if (finalSubscription.status === 'loaded')
      setAtom(finalSubscription.data)
  }, [finalSubscription, setAtom])

  return jsxs(Fragment, {
    children: [
      jsx(SubscribedLibrariesContext.Provider, {
        value: contextValue,
        children,
      }),
      enabled && jsx(SubscriptionAnalytics, { subscription: finalSubscription }),
    ],
  })
}
 ...
 */
function SubscriptionAnalytics({ subscription }: { subscription: any }) {
...
function transformFigJamSubscription(subscription: any) {
  ...
  const transformedFile = {
    ...
    currentUser: file.currentUser
      ? {
          ...
          orgAwareTeamRoles: file.currentUser.orgAwareTeamRoles.map((role) => {
            const teamSubscriptions = getResourceDataOrFallback(
              role.team?.libraryTeamSubscriptionsForFigJam,
            )
            return {
              ...role,
              team:
                role.team && teamSubscriptions
                  ? { ...role.team, libraryTeamSubscriptions: teamSubscriptions }
                  : null,
            }
          }),
        }
    ...
    libraryOrgSubscriptions:
      file.libraryOrgSubscriptionsForFigJam.status === 'loaded'
        ? file.libraryOrgSubscriptionsForFigJam.data
        : [],
    libraryTeamSubscriptions:
      file.libraryTeamSubscriptionsForFigJam.status === 'loaded'
        ? file.libraryTeamSubscriptionsForFigJam.data
        : [],
    libraryUserSubscriptions:
      file.libraryUserSubscriptionsForFigJam.status === 'loaded'
        ? file.libraryUserSubscriptionsForFigJam.data
        : [],
  }
  ...
  const transformedFile = {
    ...
    currentUser: file.currentUser
      ? {
          ...
          orgAwareTeamRoles: file.currentUser.orgAwareTeamRoles.map((role) => {
            const teamSubscriptions = getResourceDataOrFallback(
              role.team?.libraryTeamSubscriptionsForSlides,
            )
            return {
              ...role,
              team:
      ...
      : null,
    libraryOrgSubscriptions:
      file.libraryOrgSubscriptionsForSlides.status === 'loaded'
        ? file.libraryOrgSubscriptionsForSlides.data
        : [],
    libraryTeamSubscriptions:
      file.libraryTeamSubscriptionsForSlides.status === 'loaded'
        ? file.libraryTeamSubscriptionsForSlides.data
        : [],
    libraryUserSubscriptions:
      file.libraryUserSubscriptionsForSlides.status === 'loaded'
        ? file.libraryUserSubscriptionsForSlides.data
        : [],
  }
  transformedFile.computedWorkspacePublicInfo?.workspace?.librarySubscriptions?.forEach((sub) => {
  ...
  const transformedFile = {
    ...
    currentUser: file.currentUser
      ? {
          ...
          orgAwareTeamRoles: file.currentUser.orgAwareTeamRoles.map((role) => {
            const teamSubscriptions = getResourceDataOrFallback(
              role.team?.libraryTeamSubscriptionsForBuzz,
            )
            return {
              ...role,
              team:
      ...
      : null,
    libraryOrgSubscriptions:
      file.libraryOrgSubscriptionsForBuzz.status === 'loaded'
        ? file.libraryOrgSubscriptionsForBuzz.data
        : [],
    libraryTeamSubscriptions:
      file.libraryTeamSubscriptionsForBuzz.status === 'loaded'
        ? file.libraryTeamSubscriptionsForBuzz.data
        : [],
    libraryUserSubscriptions:
      file.libraryUserSubscriptionsForBuzz.status === 'loaded'
        ? file.libraryUserSubscriptionsForBuzz.data
        : [],
  }
  transformedFile.computedWorkspacePublicInfo?.workspace?.librarySubscriptions?.forEach((sub) => {
    sub.isSubscribed
      = sub.buzzSubscribed.status === 'loaded' && sub.buzzSubscribed.data
        ? sub.buzzSubscribed.data
        : false
  })
  return createLoadedState({
    file: transformedFile,
 ...
 */
export function useTransformSubscription(
  subscription: any,
  hasTeam: boolean,
 ...
 */
function transformAndFilterSubscriptions(
  subscription: any,
  hasTeam: boolean,
  hasProjectRoot: boolean,
  figmaLibrariesEnabled: boolean,
  parentOrgId: string | null,
  isProOrStudent: boolean,
) {
  if (
    subscription.status === 'loading'
    || subscription.status === 'errors'
  ...
  let libraries: any[] = [
    ...orgSubscriptions
      .filter(sub => figmaLibrariesEnabled || !sub.communityLibrary)
      .filter(sub =>
        hasProjectRoot
          ? (sub.libraryFileSubscriptionOverride || sub.libraryUserSubscriptionOverride) == null
          : (sub.libraryFileSubscriptionOverride
            || sub.libraryWorkspaceSubscriptionOverride
            || sub.libraryTeamSubscriptionOverride) == null,
      )
      .map(sub => mapLibrarySubscription(sub, LibrarySubscriptionType.ORGANIZATION)),
    ...fileSubscriptions
      .filter(sub => figmaLibrariesEnabled || !sub.communityLibrary)
      .map(sub => mapLibrarySubscription(sub, LibrarySubscriptionType.FILE)),
  ]
        ...
        .filter((preset) => {
          ...
          return (
            (hasProjectRoot
              ? !(
                  file?.parentOrgId
                  && preset.libraryOrgSubscriptionOverride != null
                )
                && !(
                  file?.teamId
                  && preset.libraryTeamSubscriptionOverride != null
                )
                && (preset.libraryFileSubscriptionOverride
                  || preset.libraryUserSubscriptionOverride) == null
              : (preset.libraryOrgSubscriptionOverride
                || preset.libraryWorkspaceSubscriptionOverride
                || preset.libraryTeamSubscriptionOverride
                || preset.libraryFileSubscriptionOverride) == null) && defaultSubscribed
          )
        })
  ...
  if (hasProjectRoot) {
    const userLibs = userSubscriptions
      .filter(sub => figmaLibrariesEnabled || !sub.communityLibrary)
      .filter(sub =>
        sub.communityLibrary
          ? sub.libraryFileSubscriptionOverride == null
          : sub.libraryFileSubscriptionOverride == null
            && sub.library?.file?.parentOrgId === parentOrgId,
      )
  ...
  if (hasTeam) {
    const teamLibs = teamSubscriptions
      .filter(sub => figmaLibrariesEnabled || !sub.communityLibrary)
      .filter(isTeamSubscriptionValid)
      .map(sub => mapLibrarySubscription(sub, LibrarySubscriptionType.TEAM))
    const workspaceLibs
      = (workspaceSubscriptions || [])
        ...
        .map(sub => mapLibrarySubscription(sub, LibrarySubscriptionType.WORKSPACE)) || []
    libraries.push(...teamLibs)
    libraries.push(...workspaceLibs)
  }
  else {
    const teamRoles
      = getTeamRolesFromFile(file)?.map(teamLibs =>
        teamLibs
          .filter(sub => figmaLibrariesEnabled || !sub.communityLibrary)
          .filter(isUserSubscriptionValid)
          .map(sub => mapLibrarySubscription(sub, LibrarySubscriptionType.TEAM)),
      ) ?? []
    libraries.push(...flatten(teamRoles))
  }

  const uniqueLibraries = uniqBy(
    libraries.filter(isNotNullish),
    lib => lib.libraryKey,
  ).filter(lib => !matchesLibraryKey(file, lib.libraryKey))
 ...
 */
export function mapLibrarySubscription(
  subscription: any,
  type: LibrarySubscriptionType,
): LibraryInfo | null {
 ...
 */
function mapCommunityLibraryPreset(
  preset: any,
  type: LibrarySubscriptionType,
): LibraryInfo | null {
...
function getTeamRolesFromFile(file: any) {
  const teamId = file?.teamId
  if (teamId) {
    return file?.currentUser?.orgAwareTeamRoles
      ?.filter(role => role.team?.id === teamId)
      .map(role => role.team?.libraryTeamSubscriptions)
      .filter(isNotNullish)
  }
  return file?.currentUser?.orgAwareTeamRoles
    ?.map(role => role.team?.libraryTeamSubscriptions)
