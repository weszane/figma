...
import { Fi, Mq, WZ, zF } from "../figma_app/297822";
import { getProductType } from "../figma_app/314264";
import { Ig } from "../figma_app/350332";
import { QUICK_ACTIONS_DROPDOWN } from "../figma_app/357047";
import { Bu, dd, Lk, Q8, rE, YH, ZG } from "../figma_app/604494";
import { JT } from "../figma_app/632248";
import { mP, U3 } from "../figma_app/737746";
import { AppStateTsApi, InteractionCpp } from "../figma_app/763686";
...
export function $$k6() {
  ...
  if (!e) throw new Error("Session ID is missing when opening a module in Quick Actions V2.");
  return {
    sessionId: e,
...
export function $$M0({
  ...
  if (isUIHiddenOrLocked(debugState.getState().progressBarState.mode) || debugState.getState().selectedView.view !== "fullscreen") return;
  let n = $$W9(t);
  Fi({
    isQAV2: n,
    moduleToOpen: t
  ...
  if (atomStoreManager.set(zF, e.source), atomStoreManager.set(rE, s), n) {
    if (atomStoreManager.set(Bu, !0), atomStoreManager.set(oh, {
      ...
      t.type === "tab" ? (atomStoreManager.set(Lk, t.module), t.filter && atomStoreManager.set(_$$t, t.filter)) : atomStoreManager.set(YH, e => [{
        module: t.module,
        name: t.name
      }, ...e]);
    }
...
export function $$F12(e, t, r) {
  if (atomStoreManager.get(Bu)) {
    ...
    t?.type === QUICK_ACTIONS_DROPDOWN && e(hideDropdownAction());
  }
  return !0;
}
export function $$j1(e) {
  return atomStoreManager.get(Bu) || e?.type === QUICK_ACTIONS_DROPDOWN;
}
function U(e) {
