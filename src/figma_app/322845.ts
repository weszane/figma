...
export function $$k6() {
  ...
  if (!e)
    throw new Error("Session ID is missing when opening a module in Quick Actions V2.")
  return {
    sessionId: e,
...
export function $$M0({
  ...
  if (isUIHiddenOrLocked(debugState.getState().progressBarState.mode) || debugState.getState().selectedView.view !== "fullscreen")
    return
  let n = $$W9(t)
  Fi({
    isQAV2: n,
  ...
  if (atomStoreManager.set(zF, e.source), atomStoreManager.set(rE, s), n) {
    if (atomStoreManager.set(Bu, !0), atomStoreManager.set(oh, {
      ...
      t.type === "tab"
        ? (atomStoreManager.set(Lk, t.module), t.filter && atomStoreManager.set(_$$t, t.filter))
        : atomStoreManager.set(YH, e => [{
            module: t.module,
            name: t.name,
          }, ...e])
    }
...
export function $$F12(e, t, r) {
  if (atomStoreManager.get(Bu)) {
    if (!r.forceClose && atomStoreManager.get(ZG))
      return !1
    ...
    t?.type === jv && e(hideDropdownAction())
  }
  return !0
}
...
export function $$H2(e = {}) {
  ...
  if (atomStoreManager.get(P)) {
    if (actionToClear && actionToClear !== atomStoreManager.get(P)?.action)
      return
