...
export class $$_3 {
  ...
  onParseThrow(e, t) {
    if (e instanceof $$p5 && e.errorType)
      return this.options.onEvaluateExpressionError?.(e.errorType, t)
  }
  ...
  preProcessForExpressionEvaluation(e) {
    return e.replace(new RegExp(this.allowedUnits, 'g'), '').replace(/[\u0660-\u0669]/g, e => '\u0660\u0661\u0662\u0663\u0664\u0665\u0666\u0667\u0668\u0669'.indexOf(e).toString()).replace(/[\u06F0-\u06F9]/g, e => '\u06F0\u06F1\u06F2\u06F3\u06F4\u06F5\u06F6\u06F7\u06F8\u06F9'.indexOf(e).toString()).replace(/\u066B/g, ',').replace(/\u066C/g, ',').replace(/\u060C/g, ',')
  }
}
export class $$h6 extends $$_3 {
  ...
  parse(e, t) {
    /\d+,\d+/.test(e) && trackEventAnalytics('editor-parsing-float-input-with-comma', {
      input: e,
    })
...
export class $$f7 {
  ...
  parse(e, t) {
    ...
    let n = e.match(r)
    return n
      ? n.filter(e => e.length).map((e, r) => {
          let n = this.pixelFormatter.parse(e, t && t[r])
          if (!isFinite(n))
            throw new Error('could not parse array input')
          return n
        })
      : []
  }
  ...
  format(e) {
    return e == null ? '' : allEqual(e) ? this.pixelFormatter.format(e[0]) : e.map(e => this.pixelFormatter.format(e)).join(', ')
  }
...
export class $$b10 extends $$_3 {
  ...
  parse(e, t) {
    ...
    return super.parse(e, t) * (r === 0 ? 1 : 0.001)
  }
...
export class $$T12 extends $$_3 {
  ...
  parse(e, t) {
    if (e.includes(':')) {
      let r = e.split(':')
      if (r.length > 3)
        throw new Error('Format must match HH:MM:SS')
      ...
      if (d || c)
        throw new Error('Cannot have fractional parts in timestamp')
...
export class $$I4 extends $$_3 {
  constructor({
    ...
    e != null && (this.decimals = e)
    t != null && (this.hidePercentSign = t)
  }
  ...
  parse(e, t) {
    return super.parse(e, t != null ? 100 * t : void 0) / 100
  }
...
export class $$v19 extends $$_3 {
  ...
  format(e) {
    return e == null ? '' : `${parseFloat(e.toFixed(2))}`
  }
}
...
export class $$N11 extends $$_3 {
  ...
  format(e) {
    return e == null ? '' : ((e = (e + 180 + 360) % 360 - 180) == -180 && (e = 180), `${parseFloat(e.toFixed(2))}\xB0`)
  }
}
...
export class $$O13 {
  ...
  static parse(e) {
    let t = e.includes('%') ? 'PERCENT' : e.includes('px') ? 'PIXELS' : void 0
    e = (e = e.toLowerCase()).replace(new RegExp('%|px', 'g'), '')
    let r = O4(e, NaN)
    if (!r.error)
      return [r.value, t]
  }
  ...
  format(e) {
    ...
    return e.units === 'PERCENT' ? `${t}%` : e.units === 'RAW' ? `${100 * t}%` : `${t}px`
  }

  getNudgeAmount(e, t) {
    return e ? t.units === 'PIXELS' ? this.bigPixelNudgeAmount : this.bigPercentageNudgeAmount : t.units === 'PIXELS' ? this.smallPixelNudgeAmount : this.smallPercentageNudgeAmount
  }
...
export class $$L1 extends $$O13 {
  ...
  parse(e, t) {
    return this.allowEmpty && e === '' || e.toLowerCase() === 'auto' ? this.autoValue : super.parse(e, t)
  }

  format(e) {
    return e == null || isNaN(e.value) ? '' : this.isAuto(e) ? getI18nString('fullscreen.auto') : super.format(e)
  }
...
export class $$P17 {
  ...
  parse(e) {
    ...
    let r = O4(e)
    if (r.error)
      throw new $$p5('Could not parse input', r.error.type)
    return {
      value: r.value,
      type: t,
    }
  ...
  clamp(e) {
    let t = e.value;
    (t = Math.min(t, e.type === 'CONTENT_SCALE' ? this.MAX_SCALE : this.MAX_SIZE)) <= 0 && (t = 1)
    return {
      value: t,
      type: e.type,
    }
