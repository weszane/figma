...
import { Component, createRef, forwardRef, PureComponent, useCallback, useEffect, useMemo, useRef, useState } from "react";
import { useDispatch, useSelector } from "react-redux";
import { Fragment, jsx, jsxs } from "react/jsx-runtime";
import { kiwiCodec } from "../905/5147";
import { m as SVG } from "../905/80904";
import { KindEnum } from "../905/129884";
import { e as SVG2 } from "../905/149844";
import { initializePrototypeLibModule } from "../905/197794";
import { convertToRgba } from "../905/222694";
import { loadVideoJs } from "../905/284552";
import { v as SVG1 } from "../905/293028";
import { VisualBellActions } from "../905/302958";
import { getI18nString, renderI18nText } from "../905/303541";
import { O as SVG3 } from "../905/365108";
import { H as SVG4 } from "../905/404982";
import { IconButton } from "../905/443068";
import { analyticsEventManager, trackEventAnalytics } from "../905/449184";
import { O as SVG5 } from "../905/487602";
import { handleExternalRedirect } from "../905/508367";
import { registerTooltip } from "../905/524523";
import { getFeatureFlags } from "../905/601108";
import { V as SVG6 } from "../905/604512";
...
import { A as Svg } from "../6828/154709";
...
registerTooltip("preload_success", () => {
  return jsxs("div", {
...
registerTooltip("preload_error", ({
  ...
  isSlides: e.getAttribute("data-tooltip-slides") === "true"
}));
...
(e => {
  ...
  e.GetFreePreviewButton = function (e) {
    let t = useDuplicateTemplateHandler(e.hubFile, !0);
    return jsx(TrackingProvider, {
      name: TrackingKeyEnum.COMMUNITY_HUB_FILE_GET_FREE_PREVIEW_BUTTON,
  ...
  function n(e, t, r, n) {
    let i = useCallback(r => n => {
      ...
      t(2, DEFAULT_ANIMATION_DURATION);
    }, [e, t]);
    return {
      ...
      adjustZoomOutFactory: useCallback(r => n => {
        ...
        t(0.5, DEFAULT_ANIMATION_DURATION);
      }, [e, t]),
      setZoomFactory: useCallback((t, n) => i => {
        ...
        r(t, DEFAULT_ANIMATION_DURATION);
      }, [e, r]),
  ...
  e.HubFileZoomButtonDropdown = function ({
    ...
    let h = useCallback(e => {
      document.activeElement instanceof HTMLInputElement || (e.key === "+" ? adjustZoomInFactory("keyboardShortcut")() : e.key === "-" ? adjustZoomOutFactory("keyboardShortcut")() : e.key === "0" ? setZoomFactory(1, "keyboardShortcut")() : e.key === "1" && resetZoomToFitCanvasContentFactory("keyboardShortcut")());
    }, [adjustZoomInFactory, adjustZoomOutFactory, resetZoomToFitCanvasContentFactory, setZoomFactory]);
    if (useWindowEvent("keydown", h), !useSelector(e => e.dropdownShown?.type === s)) return null;
    let m = t => {
  ...
  e.PrototypeZoomDropdown = function (e) {
    ...
    let [o, l] = useState(() => e.initialScalingMode ?? {
      viewportScalingMode: getContentScalingMode(s),
    ...
    let c = useCallback(e => {
      if (e.key === "z" || e.key === "Z") {
        ...
        let r = generateScalingOptions(n);
        let i = findMatchingScalingOptionId(o, r);
        let a = r.findIndex(e => e.scalingOptionId === i);
        a === -1 ? a = e.shiftKey ? r.length - 1 : 0 : e.shiftKey ? a === 0 ? a = r.length - 1 : a-- : a === r.length - 1 ? a = 0 : a++;
        let s = r[a];
        d(s);
        t(VisualBellActions.clearAll());
        t(VisualBellActions.enqueue({
          message: getI18nString("community.hub_file_viewer.zoom", {
            percent: getScalingOptionLabel(s.scalingOptionId)
          })
        }));
      }
    }, [t, n, o, d]);
    return (useWindowEvent("keydown", c), r) ? jsx(_$$j, {
      showPoint: !1,
      dispatch: t,
      onSelectItem: e => e.callback?.(),
      parentRect: e.targetRect,
      items: function (e, t, r, n, a, s, o) {
        let l = generateScalingOptions(a, r);
        let d = findMatchingScalingOptionId(t, l);
        ...
        let p = r => function ({
          ...
          let l = findMatchingScalingOptionId(t, [e]);
          return {
            displayText: getScalingOptionLabel(e.scalingOptionId, o),
            isChecked: l === e.scalingOptionId,
            callback: () => r ? r(e) : void s(handlePrototypeScaleChange({
              ...e.scalingInfo,
              userInitiated: !0,
              source: "presentation"
            })),
          ...
          recordingKey: generateRecordingKey({
            recordingKey: n
          }, `option-${getViewportScalingMode(r.scalingOptionId)}`),
          isFooter: void 0
        });
        if (!(r !== PrototypeViewMode.DEFAULT && l.length >= 4)) return l.map(e => p(e));
        {
      ...
      }(t, o, PrototypeViewMode.DEFAULT, void 0, n, d),
...
let eT = e => {
  let t = useCallback((t, r = DEFAULT_ANIMATION_DURATION) => {
    e?.port1.postMessage({
      type: R.ADJUST_ZOOM,
      params: {
        scale: t,
  ...
  let r = useCallback((t, r = DEFAULT_ANIMATION_DURATION) => {
    e?.port1.postMessage({
      type: R.SET_ZOOM,
      params: {
        scale: t,
  ...
  return {
    ...
    setCanvasSpaceCenter: useCallback((t, r = DEFAULT_ANIMATION_DURATION) => {
      e?.port1.postMessage({
...
let eS = (e, t) => {
  ...
  useEffect(() => {
    ...
    let a = t => {
      ...
      t.pages && (n.length === 0 && t.pages[0]?.id && o(t.pages[0].id), i(t.pages));
...
let ev = (e, t) => {
  ...
  let o = useCallback(e => {
    if (e.data.canvasInteraction) {
      ...
      t === "canvasMouseDown" ? onMouseDown(a) : t === "canvasMouseUp" ? onMouseUp(a) : (t === "canvasMouseDrag" || t === "canvasMouseMove") && onMouseMove(a);
    } else {
      e.data.keydown && onKeydown(e.data.keydown.key);
    }
...
function ex(e) {
  ...
  useEffect(() => {
    let e = e => {
      e.data === "LOAD_FINISHED" && (j(), U(!!isFullscreen), p(!0), onLoad?.());
    };
...
function eN(e) {
  ...
  return jsxs(Fragment, {
    children: [e.isFreemiumPreview ? jsx(n.BottomLeftContainer, {
      ...
      }), e.hubFile.viewer_mode !== "whiteboard" && e.pages.length > 1 && jsx(n.BottomLeftContainer, {
        children: jsx(n.ButtonContainer, {
...
function eW(e) {
  ...
  let s = useViewer("PROTOTYPE", r, {
    ...
    bowser: BrowserInfo,
    forceMobileOptimizations: parseQuery(customHistory.location.search)["mobile-optimizations"] != null,
    deviceInfoByIdentifier: DEVICE_PRESETS_BY_ID,
    trackAnalyticsEvent: trackEventAnalytics,
    definedAnalyticsHandler: analyticsEventManager,
    appTimerStart: e => {
      e === "rendererDraw" && t.handleDocumentRenderStart();
    },
    appTimerStop: e => {
      e === "rendererDraw" && t.handleDocumentRenderStop();
    },
    nextFrame: e => {
  ...
  } = usePrototypeNavigationState(s);
  let {
    navigateForward,
    navigateBackward,
    restartPrototype
  } = usePrototypeNavigationControls(s, null);
  return jsx(eK, {
    currFrameIndex,
    navigateBackwardEnabled,
    navigateForwardEnabled,
...
class eK extends PureComponent {
  constructor(e) {
    ...
    this.getViewport = () => {
      ...
      if (!e) {
        return this._canvasRectToViewportInfo({
          x: 0,
          y: 0,
          width: 0,
          height: 0
    ...
    this.handleSceneDidCompleteInitialLoad = () => {
      ...
      this.setBackgroundColor(t || transparentBlack);
    ...
    this.handleKeyDown = e => {
      (this.state.isFocused || this.props.isFullscreen) && (shouldNavigateToPreviousItem(e) ? this.showPreviousFrame() : shouldNavigateToNextItem(e) ? this.showNextFrame() : shouldTriggerRefresh(e) && this.restartPrototype());
    };
    this.showNextFrame = () => {
  ...
  componentDidUpdate(e) {
      ...
      try {
        sendWithRetry.crossOriginGetAny(this.props.hubFile.redirect_canvas_url, null, {
          ...
          if (!r) {
            ...
            let n = mapToDevicePresetType(t);
            r = getContentScalingMode(n);
          }
          let n = {
    ...
    e.isFullscreen !== this.props.isFullscreen && (this.props.viewer?.adjustZoom(this.zoomForFullscreenState(this.props.isFullscreen), 0.3), this.props.viewer?.setScrollToPan(this.props.isFullscreen));
  }
  ...
  render() {
    if (this.props.viewer === null) return null;
    let {
      backgroundColor,
      scalingMode
    } = this.state;
...
export class $$eJ0 extends Component {
    ...
    this.handleFullscreenTransition = (e, t) => {
      if (e !== t) {
        if (t0(e) !== t0(t)) {
          ...
          t === PreviewMode.FULLSCREEN_WITH_COMMENTS ? this.setState({
            showFullscreenComments: !0
  ...
  isInFullscreen() {
    return this.state.fullscreenTransitionState === 2;
  }
  ...
  render() {
    ...
    return jsxs("div", {
      ...
      children: [jsx("div", {
        ...
        className: t === 0 ? "hub_file_detail_view--coverImageContainerMinimized--W9o-Q hub_file_detail_view--coverImageContainerStatic--KXE3R hub_file_detail_view--_embedFullscreenTransition--sMsEd" : t === 2 ? "hub_file_detail_view--coverImageContainerFullscreen--eSIBa hub_file_detail_view--coverImageContainerStatic--KXE3R hub_file_detail_view--_embedFullscreenTransition--sMsEd" : "",
        style: a,
        children: jsxs("div", {
