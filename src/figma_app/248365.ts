...
export function $$y1(e = qB.SIGN_IN, t = {}) {
  if (t.preventDefaultRedirect && (t.authOptions ??= {}, t.authOptions.redirect_url = _$$Ay.location.pathname + _$$Ay.location.search), zl.get(E)) !function (e) {
...
export function $$T0() {
  ...
  !function (e) {
    ...
    e.current && t.origin === location.origin && (t.data?.type === "trackEvent" && t.data?.event === "Auth Actions" && t.data?.properties?.eventSubtype === "show_auth_page" && r(!0), "trackEvent" === t.data.type && sx(t.data.event, t.data.properties, t.data.options), "auth_iframe_hide" === t.data.type && b(), "auth_iframe_heartbeat" === t.data.type && zl.set(f, e => ({
      ...e,
      mounted: !0
    })), "auth_iframe_show" === t.data.type && zl.set(f, e => ({
      ...e,
      state: 2
    })));
  });
