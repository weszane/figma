...
function A({
  ...
  if (!getFeatureFlags().anticipation_props || !t)
    return null
  let u = a || !a && d
  return jsx('div', {
...
function ew(e) {
  ...
  let ej = !es && (U || el)
  return getFeatureFlags().ds_variable_props_number_def || Z !== ComponentPropType.NUMBER
    ? jsx(eO, {
        isSelected,
        singletonRow,
        isDragging,
        ...
        onInputKeyDown: (e) => {
          e.stopPropagation();
          (e.keyCode === Uz.ESCAPE || e.keyCode === Uz.ENTER) && (eu(), useGrid && Uz.ENTER && e.preventDefault())
        },
...
function eO({
  ...
  let ey = useMemo(() => {
    if (v.kind === uN.VARIANT) {
      ...
      if (v.varValue.resolvedType === VariableResolvedDataType.BOOLEAN)
        return v.varValue.value ? getI18nString('design_systems.component_properties.boolean_true') : getI18nString('design_systems.component_properties.boolean_false')
      if (v.varValue.resolvedType === VariableResolvedDataType.SLOT_CONTENT_ID)
        return null
      if (v.varValue.resolvedType !== VariableResolvedDataType.SYMBOL_ID)
        return v.defaultValue
      {
        let e = wd([v.defaultValue], er)
        if (e)
          return isInvalidValue(e) ? getI18nString('design_systems.instance_swap_picker.mixed') : getBasename(e.name)
      }
    }
  }, [v, er, M])
  ...
  let ex = useCallback((e) => {
    O(e);
    (e.keyCode === Uz.ESCAPE || e.keyCode === Uz.ENTER) && (eA.current = () => {
      eh.current?.focus()
    })
  }, [O])
  useLayoutEffect(() => {
...
function $$e6(e) {
  ...
  let {
    ...
    let c = useCallback((t) => {
      let i = consumedVariable && isValidValue(consumedVariable) && !('isMixed' in consumedVariable && consumedVariable.isMixed) && consumedVariable.type === VariableDataType.EXPRESSION
        ? consumedVariable.value.expressionArguments[0]
        : {
      ...
      t
        ? r(Yi({
            ...
            callback: (r) => {
              ...
              t.resolvedType === VariableResolvedDataType.STRING
                ? i.value[e.name] = {
        ...
        : (delete i.value[e.name], setVariantProperties(Object.keys(i.value).length === 0
            ? null
            : {
                type: VariableDataType.EXPRESSION,
                resolvedType: VariableResolvedDataType.SYMBOL_ID,
    ...
    let u = useMemo(() => {
      if (consumedVariable && isValidValue(consumedVariable)) {
        try {
          ...
          if (!t)
            return null
          if (t.type === VariableDataType.EXPRESSION && t.value.expressionFunction === OperationType.STRINGIFY && (t = t.value.expressionArguments[0]), t?.type === VariableDataType.ALIAS)
            return t.value
        }
        catch (e) {
  ...
  let {
    ...
    if (isBound && z && isNotNullish(X)) {
      ...
      return {
        assignmentValue: onExpressionSubmitted && variableConsumptionMapValue
          ? jsx(y3, {
...
function td({
  ...
  if (!selectedStatesPropertyValues)
    return null
  let G = e.name
  ...
  return jsxs(vL, {
    ...
    children: [jsx(dD.Provider, {
      ...
      children: jsx(_$$q, {
        ...
        onDragStart: () => setIsDraggingSomePropDef(!0),
        onDragEnd: () => setIsDraggingSomePropDef(!1),
        renderListItem: (r, i, a, d, c, u, _) => o
          ? jsx(td, {
              variantPropDef: r,
...
export function $$tm3({
  ...
  let A = _$$eY()
  if (!typedPropDefsExcludingHidden.length && !variantPropDefs.length)
    return null
  let N = jsx(iN, {
