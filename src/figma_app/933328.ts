...
import { handleFileLibrarySubscription, handleOptimisticLibrarySubscription } from '../figma_app/430563'
import { fullscreenValue } from '../figma_app/455680'
import { hasTextNode } from '../figma_app/457074'
...
import { closeComponentFlyoutModal } from '../figma_app/608944'
...
import { componentKeyAtom, nodeIdAtom } from '../figma_app/990334'
...
export let insertSharedComponent = createOptimistThunk((store, payload) => {
  ...
  const performInsertion = (nodeId: string, logArgs: any): void => {
    ...
    closeComponentFlyoutModal(store.dispatch)
    if (insertedNodeId !== defaultSessionLocalIDString) {
      const isComponentSet = hasTextNode(getSingletonSceneGraph().get(nodeId))
...
export let insertSharedStateGroup = createOptimistThunk(async (store, payload) => {
  ...
  const performInsertion = (nodeId: string, defaultStateKey: string, logArgs: any): string => {
    ...
    if (getFeatureFlags().anticipation_props_shadow_reads) {
      atomStoreManager.set(nodeIdAtom, insertedNodeId)
      atomStoreManager.set(componentKeyAtom, item.key)
    }
    insertionCallback?.([insertedNodeId], canvasPosition, isClick)
    closeComponentFlyoutModal(store.dispatch)
    fullscreenValue.triggerAction('commit')
  ...
  const handleInsertion = async (): Promise<void> => {
    ...
    try {
      ...
      permissionScopeHandler.user('insert-state-group', () => {
        ...
        if (firstChild && hasTextNode(firstChild) && insertedNodeId !== defaultSessionLocalIDString) {
          store.dispatch(quickStartSetTextNodeIdAction({
            nodeId: insertedNodeId,
          }))
          handleAtomEvent({
...
export let insertMultipleSlideModules = createOptimistThunk(async (e, t) => {
  ...
  if (modules.some(e => e.moduleSource !== TemplateType.SLIDES_TEMPLATE || !isModuleEnabled(e)))
    return
  let m = e.getState()
  ...
  try {
    ...
    await e1(store.dispatch, styles, (style, index) => {
      ...
      permissionScopeHandler.user('update-shared-style', () => {
        LibraryPubSub.updateSharedStyle(style.key, style.library_key, style.content_hash ?? 'INVALID', style.oldSubscribedKeysToUpdate, style.localIdsToUpdate, canvas, updateStartTime)
      })
...
export async function loadStateGroupByKey(e, t) {
  try {
    ...
    teamLibraryCache.getCanvas(r as any)
    return r
  }
  catch (e) {
    if (e?.status === 403) {
...
function dispatchLibrarySubscriptionUpdates(store: any, current?: Array<any>, previous?: Array<any>): void {
  ...
  for (const prevItem of filteredPrevious) {
    ...
    if (!currItem || currItem.isSubscribed !== prevItem.isSubscribed || currItem.figJamSubscribed !== prevItem.figJamSubscribed || currItem.slidesSubscribed !== prevItem.slidesSubscribed) {
      store.dispatch(handleOptimisticLibrarySubscription({
        libraryKey: key,
        subscriptions: {
