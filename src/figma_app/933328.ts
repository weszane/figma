...
import { q as _$$q, F7, ii, mW, Pg, VariableStyleId } from '../905/859698'
...
export async function upsertSharedSymbolOrStateGroup(item: any, targetUpsertScene: SceneIdentifier = SceneIdentifier.ACTIVE_SCENE): Promise<{
  ...
  if (item.type === PrimaryWorkflowEnum.COMPONENT) {
    ...
    const response = permissionScopeHandler.system('upsert-shared-symbol', () => LibraryPubSub.upsertSharedSymbol(item.component_key ?? ii.INVALID, item.content_hash ?? F7.INVALID, item.library_key, Confirmation.NO, buffer, targetUpsertScene))
    if (!response || isEmptyObject(response) || response.fileUpdateRequired)
      return
    if (!response.localGUID) {
      logError(_$$e.DESIGN_SYSTEMS_EDITOR, 'no local GUID in response', {
    ...
    if (!response || isEmptyObject(response) || response.fileUpdateRequired)
      return
    if (!response.localGUID)
      throw new Error('Swap to Shared Component Error, no local GUID')
    if (response.C2)
      throw new Error(response.C2)
    localGUID = response.localGUID
  }
  else {
    ...
    if (!compareLibraryItems(item, selectedFile) || alwaysFetch) {
      const upsertHandler = async () => {
        try {
          ...
          reportError(_$$e.DESIGN_SYSTEMS_EDITOR, err)
          errorCallback?.()
        }
      }
...
let insertSharedComponent = createOptimistThunk((store, payload) => {
  ...
  const insertAsChildOfGuid = payload.insertAsChildOfCanvas
    ? getSingletonSceneGraph().getCurrentPage()?.guid
    : payload.insertAsChildOfGuid
  ...
  const performInsertion = (nodeId: string, logArgs: any): void => {
    ...
    try {
      _$$L(canvasPosition, insertedNodeId)
      cI(logData, canvasPosition, insertedNodeId, sourceForTracking)
    }
    catch (error) {
      logError('auto_suggest', 'Error logging shadow reads on component insertion', { error })
    }
    ...
    if (insertedNodeId !== defaultSessionLocalIDString) {
      ...
      const hasComponentProperties = Object.keys(
        (insertedNode && insertedNode.type === 'INSTANCE' && insertedNode.componentProperties()) || {},
      ).length > 0

      if (isComponentSet && hasComponentProperties) {
        store.dispatch(_$$E({ nodeId: insertedNodeId }))
        handleAtomEvent({ id: sh })
      }
      else if (isComponentSet) {
        store.dispatch(_$$E({ nodeId: insertedNodeId }))
        handleAtomEvent({ id: Sp })
      }
      else if (hasComponentProperties) {
        handleAtomEvent({ id: An })
      }
      else {
        handleAtomEvent({ id: fe })
      }
    }
  ...
  const handleInsertion = async (): Promise<void> => {
    ...
    if (compareLibraryItems(item, selectedFile)) {
      ...
      if (!localComponent || !state.mirror.sceneGraph.get(localComponent.node_id)) {
        store.dispatch(showModalHandler({ type: _$$Q }))
        return
      }
      ...
      try {
        ...
        reportError(_$$e.DESIGN_SYSTEMS_EDITOR, error)
      ...
      try {
        ...
        permissionScopeHandler.user('insert-component', () => {
          const upsertResult = permissionScopeHandler.system(
            'upsert-shared-symbol',
            () => LibraryPubSub.upsertSharedSymbol(
              item.component_key ?? ii.INVALID,
              item.content_hash ?? F7.INVALID,
              item.library_key,
              Confirmation.NO,
              canvas,
...
let insertSharedStateGroup = createOptimistThunk(async (store, payload) => {
  ...
  const performInsertion = (nodeId: string, defaultStateKey: string, logArgs: any): string => {
    ...
    try {
      ...
      logError('auto_suggest', 'Error logging shadow reads on state group insertion', { error })
    }
  ...
  const handleInsertion = async (): Promise<void> => {
    ...
    try {
      ...
      permissionScopeHandler.user('insert-state-group', () => {
        const upsertResult = permissionScopeHandler.system(
          'upsert-shared-state-group',
          () => LibraryPubSub.upsertSharedStateGroup(
            item.key ?? _$$q.INVALID,
            item.version,
            item.library_key,
            Confirmation.NO,
            canvas,
...
let insertMultipleSlideModules = createOptimistThunk(async (e, t) => {
  ...
  if (modules.some(e => e.moduleSource !== TemplateType.SLIDES_TEMPLATE || !RX(e)))
    return
  let m = e.getState()
...
export async function upsertSharedModule(e) {
  ...
  if (!r || r.fileUpdateRequired || !r.localGUID)
    throw new Error('An error occurred while inserting this template.')
  return r
}
function e1(e, t, r) {
  ...
  try {
    ...
    await e1(store.dispatch, stateGroups, (stateGroup, index) => {
      ...
      permissionScopeHandler.user('update-shared-state-group', () => {
        LibraryPubSub.updateSharedStateGroup(
          stateGroup.key,
          stateGroup.library_key,
          stateGroup.newStateKeyToOutdatedItems,
          instanceIdsToUpdate.map(_$$v.toString),
          canvas,
          updateStartTime,
        )
      })
      ...
      if (Object.keys(stateGroup.newStateKeyToOutdatedItems).length > 0) {
        for (const outdatedItems of Object.values(stateGroup.newStateKeyToOutdatedItems)) {
          const { localIdsToUpdate, oldSubscribedKeysToUpdate } = outdatedItems as any
  ...
  try {
    ...
    await e1(store.dispatch, components, (component, index) => {
      ...
      permissionScopeHandler.user('update-shared-symbol', () => {
        LibraryPubSub.updateSharedSymbol(
          component.component_key ?? ii.INVALID,
          component.library_key,
          component.oldSubscribedKeysToUpdate,
          component.localIdsToUpdate,
          instanceIdsToUpdate.map(_$$v.toString),
          canvas,
          updateStartTime,
        )
...
const applySharedStyle = createOptimistThunk((store, payload) => {
  ...
  const styleLibraryKey = ZR(cO(style, isSharedStyle), openFile)
  ...
  const handleStyleApplicationSuccess = (localStyleGuid: string, fontLoadFailed: boolean, scene: SceneIdentifier) => {
    ...
    const applyStyleOperation = () => {
      if (targetGuids) {
        permissionScopeHandler.user('apply-style-to-nodes', () =>
...
const loadSharedStyle = createOptimistThunk((store, payload) => {
  const state = store.getState()
  const { style, targetUpsertScene = SceneIdentifier.ACTIVE_SCENE } = payload
  const selectedFile = getSelectedFile(state)
  if (!selectedFile)
    return
  ...
  if (isShared) {
    const loadHandler = async () => {
      try {
        ...
        const result = permissionScopeHandler.system('ensure-style-is-loaded', () => LibraryPubSub.getOrCreateSubscribedStyleNodeId(style.key, style.content_hash ?? VariableStyleId.INVALID, style.library_key, canvas, targetUpsertScene))
        if (result?.fileUpdateRequired)
          return
        ...
        if (style.style_type === 'TEXT' && result && result.localGUID) {
          const node = targetUpsertScene === SceneIdentifier.LINTER_SCENE
            ? new ReduxSceneGraph(FileSourceType.LINTER).get(result.localGUID)
            : state.mirror.sceneGraph.get(result.localGUID)
...
const updateSharedStyle = createOptimistThunk(async (store, payload) => {
  const { styles, updateStartTime, subscribedLibraryKeys, fileSubscribedLibraryKeys } = payload
...
const updateSelectedSharedStyleConsumers = createOptimistThunk((store, payload) => {
  ...
  const { styleUpdateInfo, oldStyleGUID, consumerGUIDsToUpdate } = payload
  const { updateAsset } = styleUpdateInfo

  teamLibraryCache.getCanvas(updateAsset).then((canvas) => {
    const state = store.getState()
    const { content_hash } = updateAsset
    if (!oldStyleGUID)
      throw new Error('guid to update can not be empty')
    if (!content_hash)
      throw new Error('content_hash not specified on style')
...
async function fetchAndUpdateStateGroups(symbolKeys: string[], stateGroupKeys: string[], store: any, options?: any) {
  ...
  try {
    if (missingSymbolKeys.length > 0 || missingStateGroupKeys.length > 0) {
      ...
      const atomStore = atomStoreManager.get(qp)
...
export async function loadComponentV2ByKey(store: any, componentKey: string, source?: string): Promise<{
  ...
  try {
    ...
    if (file) {
      ...
      if (component)
        itemsById[component.node_id] = component
      ...
      if (component)
        teamLibraryCache.getCanvas(component as any)
    }
    return {
      component,
      parentStateGroup,
...
const getOrgMigrationStatus = liveStoreInstance.Query({
  fetch: async (orgId: string) => (await PT.getOrgMigrationStatus({
    orgId,
  })).data.meta,
...
export function useLibraryInfo(params: any, options: any = {}): any {
  const queryResult = IT(getLibraryInfo(params), options)
  const dispatch = useDispatch()
  const [data] = queryResult
  const setAtom = useSetAtom(qU)
  handleStatusChangeEffect(data, (result: any) => {
    ...
    setAtom(result.files.map((item: any) => _$$E2(item)))
  })
  return queryResult
}
...
export function useAllLibrarySubscriptions(): void {
  ...
  useEffect(() => {
    if (!presetMapping || !latestPresetMapping)
      return
