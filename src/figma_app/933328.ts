...
export async function $$eV38(e, t = SceneIdentifier.ACTIVE_SCENE) {
  ...
  if (e.type === PW.COMPONENT) {
    ...
    if (!i || isEmptyObject(i) || i.fileUpdateRequired)
      return
...
let $$eW48 = createOptimistThunk((e, t) => {
  ...
  let h = async () => {
    try {
      ...
      if (t) {
        let a = item.type === PW.STATE_GROUP
    ...
    if (N2(item, f)) {
      ...
      Eo.getCanvas(item).then((e) => {
        permissionScopeHandler.user('insert-component', () => {
          ...
          if (t.fileUpdateRequired || !t.localGUID)
    ...
    if (N2(item, m)) {
      ...
      Eo.getCanvas(item).then((t) => {
        permissionScopeHandler.user('insert-state-group', () => {
          ...
          if (!n || n.fileUpdateRequired || !n.localGUID)
            throw new Error('An error occurred while adding an instance of this component.')
          ...
          a && (C$(a) && i !== defaultSessionLocalIDString
            ? (e.dispatch(_$$E({
                nodeId: i,
              })), YQ({
                id: sh,
  ...
  if (o) {
    let r = async () => {
      try {
        ...
        t.bufferCallback?.(r)
        let a = permissionScopeHandler.system('ensure-style-is-loaded', () => LibraryPubSub.getOrCreateSubscribedStyleNodeId(style.key, style.content_hash ?? IA.INVALID, style.library_key, r, targetUpsertScene))
        if (a?.fileUpdateRequired)
          return
    ...
    try {
      ...
      await e1(e.dispatch, styles, (e, r) => {
        ...
        permissionScopeHandler.user('update-shared-style', () => {
          LibraryPubSub.updateSharedStyle(e.key, e.library_key, e.content_hash ?? IA.INVALID, e.oldSubscribedKeysToUpdate, e.localIdsToUpdate, a, updateStartTime)
        })
...
let $$e718 = createOptimistThunk((e, t) => {
  ...
  Eo.getCanvas(updateAsset).then((t) => {
    ...
    if (fullscreenValue.triggerAction('commit'), !l)
...
let $$tU26 = createOptimistThunk(async (e) => {
  if (await kb.promise, !e.getState().user)
    return
