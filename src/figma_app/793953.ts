...
import { getMemoryUsage, getUsedJSHeapSize } from '../figma_app/527873'
import { getJoinStatusString } from '../figma_app/582924'
import { CorePerfInfo, FullscreenPerfInfo, ImageSourceType, Multiplayer, TextureType } from '../figma_app/763686'
...
export function $$E2(e, t) {
  return Object.keys(e).reduce((r, n) => {
    let i = t[n]
    let a = e[n]
    i != null && a != null && (r[n] = i - a)
    return r
  }, {})
}
export function $$y1() {
  let e = b()
  if (!FullscreenPerfInfo) {
    return {
      ...e,
    }
  }
...
function b() {
  let e = getUsedJSHeapSize() ?? 0
  let t = CorePerfInfo
  if (!t) {
    return {
      process_memory: e,
      emscripten_reserved_memory: getMemoryUsage(),
    }
  }
...
async function I(e, t, r, i, o) {
  let p = e === 'Fullscreen Periodic Metrics'
  if (!(!CorePerfInfo || atomStoreManager.get(fullscreenCrashStateAtom) !== 'ok' || isInteractionOrEvalMode())) {
    if (p) {
      let a = $$y1()
...
export function $$v0(e, t, r) {
  ...
  let a = e === 'Fullscreen Periodic Metrics' ? g : f
  let s = a.periodMs
