...
function Q(e, t, r) {
  ...
  let T = $$Z42(i) && !$$Z42(a)
  o
    ? AppStateTsApi?.uiState().backgroundPickerOpen.set(!0)
    : l
      ? AppStateTsApi?.uiState().backgroundPickerOpen.set(!1)
      : d
...
let $$es24 = createOptimistThunk((e, t) => {
  ...
  if (r.saveStatus && r.saveStatus.hasUnsavedChanges && r.saveStatus.tabCloseText !== SaveConnectionIssues.SUPPRESS_UNSAVED_CHANGES_UI) {
    ...
    customHistory.push(selectedViewToPath(r, i), {
...
let $$eA35 = createOptimistThunk((e, t) => {
  if (desktopAPIInstance) {
    t?.closeDesktopTabWithMessage
      ? (desktopAPIInstance.showFileBrowser(t.closeDesktopTabWithMessage), desktopAPIInstance.close({
          suppressReopening: !0,
          shouldForceClose: !0,
        }))
  ...
  if (openFile && selectedView) {
    ...
    selectedView.view === 'fullscreen' && selectedView.prevSelectedView
      ? p = selectedView.prevSelectedView
      : openFile && c && (p = {
        view: 'folder',
        folderId: c,
      })
  ...
  fullscreenValue.dispatchIfSaved(selectViewAction(p))
  V()
  e.dispatch($$ev33(t))
})
