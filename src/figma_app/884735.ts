...
export function $$H0(e) {
  ...
  let J = useHandleMouseEvent(`propPill.${nodeField}`, 'click', () => {
    !selectionHasInstanceSublayer && def && q.current && (q.current && (nodeField === RR.TEXT || nodeField === RR.VISIBLE)
      ? _?.showBindingUI(q.current)
      : V(showDropdownThunk({
          type: X,
          data: {
            nodeField,
    ...
    children: jsx('div', {
      ...
      'data-tooltip-offset-x': (() => {
        if ($.current && q.current)
          return $.current.offsetLeft + $.current.offsetWidth / 2 - (q.current.offsetLeft + q.current.offsetWidth / 2)
      })(),
...
function K(e) {
  ...
  let {
    ...
    : {
        ui3FirstButton: selectionHasInstanceSublayer
          ? jsx('span', {
              onContextMenu: V,
...
function Y(e) {
  ...
  return jsx('span', {
    onContextMenu: V,
    children: selectionHasInstanceSublayer
      ? goToPropertyButton
      : jsx('div', {
          className: UD,
          children: detachPropertyButton,
