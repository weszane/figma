...
import { codeConnectToolsEnabledAtom, isCodebaseSuggestionsEnabled, mockCodebaseSuggestions, mockCodeConnect } from "../figma_app/342355"
...
function prepareNodesByLibraryKey(nodesByLibraryKey: Map<string, NodeBatchItem[]>, nodes: INode[], sceneGraph: SceneGraph, fileKey: string, openLibraryKey: string, nodeIdToErrors: Record<string, string>) {
  for (const node of nodes) {
    try {
      ...
      const instanceList = collectInstanceKeys(node.guid, sceneGraph, fileKey, false, openLibraryKey)
      if (!backingInfo.backingLibraryKey || !backingInfo.backingNodeId)
        continue
...
function collectCodeConnectMapping(node: INode, storeContext: any, nodeIdToErrors: Record<string, string>, filterGuids: Set<string> = new Set()): CodeConnectMappingResult {
  ...
  if (!sceneGraph)
    throw new Error("No sceneGraph provided")
  if (!fileKey)
    throw new Error("No fileKey provided")
  if (!openLibraryKey)
    throw new Error("No openLibraryKey provided")
...
async function processCodeConnectBatch(params: OParams): Promise<void> {
  ...
  for (const item of batch) {
    try {
      ...
      if (!preloadData || !preloadData.docsById)
        continue
