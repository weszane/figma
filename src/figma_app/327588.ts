...
export function $$h4() {
  ...
  for (let e = 0; e < c.length; e++) {
    let t = AppStateTsApi?.canvasGrid().isRowSelected(e);
    let r = AppStateTsApi?.canvasGrid().getRowGUID(e) ?? defaultSessionLocalIDString;
    if (t && getSingletonSceneGraph().get(r)?.isCanvasGridStateGroupRow && getFeatureFlags().buzz_template_sets) return !0;
  }
  return !1;
}
export function $$m2() {
  return AppStateTsApi ? AppStateTsApi.cooperFocusView().focusNodeInFocusedNodeView : (e, t) => {};
}
export function $$g3() {
