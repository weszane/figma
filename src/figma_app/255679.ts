 ...
 * Original: $$m4
 */
export const queryUiKitsFeedbackUrls = liveStoreInstance.Query({
  fetch: async (key: string) => key ? (await fetchDynamicConfig('ui_kits_feedback_urls')).get(key, '') : ''
});
...
export function getPartnerTypeFromPreset(e: any, t: any): string | undefined {
  if (e.status === 'loaded') {
    const preset = e.data.libraryPresetSubscriptionsV2?.find((sub: any) => getResourceDataOrFallback(sub.libraryKey) === t);
    if (preset) return preset.partner_type ?? undefined;
  }
}
...
const resourceTypes = [VariableByKey, VariableCollectionByKey, StyleByKey, CommunityLibraryComponentsAndStateGroups];
...
function createResourceTypeQuery(resourceType: any) {
  return liveStoreInstance.Query({
 ...
 * Original: $$k8
 */
export function handleResourceQuery(query: any, args: any, useAtom: (atom: any) => any): any {
...
export const Ag = isKeyInQNOrR9; // Ag
export const B8 = useHasResourcePresetKey; // B8
export const HA = getPartnerTypeFromPreset; // HA
export const Os = communityLibraryComponentsQuery; // Os
export const Re = queryUiKitsFeedbackUrls; // Re
export const TX = hasHubFileOrPresetKey; // TX
export const fd = hasResourcePresetKey; // fd
export const fn = nonceAtom; // fn
export const ku = handleResourceQuery; // ku
export const lR = hasLibraryKeyInSet; // lR
export const nT = variableCollectionByKeyQuery; // nT
export const o3 = hasResourcePresetKeyStatic; // o3
export const p9 = variableByKeyQuery; // p9
export const qA = pageLoadAtom; // qA
export const ue = styleByKeyQuery; // ue
export const xm = useHasLibraryKeyInSet; // xm