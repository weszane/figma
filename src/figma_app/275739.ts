...
let I = class e {
  constructor(e) {
    ...
    this.startPollingMicState = () => {
      desktopAPIInstance && (this.microphonePermissionDenied && desktopAPIInstance.setUsingMicrophone(!1), this.micStateInterval = setInterval(() => {
        this.client && (desktopAPIInstance?.setIsInVoiceCall(this.client.connectionState !== "DISCONNECTED"), desktopAPIInstance?.setUsingMicrophone(!!this.isPublishingLocalTrack()))
      }, 5e3))
    }
    ...
    this.startPollingVolumeData = () => {
      this.volumeDataInterval || (this.volumeDataInterval = setInterval(async () => {
        if (!this.client || !this.userId)
          return
        let e = {}
        for (let e in this.averageRecentVoiceLevel) this.averageRecentVoiceLevel[e] *= 0.8
        for (let t of this.client.remoteUsers) {
          let r = await this.getDecryptedUserId(t.uid)
          if (!r)
            continue
    ...
    this.checkLocalVolumeLevels = () => {
      this.isMuted && this.unpublishedLocalAudioTrack && this.unpublishedLocalAudioTrack.getVolumeLevel() > 0.6 && this.callEventCallback?.($E.WARNING_MICROPHONE_LEVELS_HIGH_AND_MUTED)
    }
    this.updateActiveSpeakers = () => {
    ...
    this.attachDeviceListeners = (e) => {
      e.onMicrophoneChanged = (e) => {
        this.microphone?.deviceId === e.device.deviceId && e.state === "INACTIVE" && this.getMicrophones().then((e) => {
          this.microphone = e.length > 0 ? e[0] : null
        })
      }
      e.onPlaybackDeviceChanged = (e) => {
        this.playbackDevice?.deviceId === e.device.deviceId && e.state === "INACTIVE" && BrowserInfo.chrome && this.getPlaybackDevices().then((e) => {
  ...
  static createInstance() {
    return this.instance
      ...
      : new Promise((t, r) => {
        ...
        n.onerror = () => {
          r(new Error("Provider audio script load error"))
        }
  ...
  async join({
    ...
    if (await this.setMicrophonePermissions(), !this.getSystemCompatibility())
    ...
    try {
      ...
      let t = new Error(`Error joining call: ${i.message}`)
      await e.leave()
      return t
    }
    finally {
  ...
  attachClientListeners(e, t, r, n, i) {
    ...
    this.client?.on("user-published", async (e, t) => {
      if (await this.client?.subscribe(e, t), t === "audio") {
    ...
    let o = !0
    this.client?.on("network-quality", (i) => {
      o && (i.downlinkNetworkQuality >= 4 || i.uplinkNetworkQuality >= 4)
    ...
    this.client?.on("exception", (s) => {
      ...
      o && i(o)
      s.code >= 3e3
        ? (console.error(s), trackEventAnalytics("Voice Call - Error", {
            callId: e,
            token: t,
            userId: r,
            encryptionKey: n,
  ...
  async getDecryptedUserId(e) {
    try {
      ...
      reportError(ServiceCategories.UNOWNED, new Error(`Failed to decrypt encrypted user: ${e}`))
