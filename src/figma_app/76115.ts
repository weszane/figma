...
import { filterAndSortPublishedItems, isCurrentStagingStatus, processLocalComponents } from "../figma_app/80990"
import { handleFileLibrarySubscription } from "../figma_app/430563"
import { selectCurrentFile } from "../figma_app/516028"
import { hasAssetId, NO_TEAM, PrimaryWorkflowEnum, StagingStatusEnum } from "../figma_app/633080"
import { areNodesEqual, arePagesEqual, isActiveStagingStatus, isPrimaryWorkflowType, useIsAssetPublishedForCurrentFile, useSubscribedLibraryId } from "../figma_app/646357"
...
export function $$x3(e, t, r) {
  ...
  for (let e of n) {
    if (e.deletedFromSceneGraph || (i.push(e), e.status === StagingStatusEnum.NOT_STAGED || e.status === StagingStatusEnum.DELETED ? s.push(e) : a.push(e)), isCurrentStagingStatus(e.status) && d++, e.status !== StagingStatusEnum.CURRENT && e.status !== StagingStatusEnum.NOT_STAGED || (e.type === PrimaryWorkflowEnum.COMPONENT || e.type === PrimaryWorkflowEnum.STATE_GROUP) && isActiveStagingStatus(e.status) && e.old_key || e.type === PrimaryWorkflowEnum.STATE_GROUP && isActiveStagingStatus(e.status) && c.has(e.node_id)) {
      l++
      continue
    }
  }
...
export function $$R10({
  ...
  return useMemo(() => {
    ...
    let t = {}
    for (let r of [a, i]) {
      for (let n of r.subscribedHubFiles) {
        if (!n.library_key) {
