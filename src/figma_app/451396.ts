import type { SceneGraph } from "../905/830071";
...
import { evaluateJsInIframe, useIframeJsEvaluator } from "../figma_app/478201";
...
function findBackingComponentName(parentNode: SceneNode, instanceNode: InstanceNode, scene: SceneGraph): string | undefined {
  let foundName: string | undefined;
  const backingComponent = scene.get(instanceNode.symbolId ?? "");
  if (!backingComponent) {
    reportError(ServiceCategories.DEVELOPER_TOOLS, new Error(`Cant find backing component of parent node ${instanceNode.symbolId}`));
    return;
  }
  const parentFriendlyId = scene.developerFriendlyIdFromGuid(parentNode.guid);
  traverseNodeTree(backingComponent, scene, (node: SceneNode) => {
...
export function generatePlaygroundInstanceCode(template: any, instanceFigmadocs: any, breakInTemplateExecution: boolean): any {
  ...
  const nodeTreeToInject = useMemo(() => buildInstanceTree({
    node: playgroundNode,
    scene: sceneGraph,
    fileKey,
    openLibraryKey
  ...
  const getInstanceFigmadocFromJSON = useCallback((template: any, guid: string) => getBestMatchingVariant(sceneGraph)(template, guid), [sceneGraph]);
  ...
  }, [template, playgroundNode, nodeTreeToInject, instanceFigmadocs, inlineInstancesEnabled, breakExecution, getInstanceFigmadocFromJSON]);
  return useIframeJsEvaluator(generatedCode);
}
...
export async function generateInstanceCode({
  ...
  return evaluateJsInIframe(generatedCode);
}
...
function addChildToInstanceTree(node: SceneNode, parentInstance: InstanceNode, scene: SceneGraph, fileKey: string | null, openLibraryKey: string | null, path: string[]): void {
