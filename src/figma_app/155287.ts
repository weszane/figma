...
export const ManifestSchema = zod.object({
  ...
  permissions: Ip.filteredArray(zod.union([zod.enum([
    'currentuser', 'activeusers', 'teamlibrary', 'fileusers', 'payments', 'clipboardwrite'
  ]), zod.enum([
    'openexternal', 'analytics', 'cortex', 'debug', 'firstdraft', 'filekey', 'hidecursors',
    'camera', 'microphone', 'displaycapture'
  ])])).optional(),
  enableReadOnly: zod.boolean().optional(),
  capabilities: Ip.filteredArray(zod.enum([
    'textreview', 'panel', 'inspect', 'codegen', 'linkpreview', 'vscode'
  ])).optional(),
...
export const PluginDetailsSchema = PluginVersionsRecordSchema.merge(zod.object({
  ...
  monetization_status: zod.nativeEnum(UC).nullish(),
  community_resource_payment: fe.optional(),
  third_party_m10n_status: zod.nativeEnum(PN).nullish(),
 ...
 */
export function isPluginAllowListed(
  plugin: any,
  allowList: Record<string, any>,
  orgSettings: any
): boolean {
  return !!(!orgSettings || !plugin || isPrivatePlugin(plugin) || hasLocalFileId(plugin)) ||
    !!orgSettings.public_plugins_allowed &&
    (manifestContainsWidget(plugin)
      ? !orgSettings.widgets_whitelist_enforced
      : !orgSettings.plugins_whitelist_enforced
    ) || !!allowList[plugin?.plugin_id]
}
...
export function isMigratingPlugin(obj: any): boolean {
  return obj.third_party_m10n_status === PN.MIGRATING
}
...
export function manifestErrorMessage({
  type,
  text,
}: { type: ManifestErrorType; text: string }): string | undefined {
  switch (type) {
 ...
 */
export const TriggeredFromEnum = zod.union([
  zod.enum([
    'contextmenu', 'filemenu', 'runlast', 'osmenu', 'relaunch', 'quick-actions', 'universal-insert',
    'parameter-entry', 'paste_from_url', 'textreview', 'handoff-relaunch', 'handoff-panel', 'codegen',
    'related-link-click', 'related-link-preview', 'related-link-auth', 'pinned', 'try-plugin-params',
    'auto-run'
  ]),
  MM,
])
...
export const Ye = Object.values(ManifestEditorType).filter(
  e => e !== 'buzz' || !!getFeatureFlags().buzz_plugins_publishing
)
