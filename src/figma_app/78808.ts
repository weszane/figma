...
let $$B18 = createOptimistThunk((e, t) => {
  ...
  let r = fileApiHandler.putFile(t).then(() => {
    t.onSuccess?.()
  }).catch((r) => {
  ...
  let n = t.file
  let i = {
    ...mapFileProperties(n, n.key),
    ...mapFileLinkExpirationConfig(n, t.linkExpirationConfigId, t.currentUser),
  }
  let a = mapFileLinkExpirationConfigOptimistic(n, t.linkExpirationConfigId, t.currentUser)
  ...
  XHR.post(`/api/multiplayer/${a.key}/copy?${serializeQuery(i)}`).then((i) => {
    ...
    if (!i.data.error && i.data.meta && s) {
      ...
      if (trackFileEvent('File Duplicated', a.key, e.getState(), {
        ...
        source: t.source,
      }), t.checkOssSalesExperiment && setOssSalesExperimentValue(s.key), desktopAPIInstance) {
      ...
      customHistory.redirect(getDesignFileUrlWithOptions(s), '_blank')
    }
  }).catch(() => {
    e.dispatch(VisualBellActions.dequeue({}))
...
let $$H14 = createOptimistThunk((e, t) => {
  let r = t.file_key
  r && (e.getState().fileByKey[r] || fileApiHandler.getFiles({
    fileKey: r,
    ...
    e.dispatch(VisualBellActions.enqueue({
      ...
      button: {
        ...
        action: () => {
          e.dispatch($$Z8({
            fileKey: t.fileKey,
            url: getDesignFileUrlWithOptions(n),
...
let $$Z8 = createOptimistThunk((e, t) => {
  let r
  let n = t.url
  let i = t.label
  if (i) {
  ...
  r.then(() => {
    trackFileEvent('File Share Link Copied', t.fileKey, e.getState(), {
      copyLinkSource: ShareContext[t.source],
      ...t.trackingProperties,
    })
    t.skipVisualBell || J(e, t)
  }).catch(() => {
    t.showVisualBellOnErrorForInteractionTests && isInteractionOrEvalMode()
      ? J(e, t)
