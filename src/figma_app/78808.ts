...
import { trackVersionDiffingChanges } from '../figma_app/841351';
...
export const updateFilePermissionsOptimistic = createOptimistThunk((dispatchApi, payload) => {
  payload.file.created_at && console.error('It looks like you are PUT-ting to the API with the entire file object; please only pass the key and file permission attributes');
  ...
  if (Object.keys(updateProps).length > 0) getCurrentLiveGraphClient().optimisticallyUpdate(updateProps, updatePromise);
  if (optimisticConfig) getCurrentLiveGraphClient().optimisticallyCreate(optimisticConfig, updatePromise);
});
...
export const duplicateFileOptimistic = createOptimistThunk(async (dispatchApi, payload) => {
  ...
  let canCreate = false;
  if (folderId) {
    const resourceData = getResourceDataOrFallback<any>((await subscribeAndAwaitData(FileCreationPermissionsView, {
      projectId: folderId
    })).project);
    canCreate = !!resourceData && !!file.editorType && canCreateFileType(resourceData, file.editorType);
  }
  if (canCreate) params.folder_id = folderId;
  if (payload.versionId) params.version_id = payload.versionId;
  dispatchApi.dispatch(VisualBellActions.enqueue({
  ...
  sendWithRetry.post(`/api/multiplayer/${file.key}/copy?${serializeQuery(params)}`).then(response => {
    ...
    if (!response.data.error && response.data.meta && hasOpenFile) {
      ...
      if (payload.checkOssSalesExperiment) setOssSalesExperimentValue(meta.key);
    ...
    dispatchApi.dispatch(VisualBellActions.dequeue({}));
    dispatchApi.dispatch(FlashActions.error(getI18nString('file_browser.file_browser_actions.file_copy_error')));
  });
});
...
export const fetchFileByKeyOptimistic = createOptimistThunk((dispatchApi, payload) => {
  ...
  if (fileKey) {
    ...
    if (!state.fileByKey[fileKey]) {
      fileApiHandler.getFiles({
        fileKey
      }).then(result => {
        dispatchApi.dispatch(filePutAction({
          file: result.data.meta
        }));
      });
    }
  }
 ...
 */
export const startWorkshopSessionOptimistic = createOptimistThunk(async (dispatchApi, payload, {
  liveStore
}) => {
...
export const markFileViewedOptimistic = createOptimistThunk(async (dispatchApi, payload) => {
  await sendWithRetry.post(`/api/files/${payload.fileKey}/view`).then(({
    ...
    dispatchApi.dispatch(VERSION_HISTORY_SET_FILE_LAST_SEEN_AT({
      lastViewed: last_view_at,
      lastEdited: last_edit_at
    }));
    dispatchApi.dispatch(trackVersionDiffingChanges({
...
export const setFileThumbnailOptimistic = createOptimistThunk((dispatchApi, payload) => {
  updateFileThumbnail(payload.file_key, payload.thumbnail_guid).then(() => {
    const featureFlags = getFeatureFlags();
    const message = featureFlags.dse_library_pg_thumbnails ? payload.thumbnail_guid ? getI18nString('file_browser.file_browser_actions.file_thumbnail_set') : getI18nString('file_browser.file_browser_actions.default_file_thumbnail_restored') : payload.thumbnail_guid ? getI18nString('file_browser.file_browser_actions.thumbnail_set') : getI18nString('file_browser.file_browser_actions.default_thumbnail_restored');
    dispatchApi.dispatch(VisualBellActions.enqueue({
      message
    }));
  }).catch(() => {
...
export const restoreFileVersionOptimistic = createOptimistThunk((dispatchApi, payload) => {
  ...
  if (prevVersion) {
    dispatchApi.dispatch(fileRestoreAction({
      fileKey,
...
export const copyEmbedCodeOptimistic = createOptimistThunk((dispatchApi, payload) => {
  copyTextToClipboard(payload.embedCode).then(() => {
    ...
    dispatchApi.dispatch(FlashActions.error(getI18nString('file_browser.file_browser_actions.embed_code_copy_error')));
  });
});
...
export const copyShareLinkOptimistic = createOptimistThunk((dispatchApi, payload) => {
  ...
  copyPromise.then(() => {
    ...
    if (!payload.skipVisualBell) showLinkCopiedVisualBell(dispatchApi, payload);
  }).catch(() => {
...
export const renameFileOptimistic = createOptimistThunk((dispatchApi, payload) => {
  ...
  if (name.length <= 100) {
    ...
    trackFileEvent('File Renamed', fileUpdate.key, dispatchApi.getState(), {
  ...
  if (fileKeys.length) {
    sendWithRetry.post('/api/active_file_users', {
      ...
      Object.entries(data.meta.active_file_users).forEach(([fileKey, usersObj]) => {
        if (state.activeFileUsers && fileKey in state.activeFileUsers) return;
        const users = [];
        if (usersObj) {
        ...
        dispatchApi.dispatch(setActiveFileUsersAction({
          fileKey,
          users
        }));
      });
