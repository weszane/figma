...
export const updateFilePermissionsOptimistic = createOptimistThunk((dispatchApi, payload) => {
  ...
  if (Object.keys(updateProps).length > 0) WB().optimisticallyUpdate(updateProps, updatePromise);
  if (optimisticConfig) WB().optimisticallyCreate(optimisticConfig, updatePromise);
});
