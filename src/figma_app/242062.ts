...
let en = class e {
  ...
  updateWiggleMeter(t) {
    if ((t = typeof t == 'number' ? t : this.getDeltaTime()) <= 0)
      return
...
function tc({
  ...
  useEffect(() => {
    ...
    let _ = (e) => {
      ...
      if (e.target.tagName !== 'CANVAS' || p && e.pointerId === p.pointerId)
        return
      o.add(e.pointerId)
      let r = o.size
    ...
    !(function (e) {
      ...
      useEffect(() => {
        let r = e.onWiggleModeChange((r) => {
          ...
          n(r)
          r && e.getHighFiveKeyPressed()
            ? t('figjam_cursor_high_five_events', {
                type: 'enter_hands_up',
                source: 'keyboard',
              })
  ...
  return jsxs(Fragment, {
    children: [jsx(_$$p, {
      ...
      'children': [L && jsxs(AS, {
        ...
        : null, t && c.type === 'REACTING_OR_CHATTING'
        ? jsx(tc, {
            multiplayerEmoji: c,
            viewportInfo: t,
          })
...
function tm({
  ...
  if (th(g, !!(E && E.length && E[0]), N), !N || !g?.mouse)
    return null
