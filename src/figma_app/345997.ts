 ...
 */
export const FILE_TYPE_PAGE_LIMITS: Record<FFileType, number> = {
  [FFileType.DESIGN]: 3,
...
export function isTeamInGracePeriod(team: Team): boolean {
  const gracePeriodDate = team && !team.org_id && team.grace_period_end && dayjs.utc(team.grace_period_end);
...
export function isGracePeriodEndingSoon(team: Team, daysThreshold: number = 14): boolean {
  return !!(team && isTeamInGracePeriod(team)) && dayjs.utc(team.grace_period_end).valueOf() - Date.now() < 864e5 * daysThreshold;
}
