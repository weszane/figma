...
import { Alignment, ElementTypeEnum, KindEnum, PositionEnum, StatusEnum } from '../905/129884'
...
import { compareRects, createRect } from '../905/508367'
import { getTooltipEntry, isTooltipRegistered } from '../905/524523'
import { getFeatureFlags } from '../905/601108'
import { isFigmaDomain } from '../905/691205'
import { Point } from '../905/736624'
import { EE, EF, GQ, Hv, IA, IC, kb, LE, mG, Ox, pG, Qq, Qs, v5, W$, xU } from '../905/736956'
import { clearTimeoutID, hideTooltip, setTimeoutID, showTooltip, showTooltipWithDelay } from '../905/765855'
...
export function $$z0(e) {
  ...
  if (!e.visible)
    return null
  try {
    if (!(t = document.querySelectorAll(`[data-tooltip-key=${e.tooltipKey}]`)[0]))
      return null
  }
  catch (e) {
    return null
...
function W(e) {
  ...
  let s = t => t.shortcut ? t.shortcut : t.shortcutKey ? getKeyboardShortcut(e.keyboardShortcuts, camelToKebab(t.shortcutKey)) : null
  let o = e.tooltip.target
  let d = ul(o) && e.tooltip.position === PositionEnum.RIGHT
  if (e.tooltip.state !== StatusEnum.SHOWING)
    return null
  let c = r()
  if (d && !c)
    return null
      ...
      'children': (() => {
        ...
        switch (t.target.kind) {
          ...
          case ElementTypeEnum.KEY:
          {
            ...
            let s = getKeyboardShortcut(e.keyboardShortcuts, camelToKebab(t.target.key))
            if (s) {
              return jsxs('span', {
                className: GQ,
                children: [jsx('span', {
...
export function $$Y4(e, t, r, n) {
  ...
  if (e.hasAttribute('data-tooltip-show-on-target-only') && n != null && n !== e)
    return null
  ...
  function U() {
    j === null && (j = D && r
      ...
      : createRect(e.getBoundingClientRect()), j.left += v, j.right += v, j.bottom += A, j.top += A)
    return j
  }
...
function $(e, t) {
  let r = e.target
  let n = e.target
  if (!r || r.nodeType !== 1 || document.pointerLockElement)
    return 'return'
  let i = null
...
function q(e, t) {
  ...
  if (s !== 'return' && (s || (s = (function () {
    ...
    (getFeatureFlags().fpl_tooltip
      ? (r = s, n = i.tooltip, r.position === n.position && compareRects(r.targetRect, n.targetRect) && (function (e, t) {
          if (e === t)
            return !0
          if (!e || !t || e.kind !== t.kind)
            return !1
          switch (e.kind) {
            case ElementTypeEnum.TEXT:
      ...
      : s.position === i.tooltip.position && compareRects(s.targetRect, i.tooltip.targetRect))
      ? i.tooltip.state === StatusEnum.SHOWING && i.tooltip.timeoutID !== null && t.dispatch(clearTimeoutID())
      : i.tooltip.state === StatusEnum.SHOWING || s.showImmediately
        ? t.dispatch(showTooltip({
            target: s.target,
