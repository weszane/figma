...
async function x(e, t) {
  if (!e.ok) throw new Error(`Downloading wasm failed with ${e.status}`);
...
async function w(e) {
  ...
  !C || C === e || isInteractionPathCheck() || $$D6() || (C === 'fullscreen-app' ? reportError(ServiceCategories.CLIENT_PLATFORM, new Error('Previously downloaded fullscreen wasm, but is now downloading prototype-lib')) : C === 'prototype-lib' && reportError(ServiceCategories.CLIENT_PLATFORM, new Error('Previously downloaded prototype-lib wasm, but is now downloading fullscreen')));
  ...
  if (setMemoryCallback(e, getReservedHeapSize), r.headers.get('x-figma-local-build') === 'true' && (N = !0), WebAssembly.instantiateStreaming) {
    try {
      ...
      if (isInteractionPathCheck() && console.log('Error calling instantiateWasmStreaming', e), e && (!e.message || e.message.includes('Incorrect response MIME type') || e.message.includes('Unexpected response MIME type'))) {
        ...
        reportError(ServiceCategories.UNOWNED, new Error(`Working around error instantiating streaming fullscreen WASM: ${e}`));
        r = await fetch(t);
      } else {
...
export function $$B2() {
  ...
  let t = $$U5();
  t === 'immediately' ? e() : t === 'eventually' && window.addEventListener('load', () => {
    e();
  });
}
let G = null;
...
export async function $$H4(e, t = 'fullscreen-app') {
  if (V && V !== t && !isInteractionPathCheck() && (V === 'fullscreen-app' ? reportError(ServiceCategories.UNOWNED, new Error('Previously initialized fullscreen wasm, but is now initializing prototype-lib')) : V === 'prototype-lib' && reportError(ServiceCategories.UNOWNED, new Error('Previously initialized prototype-lib wasm, but is now initializing fullscreen'))), !T(t) || !window.WebAssembly) {
  ...
  try {
    ...
    !e || e.message === 'Failed to fetch' || e.message === 'Could not download wasm module' || e.message === 'TypeError: Load failed' || e.message.includes('NetworkError') || e.message.toLowerCase().includes('network error') || (e instanceof Error || (e = new Error(`Fullscreen load failure: ${e}`)), reportError(ServiceCategories.UNOWNED, e));
