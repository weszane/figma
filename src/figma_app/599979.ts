...
export function $$K31(e, t, r, n) {
  ...
  if (i.length === 0) throw new Error('Called getDefaultHubFilePublishingMetadataAuthor when no valid author exists');
...
export function $$X6(e, t) {
  ...
  let r = $$es24(t, e);
  if (r.length === 0) throw new Error('Called getDefaultPluginPublishingMetadataAuthor when no valid author exists');
...
function Z(e, t, r, i, a) {
  if (t.length === 0) throw new Error('Called getDefaultPublishingMetadataAuthor when no valid author exists');
  let s = $$q22(e, r, i);
...
export async function $$ei38(e) {
  ...
  if (status === 200) return data.meta;
}
...
export async function $$eg1(e, t) {
  if (e.files === null || !e.files[0]) throw new Error(getI18nString('community.publishing.error_thumbnail_image_not_found_no_filename'));
  t && $$eN39(t);
  let r = e.files[0];
...
export function $$eT11(e) {
  ...
  e && e.forEach((e, i) => {
    if ('buffer' in e) {
      if (e.type === 'video') {
        if (!e.thumbnail_buffer) throw new Error(getI18nString('community.publishing.error_video_thumbnail_not_found'));
        let t = {
          carousel_position: i,
