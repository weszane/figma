...
import { HubTypeEnum, isWidget, ResourceTypeNoComment } from "../figma_app/45218";
import { FigmaResourceType, HubEventType } from "../figma_app/350203";
import { cacheWidgetVersionsThunk, mergePublishedPluginThunk } from "../figma_app/559491";
...
let $$v0 = createOptimistAction("SAVE_EXTENSION", (e, {
  ...
  N.then(({
    ...
    if (e.dispatch(mergePublishedPluginThunk({
      ...s,
      overrideInstallStatus: !0
    })), E?.(), !y) {
      let t = (() => {
        if (T === "inserts_modal") return r === ResourceTypeNoComment.PLUGIN ? getI18nString("community.saves.plugin_saved") : getI18nString("community.saves.widget_saved");
        if (p) {
    ...
    t.data.status === 403 ? e.dispatch(VisualBellActions.enqueue({
...
let $$A3 = createOptimistAction("UNSAVE_EXTENSION", (e, {
  ...
  b.then(({
    ...
    if (e.dispatch(mergePublishedPluginThunk({
      ...
      let n = {
        message: (() => {
          if (c === "inserts_modal") return r === ResourceTypeNoComment.PLUGIN ? getI18nString("community.saves.plugin_removed_from_your_account") : getI18nString("community.saves.widget_removed_from_your_account");
          if (i) {
...
let x = {
  [HubTypeEnum.PLUGIN](e) {
    return pluginAPIService.getInstallStatus({
      orgId: e
    });
  },
  [HubTypeEnum.WIDGET](e) {
