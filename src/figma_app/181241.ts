...
class BaseValidator<T> {
  ...
  private logValidationFailure(error: z.ZodError): void {
    ...
    if (this.logLevel === 'error') {
      x1('APIGetValidationFailure', `Validation failed for api response for ${this.debugKey}`, this.formatValidationErrors(error), {
  ...
  async validate(requestFn: (context: ValidationContext) => Promise<ValidatorResponse>): Promise<ValidatorResponse<T>> {
    ...
    try {
      ...
      if (error instanceof z.ZodError) {
        x1('ZODValidatorError', 'Validation failed for api response', {
