...
export const siteKitAtom = atom(async (get) => {
  ...
  const isReady = !!(Lg() || isInteractionPathCheck()) || get(processSelector).status === OperationStatus.COMPLETED
...
export function useSiteKitAssetsSubscription() {
  const { libraryKeys, enabled } = useAtomWithSubscription(siteKitState)
  const queries = libraryKeys.map(libraryKey =>
    LibraryAssetDataOfType.Query({
      libraryKey,
      assetType: FComponentType.RESPONSIVE_SET,
    }),
  )
  return setupMemoizedAtomSubscription(queries, { enabled })
}
...
export function useSiteKitAssets() {
  ...
  const { enabled } = useAtomWithSubscription(siteKitState)
  const loadedState: LoadingState<{
    ...
    if (enabled) {
      ...
      if (embedConfig)
        result.libraries.push(embedConfig)
    }
    return createLoadedState(result)
  }, [assetSubscriptions, enabled])

  return useMemo(() => {
    if (loadedState.status === 'loading')
      return createLoadingState()
    const data = loadedState.data
    return data.libraries.length > 0
      ? createLoadedState(data)
      : { status: 'disabled', data: null, errors: null }
  }, [loadedState])
}
...
export function usePublishedSiteKitComponents() {
  const { publishedFromSiteFile } = useAtomWithSubscription(siteKitState)
  const assetSubscriptions = useSiteKitAssetsSubscription()
  ...
  const componentQueries = libraryKeys.map(libraryKey =>
    LibraryComponentDataByLibraryKey.Query({ libraryKey }),
  )
  const componentSubscriptions = setupMemoizedAtomSubscription(componentQueries, { enabled: true })
  ...
  return useMemo(() => {
    ...
    for (const sub of componentSubscriptions) {
      ...
      if (!resource || !libraryKey)
        continue
      const components = resource.libraryHierarchyPaths
        .flatMap(path => path.components)
        .map(component => mapComponentProperties(component, {
          type: 'team',
          file: {
            key: '',
            teamId: null,
...
export function useSiteKitCmsConfig() {
  const { cmsConfig } = useAtomWithSubscription(siteKitState)
  return cmsConfig
}
...
export function useIsCmsLibraryKey() {
  const { cmsConfig } = useAtomWithSubscription(siteKitState)
  return useCallback(
    (libraryKey: string) => cmsConfig?.libraryKey === libraryKey,
    [cmsConfig],
  )
}
...
export function useAssetPrefixComparator() {
  const { sortedPrefixes } = useAtomWithSubscription(siteKitState)
  return useMemo(() => {
...
export function useLibraryDisplayName(components: any[]) {
  const libraryKeys = useMemo(
    () => components.map(comp =>
      comp.type === FComponentType.RESPONSIVE_SET
        ? comp.sourceLibraryKey
        : SITE_KIT_EMBEDS_LIBRARY_KEY,
    ),
    [components],
  )
  const { getLibrary } = useAssetPanelContext()
  const { data } = getLibraryNames(libraryKeys)
  return useCallback(
    (component: any) => {
      const key = component.type === FComponentType.RESPONSIVE_SET
        ? component.sourceLibraryKey
        : SITE_KIT_EMBEDS_LIBRARY_KEY
