...
export function useGetLicenseTypesForSeatType(options: {
  ...
  const computeLicenseTypesMap = useMemo(() => {
    ...
    const planTier
      = (options.overridePlanTier
        ? findMatchingValue(FPlanNameType, options.overridePlanTier)
        : undefined) || currentPlan?.tier

    // Map seat types to their allowed license types
    /** Original: o */
    const seatTypeLicenseTypes = {
      [ProductAccessTypeEnum.EXPERT]: filterNotNullish([
        FProductAccessType.DESIGN,
        FProductAccessType.DEV_MODE,
        FProductAccessType.WHITEBOARD,
        FProductAccessType.SLIDES,
        FProductAccessType.SITES,
        FProductAccessType.COOPER,
        isFigmakeSitesEnabled() ? FProductAccessType.FIGMAKE : undefined,
      ]),
      [ProductAccessTypeEnum.DEVELOPER]: filterNotNullish([
        FProductAccessType.DEV_MODE,
        FProductAccessType.WHITEBOARD,
        FProductAccessType.SLIDES,
        FProductAccessType.COOPER,
      ]),
      [ProductAccessTypeEnum.COLLABORATOR]: filterNotNullish([
        FProductAccessType.WHITEBOARD,
        FProductAccessType.SLIDES,
      ]),
      [ProductAccessTypeEnum.CONTENT]: filterNotNullish([
        FProductAccessType.COOPER,
        FProductAccessType.WHITEBOARD,
        FProductAccessType.SLIDES,
      ]),
    ...
    return collaboratorSet.dict((seatType) => {
      ...
      if (visibility === 'seat_description') {
        licenseTypes = licenseTypes.filter(type =>
          shouldShowSeatDescription(type, planTier),
        )
      }
      return licenseTypes
    })
  ...
  const getLicenseTypes = useCallback(
    (seatType: ProductAccessTypeEnum) =>
      computeLicenseTypesMap[seatType]?.sort(compareProductAccessOrder) || [],
    [computeLicenseTypesMap],
  )
