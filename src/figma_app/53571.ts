...
async function O(e, t, r, n, d) {
  ...
  if (zl.set(pc, !0), t === zIx.BUILD || t === zIx.COMPLETED) {
    try {
      let e = await _$$m(h, zi(t) ?? void 0, m, p, !0)
      u = e?.id
      h && !u && $D(_$$e.DEVELOPER_TOOLS, new Error('[Dev mode activity] createSavepoint returned no id'))
    }
    catch (e) {
      $D(_$$e.DEVELOPER_TOOLS, new Error(`[Dev mode activity] Error creating version on status change: ${e.stack}`))
    }
  }
  let y = !1
  l7(zk.SYSTEM, 'store-status-activity', () => {
    _$$e2.recordStatusChange({
      ...
      $D(_$$e.DEVELOPER_TOOLS, new Error(`[Dev mode activity] Error creating activity: ${e.stack}`))
    }).$$finally(() => {
