...
export function hslToRgb(hsl: HslColor): RgbColor {
  ...
  for (let i = 0; i < 3; i++) {
    ...
    if (a < 0) a++;
    if (a > 1) a--;
    if (6 * a < 1) n[i] = r + (t - r) * 6 * a;
    else if (2 * a < 1) n[i] = t;
    else if (3 * a < 2) n[i] = r + (t - r) * (2 / 3 - a) * 6;
    else n[i] = r;
  }
...
export function rgbToHsl(rgb: RgbColor): HslColor {
  ...
  else if (r === a) o = 2 + (n - t) / s;
  else if (n === a) o = 4 + (t - r) / s;
  o = Math.min(60 * o, 360);
  if (o < 0) o += 360;
  const l = (i + a) / 2;
  return {
...
const packRgba = (r: number, g: number, b: number, a: number): number =>
  clampToByte(r) | (clampToByte(g) << 8) | (clampToByte(b) << 16) | (clampToByte(a) << 24);
...
export function labToXyz(lab: LabColor): RgbColor {
  ...
  const n = matrixMultiply([
    ...
    const t = Math.abs(e);
    return t > 0.0031308 ? (e < 0 ? -1 : 1) * (1.055 * Math.pow(t, 1 / 2.4) - 0.055) : 12.92 * e;
  });
