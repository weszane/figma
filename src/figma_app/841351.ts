...
export function $$X17(e) {
  ...
  if (e.linkedVersion && e.activeId === e.linkedVersion.id)
    return e.linkedVersion
}
async function q(e, t, r) {
  ...
  return (trackEventAnalytics('versioning_performance_metrics', {
    ...
    forwardToDatadog: !0,
  }), Fullscreen.loadHistoryChangesState(o))
    ? ''
    : 'Error loading HistoryChangesState into memory.'
}
export function $$J1(e, t) {
  if (e)
    return t.versions.find(t => t.id === e)
}
...
let $$Q11 = createOptimistThunk(async (e, t) => {
  ...
  let d = a.user
  if (!getFeatureFlags().version_diffing || !getFeatureFlags().xr_debounce_threshold || !d || a.selectedView.view !== 'fullscreen' || a.selectedView.editorType === FEditorType.Whiteboard || a.mirror.appModel.topLevelMode === ViewType.HISTORY || a.mirror.appModel.activeCanvasEditModeType === LayoutTabType.COMPARE_CHANGES)
    return l()
  let {
    openFileKey,
  } = t
  ...
  let _ = _$$A(n)
  let h = p > _ ? p : _
  if (!h.isValid() || h < _$$A().subtract(7, 'days'))
    return l()
  ...
  let T = y[0]
  let v = b[0]
  if (!(_$$A(v.created_at) > _$$A(T.created_at)) || v.user_id === d.id || v.participating_users_array?.includes(d.handle))
    return l()
  e.dispatch($$eo6())
  e.dispatch($$ep16({
    fromVersionId: T.id,
...
let $$ee4 = createOptimistThunk(async (e, t) => {
  let r = e.getState()
  let n = r.user
  if (!getFeatureFlags().version_diffing || !n || r.selectedView.view !== 'fullscreen' || r.selectedView.editorType === FEditorType.Whiteboard || r.mirror.appModel.topLevelMode === ViewType.HISTORY || r.mirror.appModel.activeCanvasEditModeType === LayoutTabType.COMPARE_CHANGES)
    return
  let {
    openFileKey,
  } = t
  ...
  let u = l > o ? l : o
  if (!u.isValid() || u < _$$A().subtract(7, 'days'))
    return
  let p = Date.now()
    ...
    let E = loadCanvasData(u).then(([e]) => {
      if (E === er) {
        ...
        if ((a = t.getState().mirror.appModel.topLevelMode) === ViewType.HISTORY || a === ViewType.DEV_HANDOFF) {
          let a
          if (_)
            Fullscreen.applyFileToCurrentScene(e); else if (atomStoreManager.set($$V14, ''), !Fullscreen.loadFigFileForPreview(e))
            return c('Error loading this version into memory.')
