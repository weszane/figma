...
class DocumentEventAggregator {
  ...
  onEventFired(event: any) {
    ...
    let existingEvent = entry.events.find(
      (ev: any) => ev.type === event.type && ev.origin === event.origin,
    )
    if (existingEvent) {
      ...
      if (node) {
        if (
          event.type === SceneChangeType.STYLE_PROPERTY_CHANGE
          || event.type === SceneChangeType.STYLE_CREATE
          || event.type === SceneChangeType.STYLE_DELETE
        ) {
    ...
    if (node) {
      if (node.styleKeyForPublish) {
        const styleEvents = entry.events.filter(
          (ev: any) =>
            ev.type === SceneChangeType.STYLE_DELETE
            || ev.type === SceneChangeType.STYLE_CREATE
            || ev.type === SceneChangeType.STYLE_PROPERTY_CHANGE,
        )
        ...
        const nodeEvents = entry.events
          .filter(
            (ev: any) =>
              ev.type === SceneChangeType.CREATE
              || ev.type === SceneChangeType.PROPERTY_CHANGE
              || ev.type === SceneChangeType.DELETE,
          )
          .map((ev: any) => {
            ...
            if (containingCanvas === '') {
              reportError(
                ServiceCategories.EXTENSIBILITY,
                new Error('Containing canvas for nodechange event is empty'),
              )
            }
            return {
              ...ev,
  ...
  injectTypeIntoEvent(entry: any, event: any) {
    if (
      event.type === SceneChangeType.CREATE
      || event.type === SceneChangeType.DELETE
      || event.type === SceneChangeType.PROPERTY_CHANGE
    ) {
      return {
        ...event,
        nodeType: entry.nodeType,
  ...
  PluginCallbacks = {
    selectionChange(isBoxSelection: boolean) {
      for (const handler of selectionHandlers)
        isBoxSelection ? handler.boxSelection() : handler.normal()
    },
    boxSelectionEnded() {
    ...
    slidesViewChange(view: any) {
      for (const handler of slidesViewHandlers) handler({ view })
    },
    ...
    pluginAccessibleNodeIds() {
      ...
      for (const provider of accessibleNodeIdsProviders)
        provider().forEach(id => ids.add(id))
      return Array.from(ids)
    },
    pluginAccessiblePages() {
      const pages = new Set<string>()
      for (const provider of accessiblePagesProviders)
        provider().forEach(page => pages.add(page))
...
export function unregisterSelectionHandler(handler: () => void) {
  selectionHandlers = selectionHandlers.filter(
    h => h.normal !== handler || (h.boxSelection.cancel?.(), false),
  )
}
...
function updateDocumentChangeCallbackRegistration() {
  if (
    documentChangeHandlers.length
    || styleChangeHandlers.length
    || nodeChangeHandlers.length
  ) {
...
function createAsyncEventDispatcher() {
  ...
  return {
    ...
    runOrTimeout: async (timeoutMs: number, ...args: any[]) => {
      ...
      return didTimeout
        ? { didRun: false }
        : { didRun: true, returnValue: result }
    },
    reset,
  }
}
