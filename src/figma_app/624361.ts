...
export async function $$b1(e, t, r, n) {
  let i = t.type === 'figFile' ? `/file/${t.fileKey}/image/batch` : `/community/file/${t.hubFileId}/image/batch`;
  try {
    ...
    if (t.status !== 200 || t.data.error) throw new Error('api error while resolving images');
    return {
...
let A = class e {
  ...
  downloadFromS3() {
    if (!this.shouldDownloadCompressedTexture && this.s3Path == null) throw new Error('Called downloadFromS3 without s3Path');
    if (this.shouldDownloadCompressedTexture && this.compressedTextureS3Path == null) throw new Error('Called downloadFromS3 without compressedTextureS3Path when downloading compressed texture');
...
class x {
  ...
  updatePriority(e, t) {
    ...
    return r != null && (r.priority = t, this.needsSort = !0, !0);
  }
  pop(e) {
    if (this.ensureSorted(), this.queue.length === 0 || this.queue[0].priority < e) return null;
    let t = Date.now();
    let r = 1 / 0;
...
let w = class e {
  constructor() {
    ...
    this.flushPendingRecordImages = debounce(async (t = !1) => {
      ...
      d.forEach(async o => {
        if (o.type === 'fileKey' && u === o.fileKey) {
          n[o.fileKey].resolve();
          return;
        }
        if (o.type === 'fileKey' && !o.fileKey) {
        ...
        for (;;) {
          try {
            let r = await XHR.put(`/api/files/${u}`, {
              ...
              image_origin_file_key: o.type === 'fileKey' ? o.fileKey : void 0,
              image_origin_library_key: o.type === 'libraryKey' ? o.libraryKey : void 0,
              is_playground: t
            });
            let {
              success,
            ...
            if (e.status >= 400 && e.status < 500) {
              ...
              (o.type === 'fileKey' ? n[o.fileKey] : a[o.libraryKey]).reject(`4xx error when pasting images ${e}`);
              return;
            }
          }
  ...
  recordImagesFromLibraryKey(e, t, r = !1) {
    ...
    if (!e) return Promise.reject(new Error('No originLibraryKey specified'));
  ...
  recordImages(e, t) {
    if (t.length === 0) return Promise.resolve();
    if (!e) return Promise.reject(new Error('No originFileKey specified'));
    ...
    for (; this.pendingDownloads.size < this.MAX_REQUESTS && this.downloadQueue.length > 0 && !this.downloadsPaused;) {
      ...
      r.download(this.imageMaxSize).then(t => {
        ...
        if (e instanceof T) {
          ...
          if (e instanceof XHRError && e.status === 403) {
            this.markImageFailed(r.hash);
            return;
          }
  ...
  async lookupImages() {
    ...
    try {
      ...
      this.incomingImagesLookedUp != null && this.incomingImagesLookedUp.resolve();
    }
  ...
  forgetImage(e) {
    ...
    let t = this.allImages.get(e);
    t && (this.allImages.$$delete(e), t.state !== 5 && t.imageIsReady());
  }
