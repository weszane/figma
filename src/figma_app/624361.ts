...
export async function batchDownloadImages(imageHashes, fileInfo, maxSize, needsCompressedTextures) {
  ...
  try {
    ...
    if (response.status !== 200 || response.data.error)
      throw new Error('api error while resolving images')
    return {
  ...
  downloadFromS3(): Promise<Uint8Array<any>> {
    ...
    return XHR.crossOriginGetAny(url, null, {
      ...
      if (error.status !== 0 && error.status < 500) {
        const errorMessage = this.s3Path?.includes('resize')
          ? 'Can\'t load image! Make sure Pixie service is running locally'
          : 'Image is missing from S3!'
...
class ImageDownloadQueue {
  ...
  pop(minPriority: number): { found: true, image: ImageDownloadItem } | { found: false, retryDelay: number } | null {
    this.ensureSorted()
    if (this.queue.length === 0 || this.queue[0].priority < minPriority)
      return null
...
class ImageManager {
  ...
  pendingRecordImagesTasks: Record<string, { promise: Promise<void>, resolve: () => void, reject: (reason?: any) => void }> = {}
  pendingImagesToRecordByLibraryKey: Record<string, string[]> = {}
  pendingRecordImagesTasksByLibraryKey: Record<string, { promise: Promise<void>, resolve: () => void, reject: (reason?: any) => void }> = {}
  ...
  flushPendingRecordImages = debounce(async (isPlayground = false) => {
    ...
    type ProcessItem = { fileKey: string, type: 'fileKey' } | { libraryKey: string, type: 'libraryKey' }

    const itemsToProcess: ProcessItem[] = [
      ...fileKeys.map(key => ({ fileKey: key, type: 'fileKey' as const })),
      ...libraryKeys.map(key => ({ libraryKey: key, type: 'libraryKey' as const })),
    ]
    ...
    itemsToProcess.forEach(async (item) => {
      ...
      for (;;) {
        try {
          ...
          const { success, failed } = ImageManager.parseFileUpdateResponseForImages(imageHashes, response)
  ...
  recordImagesFromLibraryKey(libraryKey: string, imageHashes: string[], isPlayground = false): Promise<void> {
    ...
    if (!libraryKey)
      return Promise.reject(new Error('No originLibraryKey specified'))
  ...
  maybeDownloadMoreImages(): void {
    ...
    while (
      this.pendingDownloads.size < this.MAX_REQUESTS
      && this.downloadQueue.length > 0
      && !this.downloadsPaused
    ) {
      ...
      if (queueResult.found === false) {
        const { retryDelay } = queueResult
        if (!Number.isFinite(retryDelay) || retryDelay <= 0) {
          reportError(
            ServiceCategories.SCENEGRAPH_AND_SYNC,
            new Error('Logic error in download queue, cannot compute next retry'),
          )
        }
        else {
      ...
      const imageItem = (queueResult as { found: true, image: ImageDownloadItem }).image
      ...
      imageItem.download(this.imageMaxSize)
        .then((data) => {
          // If download was cancelled, exit early
          if (this.cancelDownloads.delete(imageItem.hash))
            return
          ...
          let textureMeta: { width: number, height: number } | null = null
          ...
          if (this.imagesNeedingDirectUpload.has(imageItem.hash)) {
            ...
            let imageData = data
            if (
              ImageManager.shouldUseCompressedTextures()
              && data.slice(0, 12).toString() === KTX_HEADER
            ) {
              const ktx = read(data)
              textureMeta = {
          ...
          if (
            !textureMeta
            && !this.pendingDecodes.has(imageItem.hash)
          ) {
            ...
            reportError(
              ServiceCategories.SCENEGRAPH_AND_SYNC,
              new Error('missing pending decode after image marked as downloaded'),
            )
  ...
  async lookupImages(): Promise<void> {
    ...
    for (const image of this.incomingImages.values()) {
      if (image.state === ImageState.AWAITING_SINATRA_LOOKUP) {
        ...
        this.verbose && logInfo('image', 'lookup', { hash: image.hash })
      }
    }
    ...
    try {
      ...
      while (lookupItems.length > 0) {
        ...
        for (const image of batch) {
          ...
          if (pastePromise) {
            try {
              ...
              logError('lookupImages', 'pastePromise failed', { hash: image.hash })
            }
          }
        }
        ...
        for (const image of batch) {
          ...
          const compressedTextureUrl = ImageManager.shouldUseCompressedTextures()
            ? resolvedUrls.compressed_texture_s3_urls[image.hash]
            : null
          ...
          if (s3Url) {
            ...
            this.verbose && logInfo('image', 'enqueued-batch', { hash: image.hash })
          }
          else {
            this.markImageMissing(image.hash)
            this.verbose && logInfo('image', 'missing', { hash: image.hash })
          }
      ...
      for (const image of lookupItems) {
        if (this.incomingImages.delete(image.hash)) {
          logWarning('image', 'incoming fallback delete', { hash: image.hash })
        }
      }
  ...
  forgetImage(e) {
    if (this.imagesNeedingDirectUpload.has(e))
      return
  ...
  imageQueueEmptyPromise(): Promise<void> {
    if (!this.imageQueueEmpty) {
      ...
      this.verbose && logInfo('image', 'queue empty defer installed', { start })
      this.imageQueueEmpty.promise.finally(() => {
        this.imageQueueEmpty = null
        const elapsed = performance.now() - start
        if (this.verbose) {
          logInfo('image', 'queue empty defer cleared', { start, elapsed })
        }
        else if (elapsed > 1e4) {
          logWarning('image', 'queue empty finished after a long time', { start, elapsed })
        }
      })
  ...
  unresolvedImagesUnder(
    ...
    return hashes
      ? hashes
          .map(hash => this.allImages.get(hash))
          .filter(item => item !== undefined && item.state !== ImageState.READY && item.state !== ImageState.UPLOADING)
      : []
  }
  ...
  loadAllImagesUnder(
    ...
    exportType: ImageExportType,
    reason: string,
    progressCallback: ((remaining: number, total: number) => void) | null = null,
    activeDocument?: any,
  ...
  async loadAllImages(
    ...
    if (guids.includes('0:0') && reason !== 'playground.initialLoad') {
      logError('image', 'loadAllImagesUnder called with full document', { reason })
    }
  ...
  startUploadingImage(fileKey: string, imageData: any, contentType: string, hash: string): void {
    ...
    XHR.post(uploadUrl, imageData, {
      ...
      (response) => {
        ...
        logInfo('image', 'uploaded new image', { hash })
      ...
      (error) => {
        ...
        if (error.status === 415) {
          logError('image', 'image upload failed, removing invalid image', { hash })
...
export async function processImageWithThumbnail(imageData: ArrayBuffer, mimeType: string, imageName?: string) {
  if (!ImageCppBindings.imageTypeDecodeSupported(mimeType))
    throw new Error(`Image type ${mimeType} is not supported and cannot be decoded`)
...
async function loadImagesAndExport(exportInfo: any, exportOptions: any) {
  ...
  await getImageManager().loadAllImagesUnder(
    [includeOutsideContents ? exportInfo.containingCanvas || '-1:-1' : exportInfo.guid],
    ImageExportType.NON_ANIMATED_ONLY,
    'plugins.loadImagesAndExport',
    null,
    true,
  )
}
