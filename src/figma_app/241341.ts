...
import { activeVisualAttributes, ProjectDevelopmentPhases, styleIdMapping } from '../905/869235'
...
import { DiffImpl, FileSourceType, Fullscreen, SceneGraphHelpers, UIVisibilitySetting } from '../figma_app/763686'
import { canvasCache, CURRENT_VERSION_ID, invalidateCanvasCache, loadCanvasForVersion, setActiveVersion, versionHistoryKeyAtom } from '../figma_app/841351'
...
export function $$eE2(e) {
  ...
  useEffect(() => {
    if (origin === 'cc_versions' && eI && !ed) {
      mf(eI)
        ? trackEventAnalytics('Diff Modal No Changes Between Versions', {
            userId: k,
            fileKey: D,
            frameId: nodeId,
            versionId: selectedVersion?.id,
  ...
  useEffect(() => {
    if (!eh?.id || !eh?.zoomToSelection || !nodeId || !eH)
      return
    let {
      baseBeforeBox,
      baseAfterBox,
...
let $$eT0 = registerModal((e) => {
  ...
  useEffect(() => {
    ...
    loadCanvasForVersion(N, nodeId)?.then(({
      ...
      if (r === O)
        return
      if (!Fullscreen.loadPartialHistoryScene(t))
        throw new Error('Error loading history canvas')
      R(r)
  ...
  return jsx($$eE2, {
    ...
    historicImage: H,
    modalError: j,
    modalTitle: layerName
      ? getI18nString('collaboration.feedback.compare_changes_modal.layer_name_header', {
          layerName,
        })
