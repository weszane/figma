...
import { NodePropertyCategory, NodeType, PropertyScope, VariableResolvedDataType } from "../figma_app/763686"
...
export function $$R2(e) {
  let t
  if (e?.length === 0)
    return renderI18nText("variables.scopes.no_supported_properties")
  if (e.includes(PropertyScope.ALL_SCOPES))
    return renderI18nText("variables.scopes.all_properties_label")
  for (let r of mapFilter(e, e => $$O6(e))) {
    t = t
      ? jsx(Fragment, {
...
export function $$F1(e, t) {
  ...
  for (let n of e.scopes) {
    if (t.has(n) || n === PropertyScope.ALL_SCOPES || n === PropertyScope.ALL_FILLS && r)
      return !0
  }
  return !1
}
