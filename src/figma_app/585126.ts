    ...
    case 'billingGroup':
      {
        ...
        const matchedKey = Object.keys(groupMap).find(key => groupMap[key]?.name === decodedValue);
        if (value === 'unassigned' || value === 'Unassigned') return 'Unassigned';
 ...
 * @param groupMap
 */
export function setupFiltersFromQuery(queryString: string, groupMap: Record<string, any>): typeof DefaultFilters | null {
  if (!queryString) return null;
  const query = parseQuery(queryString);
  return Object.keys(query).reduce((filters, groupKey) => {
    if (!(groupKey in GroupKeys)) return filters;
 ...
 * @param groupMap
 */
export function setupGroupsFromFilters(filters: Record<string, any>, groupMap: Record<string, any>): Record<string, any> {
  return Object.keys(filters).reduce((groups, filterKey) => {
    ...
    return {
      ...groups,
      ...(value && {
        [groupKey]: mapFilterKeyToGroupValue(filterKey, value, groupMap)
      })
    };
  }, {});
}
