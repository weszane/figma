...
import { findLocalPluginById, findPublishedWidgetById, getLocalPlugins, useAllowlistedPlugins, useAllowlistedWidgets, useCanPerformAction, useInstalledPluginsAndWidgets, useLocalPluginsExcludingWidgets, usePluginAllowlistValidation, usePluginedWidgets, usePublishedPlugins, usePublishingPlugins, useRecentlyUsedPluginsMap, useRecentlyUsedWidgetsMap, useUserPublishedPlugins, useUserPublishedWidgets, useWidgetAllowlistValidation } from '../figma_app/844435'
import { desktopAPIInstance } from '../figma_app/876459'
import { $C, B6 as _$$B2, vt as _$$vt, BG, Bx, c6, Cq, D1, FV, Gy, JW, lI, lO, lP, MJ, ML, NH, o2, od, ot, Pg, QF, rz, sI, Sx, UK, VJ, Vx, Y2, y2, y7, Y9, Yl, ZK, Zv, zZ } from '../figma_app/968638'
import { isDevHandoffEditorType } from '../figma_app/976749'
import { dn, li, ZA } from '../figma_app/994403'
...
function em(e) {
  ...
  if (e.resource.is_private) {
    ...
    return jsx(_$$s, {
      ...
      'data-tooltip': t
        ? getI18nString('community.plugins.private_for_plugin_org_name', {
            orgName: t,
          })
...
function ef(e, i, t) {
  ...
  let o = t && i && i.length >= 2 && i[0] === B6 && i[1] === 0
  useEffect(() => {
...
export function $$ey4({
  ...
  let o = getLocalPlugins()
  let u = i === 'development' ? Object.values(o).find(i => String(i.localFileId) === e) : null
  let c = u?.plugin_id
...
function e_({
  ...
  let i = useRecentlyUsedPluginsMap()[e]
  let t = usePublishedPlugins()[e]
  let l = isResourcePaymentPending(t)
  return i
    ? hasLocalFileId(i)
      ? jsx(eN, {
...
function ek(e) {
  return usePublishedPlugins()[e.pluginId]
    ? jsx(eR, {
        pluginId: e.pluginId,
        type: 'org_private',
      })
...
function eP(e) {
  ...
  let x = useTracking()
  let R = getPublishedPluginWithPayment(e.pluginId)
  let F = Object.values(getLocalPlugins()).find(i => String(i.plugin_id) === e.pluginId)
  let N = getPluginVersion(R)
  let k = isDevModeWithCodegen(N)
  let B = checkResourceEligibility(R) && !F
  let E = checkResourceSubscriptionActive(R)
  let D = isResourcePaymentFailed(R) && !F
  let M = useCurrentUserOrg()
  let z = useAllowlistedPlugins()
  let U = !F && canRequestExtensionVersion({
    org: M,
...
function eE(e) {
  let i = useRecentlyUsedWidgetsMap()[e.widgetId]
...
function eA(e) {
  ...
  } = useWidgetAllowlistValidation(e.widgetId)
  let _ = useCallback((t) => {
    if (j) {
  ...
  let T = useCallback((e) => {
    j
      ? _(e)
      : permissionScopeHandler.user('insert-published-widget', () => {
          b()
        })
  }, [b, _, j])
  let C = useCallback((t) => {
...
function eD(e) {
  return usePluginedWidgets()[e.widgetId]
    ? jsx(eA, {
        widgetId: e.widgetId,
        type: 'org_private',
      })
...
function eO(e) {
  ...
  let t = usePluginedWidgets()[widgetId] ?? null
  if (!t)
    return null
...
function eV(e) {
  let i = getPluginedWidgetWithPayment(e.widgetId)
  if (!i)
    return null
  let t = getPluginVersion(i)
  let l = fO(i)
...
let eM = memo((e) => {
  ...
  let c = usePluginedWidgets()[e.widgetId]
  let g = checkResourceEligibility(c)
  let v = isResourcePaymentFailed(c)
  let I = c ? getPluginVersion(c) : null
  let m = i.name
  I?.name && I?.name !== '' && (m = I?.name)
  let x = useCurrentUserOrg()
  let j = useAllowlistedWidgets()
  let b = !e.localWidget && canRequestExtensionVersion({
    org: x,
...
export function $$ez5(e) {
  ...
  let w = useCallback((t) => {
    if (t.stopPropagation(), validatePublishedPluginInOrgAllowlist()) {
      if (g) {
        I
          ? i(hideDropdownAction())
          : (e.dropdownShownType && i(showDropdownThunk({
  ...
  let _ = e.usePrimaryButtonStyles
    ? jsxs(ButtonBasePrimary, {
        onClick: w,
        className: lI,
...
function e$(e) {
  ...
  return e.plugin.monetized_resource_metadata
    ? jsxs('div', {
        className: y2,
        children: [jsx('div', {
          className: lO,
