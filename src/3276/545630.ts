...
import { t$ as _$$t$, vl, wB, rN, hx as _$$hx, eR as _$$eR, F$ } from "../figma_app/12220";
...
import { Rf, sj, UR, Y6, eW as _$$eW, fG, pC, gu } from "../905/301347";
...
import { wG, MJ, F7, xS, eX as _$$eX, aW, VM, UR as _$$UR, wC, xe, wb, OM } from "../905/649567";
import { qoo, u24, IuL, M$q } from "../figma_app/27776";
...
class J {
  constructor(e, t, n, o) {
    ...
    let a = eo(e, n);
    if (!a) {
      o || 1 !== e.pins.length || e.pins[0].isOurs || (this.expandPin = !0);
      return;
    }
...
let eo = (e, t) => {
  if (1 === e.pins.length) return t.getParentOf(e.id);
...
let eN = memo(function ({
  ...
  let {
    ...
    let _ = useCallback(t => {
      0 !== t.button || ek(t) || "idle" !== r || (e.current?.setPointerCapture(t.pointerId), t.stopPropagation(), p({
        x: t.clientX,
        y: t.clientY
      }), l("ready"), m(!1));
    }, [r, e]);
    let g = useCallback(e => {
      if (m(!0), "idle" === r || !u || !t) return;
      let o = {
        x: e.clientX,
        y: e.clientY
      };
      ...
      f(a);
      "ready" === r ? (l("dragging"), n(a)) : i(a);
    }, [r, n, i, u, t]);
    let v = useCallback(t => {
      "dragging" === r ? (e.current?.releasePointerCapture(t.pointerId), t.stopPropagation(), o(h), f({
        x: 0,
        y: 0
      })) : d || s();
      l("idle");
    }, [e, r, h, o, s, d]);
    useEffect(() => ("idle" !== r && (document.addEventListener("pointermove", g), document.addEventListener("pointerup", v)), () => {
  ...
  return jsxs(Fragment, {
    children: [jsx("div", {
      ...
      style: {
        ...
        transform: 0 !== dragOffset.x || 0 !== dragOffset.y ? `translate3d(${-dragOffset.x}px, ${-dragOffset.y}px, 0px)` : void 0
      },
      onPointerDown,
      ...
      children: jsx(Fragment, {
        children: w.map(({
          ...
          return jsx(_$$Fragment, {
            children: 1 === n.pins.length ? jsx(eD, {
              pin: n.pins[0],
...
let eF = (e, t, n, o) => {
  ...
  let r = new Map();
  for (let e of i) for (let t of e.avatars) r.has(t.avatar_user_id) ? (r.get(t.avatar_user_id).num_unread_comments += t.num_unread_comments, r.get(t.avatar_user_id).num_comments += t.num_comments) : r.set(t.avatar_user_id, t);
  return {
    id: 1 === threads.length ? threads[0].id : Ao(e, threads.length),
    x: e.x,
    y: e.y,
    pins: i,
...
let eU = memo(function (e) {
  ...
  useEffect(() => {
    ...
    Math.abs(deltaZoomScale) / zoomScale > .45 || Math.abs(deltaOffsetX) / width > .95 || Math.abs(deltaOffsetY) / height > .95 ? ec(1) : em(0);
...
function th(e) {
  ...
  let u = useCallback(e => {
    "Enter" === e.key && (e.preventDefault(), r ? editorRef.current?.onSubmitHyperlink(r) : editorRef.current?.onRemoveHyperlink(), editorRef.current?.focus(), closeEditor());
...
function tV(e) {
  ...
  let f = GH();
  let v = useMemo(() => f && "loaded" === f.status && f.data && f?.feedApi?.reactionsApi ? f.feedApi.reactionsApi.getReactionsForPost(e.feedPost.feedPostPublicUuid) : {
    postReactions: [],
    commentReactions: {}
  ...
  let M = useCallback((e, t) => {
    ...
    t ? t.then(() => _$$J.$$delete(e)) : _$$J.$$delete(e);
  }, [dispatch, k]);
  let E = useCallback(() => {
    r && (dispatch(gX({
      postUuid: gk(k),
...
function nr(e) {
  ...
  let A = useCallback(n => {
    ...
    if ("fullscreen" === t && !n.page) return new Point(x, y);
  ...
  let eu = useCallback((t, n) => {
    ...
    1 === f.length && ed.current?.focus();
    n ? n.then(() => _$$l.$$delete(e.thread.key, t)) : _$$l.$$delete(e.thread.key, t);
  ...
  let ej = useCallback(() => {
    if (setIsPinned(!1), ew(), !_$$I(p)) {
      if (u && ed.current?.isAllSelected() || Kx(p) && 0 === f.length) {
        el([]);
        dispatch(pD());
        return;
      }
  ...
  let eR = useMemo(() => function (e) {
    return jsx(ns, {
      ...
      positionRef: k,
      threadId: ei,
      disableCentering: "communityHub" === t || "prototype" === t,
      children: e.children
    });
  }, [ei, k, ed, t]);
        ...
        children: [eS && eA && jsx(th, {
          ...
          children: [jsx(t8, {
            ...
            children: [jsx("div", {
              ...
              mentionables: e.mentionables,
              mountInputFocused: !0,
              prevAuthor: 0 === a ? null : comments[a - 1].user_id,
...
function ny(e) {
  ...
  return jsx(nx, {
    ...
    useAbsolute: "communityHub" === s.view
  });
}
...
function nP(e) {
  ...
  let D = useCallback((t, n) => {
    N && (d(_$$nb({
      attachmentId: t,
      attachment: null
    })), 0 === Object.keys(S.getState().comments.newComment.attachments || {}).length && e.editorRef.current?.focus(), n ? n.then(() => _$$l.$$delete(N, t)) : _$$l.$$delete(N, t));
  }, [d, N, S, e.editorRef]);
  let A = useCallback(() => {
    onSubmit(e.thread);
  }, [onSubmit, e.thread]);
  ...
  let O = useCallback(() => {
    ...
    if (_$$I(t.messageMeta)) {
      R();
      return;
    }
    (discardAttempts && e.editorRef.current?.isAllSelected() || Kx(t.messageMeta) && 0 === Object.keys(t.attachments || {}).length) && (d(Z5({
      messageMeta: []
    })), R());
...
function nG(e) {
  ...
  let j = useRef();
  if (useEffect(() => {
    if (j.current && "details" === e.mode && !e.activeId) {
      j.current();
      return;
    }
  }, [e.mode, j, e.activeId]), !s) return null;
  let {
    x: _x3,
    y: _y3,
    width,
...
export function $$nY1(e, t, n) {
  if (0 === e.comments.length) return !1;
...
function n0(e) {
  ...
  t = g.status;
  _$$t(e => {
    "loaded" === t && e("comments_rendered");
  }, [t]);
  let x = hC(h.threads, []);
  let b = "loaded" === h.threads.status;
  let y = Le();
  ...
  let [A, L] = useMemo(() => {
    ...
    return -1 === e ? [void 0, void 0] : [D[e - 1], D[e + 1]];
  }, [D, activeId]);
  ...
  return jsxs(Fragment, {
    children: [getFeatureFlags().comments_react ? jsx(eH, {
      ...
      ref: F,
      activeId,
      commentCreationDisabled: "pins" === e.mode || e.commentCreationDisabled,
...
function n2(e) {
  ...
  let g = useCallback(e => {
    ...
    "loaded" === t.status && t.data.find(t => t.id === e.threadId) && (e.onCommentValidationFailure = (t, o, a, i) => dB(f, s, t, o, a, i, e.uuid, n), s(lV({
      ...e,
      commentsWriteApi: f,
