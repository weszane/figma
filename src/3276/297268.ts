...
import { CLUSTER_SIZE } from '../905/936492';
import { g as _$$g, H as _$$H } from '../3276/441754';
...
let ee = CommentPinElement.getPinSize(3);
let et = Math.max(CLUSTER_SIZE, ee.width, ee.height);
...
export function $$ea2({
  ...
  return Math.min(u, e.minZoomPercentage / 100 + 0.5);
}
...
let es = (e, t, n, o, a, i) => {
  ...
  let d = new Map();
  for (let e of r) {
    for (let t of e.avatars) d.has(t.avatar_user_id) ? (d.get(t.avatar_user_id).num_unread_comments += t.num_unread_comments, d.get(t.avatar_user_id).num_comments += t.num_comments) : d.set(t.avatar_user_id, t);
  }
  let c = threads.length === 1;
  return {
    id: c ? threads[0].id : formatPinCoordinates(e, threads.length),
    x: e.x,
    y: e.y,
    pins: r,
...
'customElements' in window && customElements.define(ClusteredPinsElement.desiredElementName, ClusteredPinsElement);
// todo: this giant component
let er = memo(e => {
  ...
  let ej = useAtomWithSubscription(isOnboardingCompletedAtom);
  let ek = useRef(null);
  ...
  let e$ = useCallback(e => {
    if (ez) {
      if (typeof ez.onViewportUpdate != 'function') {
        reportError(ServiceCategories.WAYFINDING, new Error('pinRenderer.onViewportUpdate is not a function'), {
          extra: {
  ...
  let eK = useCallback(e => {
    let t;
    let o = aggregateUserComments(e.comments);
    let a = e.comments.slice(1).reduce((e, t) => t.isUnread ? e + 1 : e, 0);
    if (e.boundingBoxAnchorCanvasPosition) {
      t = e.boundingBoxAnchorCanvasPosition;
    } else if (e.comments[0].client_meta?.selection_box_anchor && (t = e.comments[0].client_meta.selection_box_anchor, e.comments[0].client_meta?.in_frame)) {
    ...
    let r = {
      ...
      isPinnedToFile: isPinnedCommentsEnabled() ? !!e.commentPin : void 0
    };
  ...
  let e5 = (() => {
    let e = useLabConfiguration(labConfigurations.commentsA11y);
    return useCallback((t, n) => o => {
      e && o.key === 'Enter' && eV(t, n)().then(() => {
  ...
  useEffect(() => {
    ...
    Math.abs(deltaZoomScale) / zoomScale > 0.45 || Math.abs(deltaOffsetX) / width > 0.95 || Math.abs(deltaOffsetY) / height > 0.95 ? e4(1) : e6(0);
  ...
  let te = e.activeThread ? aggregateUserComments(e.activeThread.comments).length : 1;
  let {
    setActivePinSize
  } = e;
  useEffect(() => {
    ...
    if (eo) {
      let e = CommentPinElement.getPinSize(1);
      setActivePinSize({
        height: e.height,
        width: e.width
      });
    ...
    setActivePinSize({
      width: CommentPinElement.getPinSize(te).width,
      height: 0
    });
  ...
  let ta = useLabConfiguration(labConfigurations.commentsA11y);
  let ti = useCallback(() => {
    let e = clusteredPinsInstance?.getFocusedClusterId();
  ...
  let tr = function (e) {
    ...
    return useCallback(n => {
      let o = clusteredPinsInstance?.getFocusedClusterId();
      let a = e.all().map(e => e.id);
      let i = (a.findIndex(e => e === o) + (n === 1 ? -1 : 1) + a.length) % a.length;
      let s = e.all()[i];
      if (!s) return;
      let r = a[i];
      t(s.cluster)?.then(() => {
        clusteredPinsInstance?.focusPinById(r);
      });
    }, [e, t]);
  }(e7);
