...
export function $$em1(e) {
  ...
  let eg = q5();
  let eh = eg?.key || null;
  let ef = d4(e => e.currentUserOrgId);
  let eC = eg?.teamId;
  let e_ = d4(e => e.orgUsersByOrgId);
  let eb = _$$i();
  let ey = d4(e => e.comments.editingComment);
  ...
  let eT = lg();
  let ej = HW();
  let ew = d4(e => e.comments.activeDragTarget);
  let eR = _$$y();
  let ek = md(_$$H);
  ...
  useEffect(() => {
    ...
    uE("Comment composer opened", eF.getState(), {
      isThread: t
    });
    $ || eF.dispatch(_$$b({
  ...
  let eS = d4(e => e.comments.emojiPicker);
  ...
  let eK = qP(e.recordingKey, "update", ([t, n]) => {
    if (!n) {
      e.updateMessage(t);
      return;
    }
  ...
  let eU = qP(e.recordingKey, "emojiPick", t => {
