...
var s = [`a[href]${i.inert}${i.negTabIndex}`, `area[href]${i.inert}${i.negTabIndex}`, `input:not([type="hidden"]):not([type="radio"])${i.inert}${i.negTabIndex}${i.disabled}`, `input[type="radio"]${i.inert}${i.negTabIndex}${i.disabled}`, `select${i.inert}${i.negTabIndex}${i.disabled}`, `textarea${i.inert}${i.negTabIndex}${i.disabled}`, `button${i.inert}${i.negTabIndex}${i.disabled}`, `details${i.inert} > summary:first-of-type${i.negTabIndex}`, `iframe${i.inert}${i.negTabIndex}`, `audio[controls]${i.inert}${i.negTabIndex}`, `video[controls]${i.inert}${i.negTabIndex}`, `[contenteditable]${i.inert}${i.negTabIndex}`, `[tabindex]${i.inert}${i.negTabIndex}`];
function o(e) {
...
function v(e) {
  return (!e.shadowRoot || "-1" !== e.getAttribute("tabindex")) && !e.matches(":disabled,[hidden],[inert]");
}
...
export class $$w0 {
  ...
  bindKeypress(e) {
    if (O('[aria-modal="true"]', y()) !== this.$el) return;
    let r = !1;
    try {
      r = !!this.$el.querySelector('[popover]:not([popover="manual"]):popover-open');
    } catch {}
    "Escape" !== e.key || "alertdialog" === this.$el.getAttribute("role") || r || (e.preventDefault(), this.hide(e));
    "Tab" === e.key && b(this.$el, e);
  }
  maintainFocus(e) {
...
function k() {
  for (let e of document.querySelectorAll("[data-a11y-dialog]")) new $$w0(e);
}
"undefined" != typeof document && ("loading" === document.readyState ? document.addEventListener("DOMContentLoaded", k) : k());
export const A = $$w0;