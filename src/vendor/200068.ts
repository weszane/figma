...
function y(e, r) {
  ...
  for (let i of e.checks) {
    switch (i.kind) {
      case "min":
        r.target === "jsonSchema7" ? i.inclusive ? m(n, "minimum", i.value, i.message, r) : m(n, "exclusiveMinimum", i.value, i.message, r) : (i.inclusive || (n.exclusiveMinimum = !0), m(n, "minimum", i.value, i.message, r))
        break
      case "max":
        r.target === "jsonSchema7" ? i.inclusive ? m(n, "maximum", i.value, i.message, r) : m(n, "exclusiveMaximum", i.value, i.message, r) : (i.inclusive || (n.exclusiveMaximum = !0), m(n, "maximum", i.value, i.message, r))
        break
      case "multipleOf":
...
function S(e, r) {
  return r.effectStrategy === "input" ? eo(e.schema._def, r) : {}
}
function E(e) {
  return {
    type: "string",
...
let A = e => (!("type" in e) || e.type !== "string") && "allOf" in e
function C(e, r) {
  ...
  let i = r.target === "jsonSchema2019-09"
    ? {
...
let I = {
  ...
  emoji: () => (void 0 === i && (i = new RegExp("^([\\p{Extended_Pictographic}\\p{Emoji_Component}])+$", "u")), i),
  ipv4Cidr: /^(?:(?:25[0-5]|2[0-4]\d|1\d\d|[1-9]\d|\d)\.){3}(?:25[0-5]|2[0-4]\d|1\d\d|[1-9]\d|\d)\/(3[0-2]|[12]?\d)$/,
  ipv6Cidr: /^(([0-9a-fA-F]{1,4}:){7}[0-9a-fA-F]{1,4}|([0-9a-fA-F]{1,4}:){1,7}:|([0-9a-fA-F]{1,4}:){1,6}:[0-9a-fA-F]{1,4}|([0-9a-fA-F]{1,4}:){1,5}(:[0-9a-fA-F]{1,4}){1,2}|([0-9a-fA-F]{1,4}:){1,4}(:[0-9a-fA-F]{1,4}){1,3}|([0-9a-fA-F]{1,4}:){1,3}(:[0-9a-fA-F]{1,4}){1,4}|([0-9a-fA-F]{1,4}:){1,2}(:[0-9a-fA-F]{1,4}){1,5}|[0-9a-fA-F]{1,4}:((:[0-9a-fA-F]{1,4}){1,6})|:((:[0-9a-fA-F]{1,4}){1,7}|:)|fe80:(:[0-9a-fA-F]{0,4}){0,4}%[0-9a-zA-Z]+|::(ffff(:0{1,4})?:)?((25[0-5]|(2[0-4]|1?\d)?\d)\.){3}(25[0-5]|(2[0-4]|1?\d)?\d)|([0-9a-fA-F]{1,4}:){1,4}:((25[0-5]|(2[0-4]|1?\d)?\d)\.){3}(25[0-5]|(2[0-4]|1?\d)?\d))\/(12[0-8]|1[01]\d|[1-9]?\d)$/,
  base64: /^([0-9a-z+/]{4})*(([0-9a-z+/]{2}==)|([0-9a-z+/]{3}=))?$/i,
  base64url: /^([\w-]{4})*(([\w-]{2}(==)?)|([\w-]{3}(=)?))?$/,
  nanoid: /^[\w-]{21}$/,
  jwt: /^[\w-]+\.[\w-]+\.[\w-]*$/,
}
function P(e, r) {
  ...
  if (e.checks) {
    for (let i of e.checks) {
      switch (i.kind) {
        case "min":
          m(n, "minLength", typeof n.minLength == "number" ? Math.max(n.minLength, i.value) : i.value, i.message, r)
          break
        case "max":
          m(n, "maxLength", typeof n.maxLength == "number" ? Math.min(n.maxLength, i.value) : i.value, i.message, r)
          break
        case "email":
        ...
        case "cuid2":
          ...
          break
        case "startsWith":
          $(n, new RegExp(`^${R(i.value, r)}`), i.message, r)
          break
        case "endsWith":
          $(n, new RegExp(`${R(i.value, r)}$`), i.message, r)
          break
        ...
        case "includes":
          ...
          break
        case "ip":
          i.version !== "v6" && N(n, "ipv4", i.message, r)
          i.version !== "v4" && N(n, "ipv6", i.message, r)
          break
        case "base64url":
        ...
        case "jwt":
          ...
          break
        case "cidr":
          i.version !== "v6" && $(n, I.ipv4Cidr, i.message, r)
          i.version !== "v4" && $(n, I.ipv6Cidr, i.message, r)
          break
        case "emoji":
...
function R(e, r) {
  return r.patternStrategy === "escape" ? D(e) : e
}
...
function N(e, r, n, i) {
  e.format || e.anyOf?.some(e => e.format)
    ? (e.anyOf || (e.anyOf = []), e.format && (e.anyOf.push({
        format: e.format,
...
function $(e, r, n, i) {
  e.pattern || e.allOf?.some(e => e.pattern)
    ? (e.allOf || (e.allOf = []), e.pattern && (e.allOf.push({
        pattern: e.pattern,
        ...(e.errorMessage && i.errorMessages && {
          errorMessage: {
...
function j(e, r) {
  if (r.target === "openAi" && console.warn("Warning: OpenAI may not support records in schemas! Try an array of key-value pairs instead."), r.target === "openApi3" && e.keyType?._def.typeName === ZodFirstPartyTypeKind.ZodEnum) {
    return {
      type: "object",
  ...
  if (e.keyType?._def.typeName === ZodFirstPartyTypeKind.ZodEnum) {
    return {
      ...n,
      propertyNames: {
...
function Z(e) {
  let r = e.values
  let n = Object.keys(e.values).filter(e => typeof r[r[e]] != "number").map(e => r[e])
...
let B = (e, r) => {
  ...
  })).filter(e => !!e && (!r.strictUnions || typeof e == "object" && Object.keys(e).length > 0))
  return n.length
    ? {
        anyOf: n,
      }
...
function G(e, r) {
  ...
  switch (e.unknownKeys) {
    ...
    case "strip":
      return r.removeAdditionalStrategy === "strict" ? r.allowedAdditionalProperties : r.rejectedAdditionalProperties
  }
}
function X(e) {
  try {
...
let H = (e, r) => {
  if (r.currentPath.toString() === r.propertyPath?.toString())
    return eo(e.innerType._def, r)
  let n = eo(e.innerType._def, {
    ...r,
...
let K = (e, r) => {
  if (r.pipeStrategy === "input")
    return eo(e.$$in._def, r)
  if (r.pipeStrategy === "output")
    return eo(e.out._def, r)
  let n = eo(e.$$in._def, {
    ...r,
    currentPath: [...r.currentPath, "allOf", "0"],
  })
...
function eo(e, r, n = !1) {
  ...
  let h = typeof a == "function" ? eo(a(), r) : a
...
let ea = (e, r) => {
  switch (r.$refStrategy) {
    ...
    case "seen":
      ...
      return r.$refStrategy === "seen" ? {} : void 0
  }
}
let el = (e, r) => {
  let n = 0
...
let $$ec0 = (e, r) => {
  let n = h(r)
  let i = typeof r == "object" && r.definitions
    ? Object.entries(r.definitions).reduce((e, [r, i]) => ({
        ...e,
        [r]: eo(i._def, {
          ...n,
    ...
    : void 0
  let s = typeof r == "string" ? r : r?.nameStrategy === "title" ? void 0 : r?.name
  let o = eo(e._def, void 0 === s
    ? n
    : {
        ...n,
        currentPath: [...n.basePath, n.definitionPath, s],
      }, !1) ?? {}
  let a = typeof r == "object" && void 0 !== r.name && r.nameStrategy === "title" ? r.name : void 0
  void 0 !== a && (o.title = a)
  let d = void 0 === s
    ? i
      ? {
      ...
      : o
    : {
        $ref: [...(n.$refStrategy === "relative" ? [] : n.basePath), n.definitionPath, s].join("/"),
        [n.definitionPath]: {
          ...i,
          [s]: o,
        },
      }
  n.target === "jsonSchema7" ? d.$schema = "http://json-schema.org/draft-07/schema#" : (n.target === "jsonSchema2019-09" || n.target === "openAi") && (d.$schema = "https://json-schema.org/draft/2019-09/schema#")
  n.target === "openAi" && ("anyOf" in d || "oneOf" in d || "allOf" in d || "type" in d && Array.isArray(d.type)) && console.warn("Warning: OpenAI may not support schemas with unions as roots! Try wrapping it in an object property.")
  return d
}
export const Ik = $$ec0
