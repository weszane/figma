...
export function useMergeRefs(e) {
  return useMemo(() => e.every(e => e == null) ? null : r => {
    e.forEach(e => {
...
function X(e, r) {
  ...
  if (event.key === U && (stopEvent && I(event), prevIndex === -1 ? v = minIndex : (v = G(e, {
...
function H(e, r, n) {
  ...
  e.forEach((e, o) => {
    ...
    for (n && (s = 0); !d;) {
      ...
      s % r + width <= r && e.every(e => i[e] == null) ? (e.forEach(e => {
        i[e] = o;
      }), d = !0) : s++;
    }
  });
...
function eP(e, r) {
  ...
  for (; i.length;) {
    i = e.filter(e => {
      let r;
      return (r = i) == null ? void 0 : r.some(r => {
        let n;
...
function useFloatingPortalNode(e) {
  ...
  et(() => {
    if (root === null || !i || p.current) return;
...
export function FloatingFocusManager(e) {
  ...
  et(() => {
    if (disabled || !K || typeof MutationObserver != 'function' || L) return;
    let e = () => {
...
export function useDismiss(e, r) {
  ...
  let j = Z(e => {
    let r;
    if (!open || !enabled || !escapeKey || e.key !== 'Escape' || L.current) return;
  ...
  let F = Z(e => {
    ...
    P.current = !1;
    let s = R.current;
    if (R.current = !1, outsidePressEvent === 'click' && s || n || typeof I == 'function' && !I(e)) return;
    let p = A(e);
    let g = `[${ew('inert')}]`;
    ...
    if (sb(p) && V) {
      ...
      if (n && L9(p).direction === 'rtl' && (i = e.offsetX <= p.offsetWidth - p.clientWidth), i || r && e.offsetY > p.clientHeight) return;
    }
...
export function useListNavigation(e, r) {
  ...
  let eI = Z(e => {
    if (eu.current = !1, ef.current = !0, e.which === 229 || !em.current && e.currentTarget === B.current) return;
    ...
    if (es || (e.key === 'Home' && (I(e), ea.current = s, ei(ea.current)), e.key === 'End' && (I(e), ea.current = d, ei(ea.current))), cols > 1) {
      ...
      if (a != null && (ea.current = a, ei(ea.current)), orientation === 'both') return;
    }
    if (tO(e.key, orientation)) {
      ...
      tx(e.key, orientation, rtl) ? loop ? ea.current = r >= d ? allowEscape && r !== listRef.current.length ? -1 : s : G(listRef, {
        ...
        startingIndex: r,
        disabledIndices
      })) : loop ? ea.current = r <= s ? allowEscape && r !== -1 ? listRef.current.length : d : G(listRef, {
        startingIndex: r,
        decrement: !0,
        disabledIndices
...
export function useTypeahead(e, r) {
  ...
  let T = Z(e => {
    ...
    if (x.current.length > 0 && x.current[0] !== ' ' && (r(n, n, x.current) === -1 ? C(!1) : e.key === ' ' && I(e)), n == null || A.current.includes(e.key) || e.key.length !== 1 || e.ctrlKey || e.metaKey || e.altKey) return;
    open && e.key !== ' ' && (I(e), C(!0));
    n.every(e => {
      let r;
      let n;
...
export function useInnerOffset(e, r) {
  ...
  useEffect(() => {
    if (!enabled) return;
    function e(e) {
      if (e.ctrlKey || !r || overflowRef.current == null) return;
      let n = e.deltaY;
...
export function safePolygon(e) {
  ...
  let v = e => {
    ...
    return function (e) {
      ...
      if (clearTimeout(r), !elements.domReference || !elements.floating || placement == null || x == null || _y == null) return;
      let {
        clientX,
        clientY
      } = e;
      ...
      if (P === 'top' && _y >= T.bottom - 1 || P === 'bottom' && _y <= T.top + 1 || P === 'left' && x >= T.right - 1 || P === 'right' && x <= T.left + 1) return O();
      let F = [];
      switch (P) {
        case 'top':
