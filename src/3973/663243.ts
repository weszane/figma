 ...
 */
export function getExperimentConfig(e: {
  status: OperationStatus;
}, experimentName: string, rule: string, keepDeviceValue = false, disableExposureLogging = false): DynamicConfig {
  if (!Statsig.initializeCalled() || getInitialOptions().e2e_ci_test || e.status !== OperationStatus.COMPLETED) {
  ...
  try {
    duration = measureSyncDuration('statsigGetExperimentValDuration', ServiceCategories.GROWTH_PLATFORM, () => {
      config = disableExposureLogging ? Statsig.getExperimentWithExposureLoggingDisabled(experimentName, {
        keepDeviceValue
      }) : Statsig.getExperiment(experimentName, keepDeviceValue);
    }, sampled);
  } catch (err) {
    const userId = getInitialOptions().user_data?.id || 'undefined';
    reportStatsigError(`Errored while fetching experiment config for experiment ${experimentName} and user ${userId}: ${err}`);
  ...
  if (error === null && sampled) {
    trackExperimentEvaluation(experimentName, duration, rule, config.getEvaluationDetails().reason, config.getRuleID(), currentUser);
  }

  // Track invalid bootstrap info if evaluation reason is InvalidBootstrap
  if (config.getEvaluationDetails().reason === EvaluationReason.InvalidBootstrap) {
    const atomStore = atomStoreManager.get<{
      evaluated_keys?: {
        userID?: string;
        customIDs?: string[];
      };
    ...
    if (atomStore) {
      trackInvalidBootstrapInfo(experimentName, atomStore.evaluated_keys?.userID, JSON.stringify(atomStore.evaluated_keys?.customIDs), currentUser?.userID?.toString(), JSON.stringify(currentUser?.customIDs));
    }
  }

  // Log performance metrics for fullscreen view experiment
  if (isInteractionPathCheck() && EXPERIMENT_NAME_FULLSCREEN_VIEW === experimentName) {
