...
function J({
  ...
  useEffect(() => {
    let a = a => {
      let l;
      let r = _j(a);
      if (r && (a.preventDefault(), a.stopPropagation(), (l = r === kV.BACK ? 0 === n ? t.length - 1 : n - 1 : n === t.length - 1 ? 0 : n + 1) !== n && l >= 0 && l < t.length)) {
        let n = t[l];
        if (n) {
...
function eu({
  ...
  let c = "loading" === inlinePreviewStatus;
