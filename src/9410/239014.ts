        ...
        children: [jsx("div", {
          ...
          onClick: () => {
            c && !d && (h("figjam_pages_summary.preview_refreshed", {
              previewPageId: i
            }), m(APILoadingStatus.LOADING), fileApiHandler.updatePageCheckpointThumbnails({
              fileKey: c.key
            }).catch(() => {
...
export function $$j1(e) {
  ...
  let i = useSubscription(PageThumbnailsByFileKeyView, {
    fileKey: t?.key ?? ""
  }, {
    enabled: !!(t?.key && e)
  });
  return resourceUtils.useTransform(i, e => {
    ...
    try {
      ...
      reportError(ServiceCategories.FIGJAM, Error(`PageSummaryPreview: Failed to parse page thumbnail data. Error: ${i.message}.`), {
        tags: {
          fileKey: t?.key
        },
        extra: {
