...
import { Egt, Ez5, FAf, glU, hKj, m1T, NfO, NLJ, qmM, z7E, zkO } from '../figma_app/763686'
...
class L {
  ...
  insertStartingStateFromHubFile({
    ...
    if (!e)
      throw new Error('Hub file ID is not provided')
    let n = async (e) => {
    ...
    return this.startingStateHubFiles[e]
      ? n(this.startingStateHubFiles[e])
      : new Promise((t, i) => {
        this.dispatch(ts({
          hubFileId: e,
          callback: async (e) => {
  ...
  async insertNode(e, t, i, r, n) {
    ...
    if (e.nodeType === 'RECTANGLE') {
      ...
      s = l7.onboarding('create-node', () => this.sceneGraph.createNode(e.nodeType))
    }
    if (!s)
      return null
    ...
    if (this.repositionNode(s, i, r), l7.onboarding('rename-node', () => {
      ...
      l7.onboarding('resize-frame', () => s?.resizeWithoutConstraints(e.width || 100, e.height || 100))
    }
  ...
  getCursorBotNodeInsertionLocation(e) {
    ...
    if (e.locationType === bn.INSIDE_SCENE_NODE) {
      ...
      if (!n) {
        return useDefaultLocationIfParentNotFound
          ? this.getCursorBotNodeInsertionLocation({
              ...e,
    ...
    let i = async () => {
      switch (e.type) {
        ...
        case HQ.FOCUS_INPUT:
        {
          ...
          t
            ? t.focus()
            : logError(tF, 'Failed to find input to focus', {
                selector: e.selector,
              }, {
                reportAsSentryError: !0,
              })
