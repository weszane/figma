...
import { NT, wr, aY } from "../figma_app/741237";
import { f as _$$f } from "../905/940356";
import { ut } from "../figma_app/84367";
import { viewportNavigatorContext } from "../figma_app/298911";
import { Lw, WW, jr, Sf, H7, nG, Ds, Dh, a8, iB, oO, $i, jE } from "../figma_app/467440";
import { ServiceCategories as _$$e } from "../905/165054";
import { l7 } from "../905/189185";
import { cy, WY, E6 } from "../figma_app/387100";
...
import { h0, HQ, mf, zu, JJ, bn, $2, GF, NI } from "../figma_app/61403";
import { tF, Y4, YX, dK, mF, L7, $1 } from "../figma_app/631279";
let O = Lg() ? 0 : 300;
class L {
  constructor(e, t, i, r, n, s, o, l, d, c, u, p, h, m, f, g, _, x, y, C, v) {
    ...
    this.blockUserEvents = () => {
      Ez5.editorState().shouldBlockUserEvents.set(!0);
      Lg() || (this.unblockUserEventsTimeout = setTimeout(() => {
        this.unblockUserEvents();
      }, 6e4));
    };
    this.unblockUserEvents = () => {
      this.setCursorBotTutorialPhase(h0.AWAITING_PLAYBACK);
      Ez5.editorState().shouldBlockUserEvents.set(!1);
  ...
  runTutorial(e, t) {
    ...
    this.currentTutorialRunningPromise.$$finally(() => {
      ...
      Lo(tF, "Tutorial completed or stopped", {
        elapsedDuration: t,
        tutorialName: e.name
      });
    });
  ...
  insertStartingStateFromHubFile({
    ...
    if (!e) throw Error("Hub file ID is not provided");
    let n = async e => {
      let n = qmM.findBoundsToCenterNewTopLevelNode(t + 88, i);
      let s = n.origin.x + 88;
      let o = n.origin.y;
      ...
      await this.insertTemplate({
        ...
        templateInsertionDirection: hKj.CUSTOM_POSITION,
  ...
  async stopCurrentTutorial(e) {
    ...
    this.unblockUserEvents();
    e && sx("Cursor Bot Tutorial Stopped", {
      ...e
    });
  }
  ...
  resetCursorBotStateToDefaults() {
    ...
    this.setCursorType(z7E.DEFAULT);
  ...
  async navigateToPageNodesOrDefaultViewport() {
    ...
    e && e.length > 0 ? NfO.scrollAndZoomIntoView(e) : await this.navigate({
      centerX: 0,
      centerY: 0,
      scale: 3
    });
  ...
  getSelectedNodeIfMatches(e) {
    if ("" !== glU.getFirstSelectedNodeIdForCurrentPage()) {
      let t = glU.getFirstSelectedNodeIdForCurrentPage();
  ...
  computeAndSetTutorialDuration(e) {
    ...
    Lo(tF, "Estimated tutorial duration", {
      tutorialDuration: t,
      tutorialName: e.name
    });
  }
  async setupStateAndCanvas(e) {
    if (this.blockUserEvents(), this.shouldStop = !1, this.timeoutIds = [], this.computeAndSetTutorialDuration(e), e.startingViewportFrameType && this.setCursorBotOverlayType(e.startingViewportFrameType), this.setCursorBotTutorialPhase(h0.PLAYING), this.hasWaved || (this.setTimeoutWrapper({
      ...
      if (prioritizeSelectedNode && (t = this.getSelectedNodeIfMatches(centerNodeMatcher)), t || (t = WY(centerNodeMatcher, this.sceneGraph)), !t && resetStartingState) try {
      ...
      if (t) {
        ...
        let i = NfO.getViewportBounds();
        let r = dK(t, {
          width: i.width,
          height: i.height,
          zoom: this.getViewportInfo().zoomScale
        });
        null == r || isNaN(r) || (e.scale = r);
        await this.navigate(e);
      } else await this.navigateToPageNodesOrDefaultViewport();
    } else await this.navigateToPageNodesOrDefaultViewport();
  }
  ...
  repositionNode(e, t, i) {
    l7.onboarding("reposition-node", () => e.relativeTransform = {
      ...e.relativeTransform,
      m02: t,
      m12: i
    });
  ...
  async insertNode(e, t, i, r, n) {
    ...
    } else s = l7.onboarding("create-node", () => this.sceneGraph.createNode(e.nodeType));
    if (!s) return null;
    if (l7.onboarding("size", () => {
      s && Egt.setSelectedNodeAndCanvas(s.guid, !1);
    }), n) {
      let e = cy(n, this.sceneGraph, this.tutorialParentNode);
      l7.onboarding("append-child", () => s && e?.appendChild(s));
    }
    if (this.repositionNode(s, i, r), l7.onboarding("rename-node", () => {
      s && (s.name = t);
    }), "FRAME" === e.nodeType) l7.onboarding("resize-frame", () => s?.resizeWithoutConstraints(e.width || 100, e.height || 100));else if ("TEXT" === e.nodeType) {
      glU.triggerAction("set-tool-default", null);
      this.setCursorType(z7E.DEFAULT);
      let t = s.parentNode;
      if (null !== t) {
        let e = "CANVAS" === t.type ? t.backgroundColor : t.fills[0].color;
        l7.onboarding("set-text-color", () => {
          s && (s.fills = [{
            ...s.fills[0],
            color: HD(e || UE) ? UE : Uv
          }]);
      ...
      e.shouldCenterAlign && this.setTimeoutWrapper({
        callback: () => {
          l7.onboarding("set-text-horizontal-alignment", () => {
  ...
  getCursorBotNodeInsertionLocation(e) {
    ...
    if (e.locationType === bn.INSIDE_SCENE_NODE) {
      ...
      if (!n) return useDefaultLocationIfParentNotFound ? this.getCursorBotNodeInsertionLocation({
        ...e,
    ...
    } else if (e.locationType === bn.CANVAS_COORDINATE) t = {
      cursorBotX: e.location.x,
      cursorBotY: e.location.y,
      insertionX: e.location.x,
      insertionY: e.location.y
    };else {
      let i = qmM.findBoundsToCenterNewTopLevelNode(e.payload.width ?? 100, e.payload.height ?? 100);
      t = {
    ...
    if (void 0 !== e.cursorPosition && null == e.nodeDrawingAnimationDuration) {
  ...
  async transition(e, t) {
    ...
    let i = async () => {
      switch (e.type) {
        ...
        case HQ.TARGET_SCENE_NODE:
          {
            ...
            e.shouldSelect && Egt.setSelectedNodeAndCanvas(i.guid, !1);
            return;
          }
        case HQ.SELECT_SCENE_NODE:
          {
            ...
            Egt.setSelectedNodeAndCanvas(i.guid, !1);
            return;
          }
        case HQ.INSERT_NODE:
          {
            ...
            e.nodeDrawingAnimationDuration && "FRAME" === e.payload.nodeType && (this.setCursorBotAnimationLayerPosition({
              x: i.cursorBotX,
              y: i.cursorBotY,
              width: n,
              height: r,
        ...
        case HQ.CHANGE_TOOL:
          [NLJ.FRAME, NLJ.TYPE].includes(e.toolType) ? (e.toolType === NLJ.TYPE ? glU.triggerAction("set-tool-type", null) : e.toolType === NLJ.FRAME && glU.triggerAction("set-tool-frame", null), this.setCursorType(z7E.CROSSHAIR)) : e.toolType === NLJ.HAND ? (glU.triggerAction("set-tool-hand", null), this.setCursorType(z7E.HAND)) : e.toolType === NLJ.NONE && (glU.triggerAction("set-tool-default", null), this.setCursorType(z7E.DEFAULT));
          return;
        ...
        case HQ.UPDATE_SELECTED_NODE_PROPERTIES:
          {
            ...
            let i = this.sceneGraph.get(t);
            if (!i) {
              x1(tF, "Failed to find selected node to update", void 0, {
                reportAsSentryError: !0
              });
              return;
            }
            this.appModel.activeCanvasEditModeType === m1T.TEXT && (wr(), Egt.setSelectedNodeAndCanvas(t, !1));
            Y5.updateSelectionProperties({
              ...("function" == typeof e.properties ? e.properties(i) : e.properties)
            }, {
              shouldCommit: zk.YES,
              editScopeType: zkO.ONBOARDING
            });
        ...
        case HQ.FOCUS_INPUT:
          {
            ...
            t ? t.focus() : x1(tF, "Failed to find input to focus", {
              selector: e.selector
            }, {
              reportAsSentryError: !0
            });
